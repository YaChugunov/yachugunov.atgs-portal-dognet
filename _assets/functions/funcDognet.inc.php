            ...
            if ($_QRY1) {
                ...
                while ($_ROW1 = mysqli_fetch_assoc($_QRY1)) {
                    ...
                    while ($_ROW2 = mysqli_fetch_assoc($_QRY2)) {
                        ...
                        $db->update('dognet_kalplanchf_zadol', [
                ...
                if ($action == 'CRT') {
                    ...
                    //
                    //
                    $db->insert('dognet_testingDocStatusZadol', [
                        'koddoc'        => $koddoc,
...
function KALPLANCHF_PR_ZADOLG_CHF($db, $source_tableName, $source_rowID, $action, $kodchfact)
{
    ...
    if ($source_tableName == 'kalplanchf') {
        ...
        if ($action == 'CRT') {
            $db->insert('dognet_kalplanchf_zadol', [
                'kodchfact'     => $__kodchfact,
...
function DOCAVANS_PR_OSTATOK_DOC($db, $source_tableName, $source_rowID, $action)
{
    ...
    if ($source_tableName == 'docavans') {
        ...
        if ($action == 'CRT') {
            ...
            $db->insert('dognet_docavans_ostatok', [
                'kodavans'     => $__kodavans,
                'koddoc'       => $__koddoc,
...
function DOCSTAGE_PR_PROGRESS($db, $source_tableName, $source_rowID, $action)
{
    ...
    if ($source_tableName == 'docavans') {
        if ($action == 'CRT' || $action == 'UPD') {
            ...
            $db->update('dognet_dockalplan_progress', [
                'sumavstage' => $_ROW_S['SumAllAvans'],
            ], ['kodkalplan' => $_QRY[0]['koddoc']]);
        }
        if ($action == 'DEL') {
            $_QRY   = $db->sql('SELECT * FROM dognet_docavans WHERE id=' . $source_rowID)->fetchAll();
            $_QRY_S = mysqlQuery('SELECT SUM(summaavans) as SumAllAvans FROM dognet_docavans WHERE koddoc=' . $_QRY[0]['koddoc']);
        ...
        if ($action == 'CRT') {
            ...
            if ($_QRY && $_QRY[0]['kodshab'] == 1 || $_QRY[0]['kodshab'] == 3) {
                ...
                $db->insert('dognet_dockalplan', [
                    ...
                    'dateplan'      => date('Y-12-31'),
                    'dateoplall'    => date('Y-12-31'),
                ]);

                $db->insert('dognet_dockalplan_progress', [
                    'koddoc'       => $_QRY[0]['koddoc'],
                    'kodkalplan'   => $nextkodkalplan,
...
//
// *!
// *! ЗАПИСЬ КОММЕНТАРИЯ К ЗАЯВКЕ В ТАБЛИЦУ ЧАТОВ ПО ЗАЯВКЕ
// *! @table: dognet_doczayv_logComments
// *!
...
// Функция
//
function doczayv_saveToCommentLog($kod, $act, $txt, $commid)
{
    // Уникальный KOD заявки в БД
    $kodzayv     = !empty($kod) ? $kod : null;
    // Временная метка
    $timestamp   = date('Y-m-d H:i:s');
    // Источник комментария (из формы заявки [FORM] или из чата [COMM])
    $action      = !empty($act) ? $act : null;
    // Текст комментария
    $commentText = !empty($txt) ? $txt : null;
    // Дополнение к комментарию (если источник = [FORM])
    switch ($action) {
        case 'FORM':
            $commentAdd = 'В форме заявки обновлена дополнительная информация';
            break;
        case 'COMMENT':
            $commentAdd = 'Добавлено через чат заявки';
            break;
        case 'EMAIL':
            $commentAdd = 'Добавлено в качестве комментария к рассылке';
            break;
        default:
            $commentAdd = null;
    }
    ...
    //
    $username  = !empty($_SESSION['lastname']) ? $_SESSION['lastname'] : '';
    $username .= !empty($_SESSION['firstname']) ? ' ' . $_SESSION['firstname'] : '';
    $username  = !empty($username) ? $username : null;
    //
    if (!empty($commid)) {
        $_reqGet         = mysqli_fetch_assoc(mysqlQuery("SELECT * FROM dognet_doczayv_logComments WHERE commentID='{$commid}'"));
        //
        $prevcommentText = !empty($_reqGet['commentText']) ? $_reqGet['commentText'] : null;
        //
        $_reqUpd         = mysqlQuery("UPDATE dognet_doczayv_logComments SET
...
\t\t");
        ...
        // ID комментария
        $commentid = !empty($kodzayv) ? $kodzayv . '.Z.' . time() : null;
        //
        $_reqIns   = mysqlQuery("INSERT INTO dognet_doczayv_logComments
\t\t\t(timestamp, action, kodzayv, commentID, userid, username, commentText, commentAdd)
\t\t\tVALUES
\t\t\t('{$timestamp}', '{$action}', '{$kodzayv}', '{$commentid}', '{$userid}', '{$username}', '{$commentText}', '{$commentAdd}')
\t\t");
    }
    $_reqUpdateCntComm = mysqlQuery("UPDATE dognet_doczayv SET cntComments=(SELECT COUNT(*) as cnt FROM dognet_doczayv_logComments WHERE kodzayv='{$kodzayv}') WHERE kodzayv='{$kodzayv}'");
}
