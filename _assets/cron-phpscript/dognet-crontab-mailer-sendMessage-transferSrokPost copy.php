...
$_SQLReq_Docs = "
SELECT * FROM dognet_docbase 
WHERE kodtip IN (245287841608965,245287841599652) 
AND kodstatus IN (245381842747296) 
AND koddel<>'99' 
...
if (isset($_QRY_Docs)) {
	while ($_ROW_Docs = mysqli_fetch_assoc($_QRY_Docs)) {
		...
		echo '<br>($strEndDoc - $strNow): >>> ' . $strEndDoc . " - " . $strNow . " = " . $diff;
		...
		if ($diff <= 7 && $diff >= 6) {
			...
			#
			$_docSrok = $_ROW_Docs['dayenddoc'] . "." . $_ROW_Docs['monthenddoc'] . "." . $_ROW_Docs['yearenddoc'];
			$_docSrok = date("d.m.Y", strtotime($_docSrok));
			$_docName = $_ROW_Docs['docnamefullm'];
			$_docNumber = "3-4/" . $_ROW_Docs['docnumber'];
			$_docSum = number_format($_ROW_Docs['docsumma'], 2, '.', '');
