...
#
function updateFields_blankdocsub($db, $action_blankdocsub, $id, $values, $__deptnum) {
	# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	#
	# :::
	...
	if ($action_blankdocsub == 'CRT') {
		...
		if ($_QRY[0]['kodblankcreate'] == '1') {
			...
			if ($values['sendEmailSUB'] == 1) {
				// SendMessage_NewBlankCreate('dognet_blankdocsub', $id, $_nextNumberblank, 'do@atgs.ru');
				// SendMessage_NewBlankCreate('dognet_blankdocsub', $id, $_nextNumberblank, 'chugunov@atgs.ru');
				// 				SendMessage_NewBlankCreate('dognet_blankdocsub', $id, "ТЕСТ", 'do@atgs.ru');
			}
		}
		#
		#
...
# по тригеру preEdit
#
function updateFields_blankdocsub_preRemove($db, $action_blankdocsub, $id, $values) {
	# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	#
	# :::
	# ::: Если была нажата кнопка "НОВЫЙ СЧЕТ"
	...
	if ($action_blankdocsub == 'DEL') {
		...
		#
		// Удаляем всё...
		$_QRY = $db->sql("SELECT kodblankwork FROM dognet_blankdocsub WHERE id=" . $id)->fetchAll();
...
Editor::inst($db, 'dognet_blankdocsub')
	->fields(
		...
		// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
		// 		Field::inst( 'dognet_blankdocsub.sendblank' ),
		// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	...
	->on('preGet', function ($editor, $id) use ($KODISPOL, $DEPTNUM) {
		...
		$editor->where( function ( $q1 ) use ($KODISPOL, $DEPTNUM) {
		    $q1->where( 'dognet_blankdocsub.kodispol', $KODISPOL );
		    $q1->or_where( 'dognet_blankdocsub.userid_blankcreator', $_SESSION['id'] );
		    $q1->or_where( 'dognet_blankdocsub.userdept_blankcreator', $DEPTNUM );
		} );
*/
		$editor->where(function ($q) {
			...
			$q->and_where(function ($q1) {
				$q1->or_where(function ($q11) {
					$q11->and_where('dognet_blankdocsub.kodtipblank', 'CR', '=');
					$q11->and_where('dognet_blankdocsub.kodblankcreate', '1', '!=');
				});
				$q1->or_where(function ($q12) {
					$q12->and_where('dognet_blankdocsub.kodtipblank', 'CR', '=');
					$q12->and_where('dognet_blankdocsub.kodblankcreate', '1', '=');
				...
				$q1->or_where(function ($q13) {
					$q13->and_where('dognet_blankdocsub.kodtipblank', 'DO', '=');
	...
	->on('postCreate', function ($editor, $id, $values, $row) use ($KODISPOL, $DEPTNUM) {
		updateFields_blankdocsub($editor->db(), 'CRT', $id, $values, $DEPTNUM);
	})
	->on('postEdit', function ($editor, $id, $values, $row) use ($KODISPOL, $DEPTNUM) {
		updateFields_blankdocsub($editor->db(), 'UPD', $id, $values, $DEPTNUM);


		// 		if ( $values['dognet_blankdocsub']['kodusespzakaz'] === "1" ) {
		// 			PR_SENDMAIL();
		// 		}

	})
	->on('preRemove', function ($editor, $id, $values) {
		updateFields_blankdocsub_preRemove($editor->db(), 'DEL', $id, $values);
	})
	->on('postRemove', function ($editor, $id, $values) use ($KODISPOL, $DEPTNUM) {
		updateFields_blankdocsub($editor->db(), 'DEL', $id, $values, $DEPTNUM);
	})
	// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	#
	#
	...
	//  ->where( 'dognet_blankdocsub.userid_blankcreator', $_SESSION['id'] )
	->where('dognet_blankdocsub.userdept_blankcreator', $DEPTNUM)
	->process($_POST)
	->json();
