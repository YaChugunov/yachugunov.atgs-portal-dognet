...
//
function newKodstatus() {
	$query = mysqlQuery("SELECT MAX(kodstatus) as lastKod FROM dognet_spstatus ORDER BY id DESC");
...
#
	if ( $action == 'CRT' ) {
		$newKodstatus = newKodstatus();
		$db->update( 'dognet_spstatus', array(
			'kodtip'			=> $newKodstatus
		), array(
			'id' => $id
		));
