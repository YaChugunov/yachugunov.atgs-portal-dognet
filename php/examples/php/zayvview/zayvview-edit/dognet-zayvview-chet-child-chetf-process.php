...
// 	$Const_Part_Of_PATH = "/STORAGEDOC/DOCOTGRP/OTGR".$__CURRENT_YEAR."/";
	$Const_Part_Of_PATH = "/STORAGEDOC/ZAYVCFZ/CFZ".$__CURRENT_YEAR."/TMP_TEST/";
#
#
...
// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
		Field::inst( 'dognet_doczayvchetf.kodzayvchet' )
			->options( Options::inst()
				...
				->render( function ( $row ) use ($__kodzayvchet) {
					return ("Счет : ".$row['zayvchetnumber']);
					})
				->where(function ($q) use ($__kodzayvchet)  {
					$q->where('kodzayvchet', $__kodzayvchet, '=');
					})
			)
