...
#
        ...
        if ($_QRY_1) {
            ...
            if ($_QRY_specRowID) {
                //
                $db->update('dognet_doczayvdopspec', [
                    'docFileID' => "",
...
#
# UPD 15.07.23
# Пишем комментарий заявителя в общий чат с комментариями
#
##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### #####
#
#
...
{
    $reqDB      = $db->sql("SELECT kodzayv, zayvchetcom FROM dognet_doczayv WHERE id=" . $id)->fetchAll();
    $_timestamp = date("Y-m-d H:i:s");
    $_kodzayv   = $reqDB[0]['kodzayv'];
    $_commentID = $_kodzayv . ".Z." . time();
    $_userid    = $_SESSION['id'];
    $_username  = $_SESSION['lastname'] . " " . $_SESSION['firstname'];
    if ('CRT' == $action) {
        $docCommentForm = $values['dognet_doczayv']['zayvchetcom'];
        if (! empty($docCommentForm)) {
            $db->insert('dognet_doczayv_logComments', [
                'koddel'          => "",
    ...
    if ('UPD' == $action) {
        $docCommentForm = $values['dognet_doczayv']['zayvchetcom'];
        if ($docCommentForm != $reqDB[0]['zayvchetcom']) {
            $db->insert('dognet_doczayv_logComments', [
                'koddel'          => "",
                'timestamp'       => $_timestamp,
                'action'          => 'FORM',
...
{
    $reqDB    = $db->sql("SELECT kodzayv FROM dognet_doczayv WHERE id=" . $id)->fetchAll();
    $_kodzayv = $reqDB[0]['kodzayv'];
    $_reqDB1  = $db->sql("SELECT COUNT(*) as CommCounts FROM dognet_doczayv_logComments WHERE action IN ('COMMENT','FORM','EMAIL') AND kodzayv='{$_kodzayv}'")->fetchAll();
    $counts   = $_reqDB1[0]['CommCounts'];
    $_reqDB2  = $db->sql("UPDATE dognet_doczayv SET cntComments = '{$counts}' WHERE kodzayv = '{$_kodzayv}'");
}
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
#
#
...
// ----- ----- -----
        Field::inst('dognet_doczayv.koddoc')
            ->options(Options::inst()
                    ...
                    ->value('koddoc')
                    ->label(['koddoc', 'yearnachdoc', 'docnumber', 'docnameshot'])
                    ->render(function ($row) {
...
// ----- ----- -----
        Field::inst('dognet_doczayv.kodzayvtel')
            ->options(Options::inst()
                    ...
                    ->value('kodzayvtel')
                    ->label(['kodzayvtel', 'namezayvtelshot'])
                    ->render(function ($row) {
...
// ----- ----- -----
        Field::inst('dognet_doczayv.kodtipzayvall')
            ->options(Options::inst()
                    ...
                    ->value('kodtipzayvall')
                    ->label(['kodtipzayvall', 'nametipzayvshotall'])
                    ->render(function ($row) {
...
// ----- ----- -----
        ...
        Field::inst('dognet_doczayv.cntComments'),
// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
...
// ---
                    ...
                    ->validator(Validate::fileExtensions(['png', 'jpg', 'pdf', 'doc', 'docx', 'xls', 'xlsx'], "Загрузите документ"))
            ),
// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
...
// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    ...
    ->on('preEdit', function ($editor_doczayv, $id, $values) {
        updateLogComments($editor_doczayv->db(), 'UPD', $id, $values);
        updateCountComments($editor_doczayv->db(), $id);
    })
    ->on('postCreate', function ($editor_doczayv, $id, $values, $row) use ($varFileArray) {
        ...
        updateLogComments($editor_doczayv->db(), 'CRT', $id, $values);
        updateCountComments($editor_doczayv->db(), $id);
    })
