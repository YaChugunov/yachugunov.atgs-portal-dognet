...
#
function updateFields_doczayvchet($db, $action_doczayvchet, $id, $values, $varFileArray, $kodzayv) {
	# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	#
	# :::
...
if (!isset($_POST['kodzayv']) || !is_numeric($_POST['kodzayv'])) {
	...
	Editor::inst($db, 'dognet_doczayvchet')
			...
			Field::inst('dognet_doczayvchet.kodpokup')
				->options(
					Options::inst()
						...
						->value('kodpokup')
						->label(array('kodpokup', 'namepokupshot', 'namepokupfull'))
						->render(function ($row) {
							return ($row['namepokupfull']);
						})
						->where(function ($q) {
							$q->where('koddel', '99', '!=');
						})
		...
		// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
		->on('postCreate', function ($editor_doczayvchet, $id, $values, $row) use ($varFileArray, $__kodzayv) {
			updateFields_doczayvchet($editor_doczayvchet->db(), 'CRT', $id, $values, $varFileArray, $__kodzayv);
		})
		// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
		->on('postEdit', function ($editor_doczayvchet, $id, $values, $row) use ($varFileArray, $__kodzayv) {
			updateFields_doczayvchet($editor_doczayvchet->db(), 'UPD', $id, $values, $varFileArray, $__kodzayv);
		})
		// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
		->on('preRemove', function ($editor_doczayvchet, $id, $values) use ($varFileArray, $__kodzayv) {
			delAttachment($editor_doczayvchet->db(), $id);
			delLinked_Chf_and_Files($editor_doczayvchet->db(), $id);
			updateFields_doczayvchet($editor_doczayvchet->db(), 'DEL', $id, $values, null, $__kodzayv);
		})
		// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
