...
# для таблицы этапов 'dognet_docbase'
# ----- ----- -----
function nextNumberdop($kodzayvdop) {
	$query = mysqlQuery("SELECT MAX(numberdop) as lastNumber FROM dognet_doczayvdop WHERE kodzayvdop=".$kodzayvdop." ORDER BY id DESC");
	$row = mysqli_fetch_assoc($query);
...
#
function updateFields_doczayvdop ( $db, $action_doczayvchet, $id, $values ) {
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
#
# :::
...
// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	->on( 'postCreate', function ( $editor_docbase, $id, $values, $row ) {
		updateFields_doczayvdop( $editor_docbase->db(), 'CRT', $id, $values );
	} )
// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	->on( 'postEdit', function ( $editor_docbase, $id, $values, $row ) {
		updateFields_doczayvdop( $editor_docbase->db(), 'UPD', $id, $values );
	} )
/*
...
*/
	->on( 'preRemove', function ( $editor_docbase, $id, $values ) {
		updateFields_doczayvdop( $editor_docbase->db(), 'DEL', $id, $values );
	} )
