...
#
function updateFields_doczayvchet_from($db, $action_doczayvchetf, $id) {
	# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	#
	# :::
...
# Функция обновления полей основной таблицы (dognet_kalplanchf)
#
function updateFields_doczayvchetf($db, $action_doczayvchetf, $id, $values, $varFileArray) {
	# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	#
	# :::
	...
	if ($action_doczayvchetf == 'CRT') {
		...
		$_QRY = $db->sql("SELECT kodzayvchet, zayvchetfsumma, kodusevalid FROM dognet_doczayvchetf WHERE id=" . $id)->fetchAll();
		$kod = $_QRY[0]['kodzayvchet'];
		$sumchf = $_QRY[0]['zayvchetfsumma'];
		$_QRY1 = $db->sql("SELECT zayvchetsumma FROM dognet_doczayvchet WHERE kodzayvchet=" . $kod)->fetchAll();
		$sumch = $_QRY1[0]['zayvchetsumma'];
		...
		$controlzadol = ($perc < 100 && $zadol > 0) ? 1 : 0;
		//
		$db->update('dognet_doczayvchet', array(
			'zayvchetpr' => $perc,
			'zayvchetzadol' => $zadol,
		...
		// .....
		$kodvalid = $_QRY[0]['kodusevalid'];
		switch ($kodvalid) {
			case "0":
				$namevalid = "Не определено";
				break;
			case "1":
				$namevalid = "Проверено";
				break;
			case "2":
				$namevalid = "Не проверено";
				break;
			default:
				$namevalid = "";
		}
	...
	if ($action_doczayvchetf == 'UPD') {
		...
		$_QRY_1 = $db->sql("SELECT kodzayvchet, zayvchetfsumma, kodusevalid FROM dognet_doczayvchetf WHERE id=" . $id)->fetchAll();
		...
		if ($_QRY_3) {
			...
			if ($_docFileID != "") {
				...
				if ($_QRY_paperfiles[0]['flag'] == "0") {
					...
					$__newFileName = "NEW-CFZ{$_kodzayvchetf}-" . $_SESSION['id'] . "-" . date("YmdHis");
...
Editor::inst($db, 'dognet_doczayvchetf')
	->fields(
		...
		Field::inst('dognet_doczayvchetf.kodzayvchet')
			->options(
				Options::inst()
					->table('dognet_doczayvchet')
					->value('kodzayvchet')
					->label(array('kodzayv', 'kodzayvchet', 'zayvchetnumber', 'zayvchetdate'))
					->render(function ($row) {
						$date = date('d.m.Y', strtotime($row['zayvchetdate']));
						$year = date('Y', strtotime($row['zayvchetdate']));
						return $year . " : счет № " . $row['zayvchetnumber'] . " от " . $date;
					})
					->where(function ($q) {
						$q->where('koddel', '99', '!=');
						$q->where('YEAR(zayvchetdate)', (date("Y") - 1), '>=');
					})
			)
			->validator(Validate::notEmpty(
				ValidateOptions::inst()
					->message('Счет обязателен')
			)),
		// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	...
	// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	->on('postCreate', function ($editor_doczayvchetf, $id, $values, $row) use ($varFileArray) {
		updateFields_doczayvchetf($editor_doczayvchetf->db(), 'CRT', $id, $values, $varFileArray);
	})
	// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	->on('preEdit', function ($editor_doczayvchetf, $id, $values) {
		updateFields_doczayvchet_from($editor_doczayvchetf->db(), 'UPD', $id);
	})
	// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	->on('postEdit', function ($editor_doczayvchetf, $id, $values, $row) use ($varFileArray) {
		updateFields_doczayvchetf($editor_doczayvchetf->db(), 'UPD', $id, $values, $varFileArray);
	})
	// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	->on('preRemove', function ($editor_doczayvchetf, $id, $values) use ($varFileArray) {
		delAttachment($editor_doczayvchetf->db(), $id);
		updateFields_doczayvchetf($editor_doczayvchetf->db(), 'DEL', $id, $values, null);
	})
	#
	#
	// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	...
	->where('YEAR(dognet_doczayvchetf.zayvchetfdate)', (date("Y") - 1), '>=')
	->process($_POST)
	->json();
