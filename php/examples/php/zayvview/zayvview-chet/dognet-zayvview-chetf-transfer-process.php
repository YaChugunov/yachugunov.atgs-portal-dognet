...
#
function updateFields ( $db, $id, $values, $row, $chetfRowID, $kodzayvchet_from ) {

	$_QRY_CHET = $db->sql( "SELECT kodzayvchet FROM dognet_doczayvchet WHERE id=".$id )->fetchAll();
	$_QRY_CHETF = mysqlQuery( "UPDATE dognet_doczayvchetf SET kodzayvchet=".$_QRY_CHET[0]['kodzayvchet']." WHERE id=".$chetfRowID );
...
// Для счета, с которого переносится счет-фактура
			$_QRY_CHET_FROM = $db->sql( "SELECT zayvchetsumma FROM dognet_doczayvchet WHERE kodzayvchet=".$kodzayvchet_from )->fetchAll();
			$sumch_from = $_QRY_CHET_FROM[0]['zayvchetsumma'];

			$_QRY_CHETF_FROM = $db->sql( "SELECT SUM(zayvchetfsumma) as SummaChf FROM dognet_doczayvchetf WHERE kodzayvchet=".$kodzayvchet_from )->fetchAll();
...
if ( !isset($_POST['chfRowID']) || !is_numeric($_POST['chfRowID']) ) {
	echo json_encode( [ "data" => [] ] );
}
else {
$__chetfRowID = $_POST['chfRowID'];
$__kodzayvchet_from = $_POST['kodzayvchet_from'];
...
#
// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	->on( 'postEdit', function ( $editor_docbase, $id, $values, $row ) use ($__chetfRowID, $__kodzayvchet_from)  {
		updateFields( $editor_docbase->db(), $id, $values, $row, $__chetfRowID, $__kodzayvchet_from );
	} )
// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
// 	->leftJoin( 'dognet_doczayvchet', 'dognet_doczayvchet.kodzayvchet', '=', 'dognet_doczayvchetf.kodzayvchet' )
  ->where( 'dognet_doczayvchetf.id', $__chetfRowID )
	->process( $_POST )
	->json();
}