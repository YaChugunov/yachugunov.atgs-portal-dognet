...
function updateFields_doczayvchet($db, $action_doczayvchet, $id, $values, $varFileArray) {
	...
	if ($action_doczayvchet == 'UPD') {
		...
		$_QRY_1 = $db->sql("SELECT kodcontragent, kodzayv, zayvchetsumma FROM dognet_doczayvchet WHERE id=" . $id)->fetchAll();
...
Editor::inst($db, 'dognet_doczayvchet')
	...
	->where('YEAR(dognet_doczayvchet.zayvchetdate)', (date("Y") - 1), '<=')
	->process($_POST)
	->json();
