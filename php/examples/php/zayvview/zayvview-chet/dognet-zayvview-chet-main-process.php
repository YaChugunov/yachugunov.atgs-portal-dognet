...
#
function updateFields_doczayvchet($db, $action_doczayvchet, $id, $values, $varFileArray) {
	# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	#
	# :::
		...
		if ($_QRY_3) {
			...
			if ($_docFileID != "") {
				...
				if ($_QRY_paperfiles[0]['flag'] == "0") {
					...
					$__newFileName = "NEW-CTZ{$_kodzayvchet}-" . $_SESSION['id'] . "-" . date("YmdHis");
...
Editor::inst($db, 'dognet_doczayvchet')
	->fields(
		...
		// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
		Field::inst('dognet_doczayvchet.koddened')
			->options(
				Options::inst()
		...
		Field::inst('dognet_doczayvchet.srokpostavki'),
		// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	...
	// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	->on('postCreate', function ($editor_doczayvchet, $id, $values, $row) use ($varFileArray) {
		updateFields_doczayvchet($editor_doczayvchet->db(), 'CRT', $id, $values, $varFileArray);
	})
	//
	->on('postEdit', function ($editor_doczayvchet, $id, $values, $row) use ($varFileArray) {
		updateFields_doczayvchet($editor_doczayvchet->db(), 'UPD', $id, $values, $varFileArray);
	})
	//
	->on('preRemove', function ($editor_doczayvchet, $id, $values) use ($varFileArray) {
		delAttachment($editor_doczayvchet->db(), $id);
		delLinked_Chf_and_Files($editor_doczayvchet->db(), $id);
		updateFields_doczayvchet($editor_doczayvchet->db(), 'DEL', $id, $values, null);
	})
	...
	->leftJoin('dognet_spdened', 'dognet_spdened.koddened', '=', 'dognet_doczayvchet.koddened')
	...
	->where('YEAR(dognet_doczayvchet.zayvchetdate)', (date("Y") - 2), '>=')
	->process($_POST)
	->json();
