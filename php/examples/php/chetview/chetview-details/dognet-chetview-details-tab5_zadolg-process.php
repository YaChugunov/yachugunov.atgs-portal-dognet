...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- 
function SummaStage($varKodkalplan) { 
// Из таблицы dognet_dockalplan определяем код(ы) этапа(ов) - kodkalplan
	$query_getSummastage = mysqlQuery( "SELECT summastage FROM dognet_dockalplan WHERE koddoc = ".$varKoddoc." AND kodkalplan = ".$varKodkalplan." AND koddel <> '99'" );
...
// Из таблицы dognet_kalplanchf определяем код(ы) счета(ов)-фактур - kodchfact относящихся к соответствующему этапу (календарному плану)
	...
	return $varSummastage - ($varSummaOplatChf+$varSummaAvansChf);
}
