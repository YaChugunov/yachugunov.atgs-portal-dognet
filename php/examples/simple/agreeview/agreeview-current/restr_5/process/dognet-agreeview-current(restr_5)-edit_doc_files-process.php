...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	...
	// Выбираем идентификаторы создаваемого документа
		$_QRY_docPaper = $db->sql( "SELECT koddocpaper, koddoc, kodpaper, docFileID FROM dognet_docpaper WHERE id=".$id )->fetchAll();
		if ($_QRY_docPaper) {
...
#
			...
			// Переименовываем файл
				...
				if ($_QRY_paperfiles[0]['flag'] == "0") {
					...
					$__newFileName = "NEW-DPU{$_koddocpaper}";
...
#
#
	$__rowFileID = $db->sql( "SELECT docFileID FROM dognet_docpaper WHERE id=".$id )->fetchAll();
...
// Database table to update
	            $db->update( 'dognet_docpaper_files', [
								'flag' => '0',
// ---
...
// ---
          ...
          ->db( 'dognet_docpaper_files', 'id', array(
