...
// :::
?>
<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/docview/docview-details/restr_5/tabs/css/docview-details-common-tab2_progress.css">
...
<?php
	...
	if (mysqli_num_rows($query_getKalplan) >= 1) {
		while ($row_getKalplan = mysqli_fetch_array($query_getKalplan, MYSQLI_ASSOC)) {
			...
			$objectready = $row_getKalplan['idobjectready'];
# ----- ----- ----- ----- -----
...
?>
				<tr>
					<td class="docview-details-tab2-numberstage text-center"><?php echo $row_getKalplan['numberstage']; ?></td>
					<td class="docview-details-tab2-namestage text-left"><?php echo $row_getKalplan['nameshotstage']; ?></td>
					<td class="docview-details-tab2-dateplan text-left"><?php echo $firstavans; ?></td>
					<td class="docview-details-tab2-dateplan text-left <?php echo ($objectready) ? "highlight_warning" : ""; ?>"><?php echo $expiry_date; ?></td>
					<td class="docview-details-tab2-summastage text-right"><?php echo number_format((float)($row_getKalplan['summastage']), 2, '.', ' ').$__dened; ?></td>
					<td class="docview-details-tab2-summastage text-right"><?php echo number_format((float)(StageSumChf($row_getKalplan['kodkalplan'])), 2, '.', ' ').$__dened; ?></td>
					<td class="docview-details-tab2-summastage <?php echo (StageZadolg($row_getKalplan['kodkalplan']) <> 0 ? 'text-danger' : 'text-default'); ?> text-right"><?php echo number_format((float)(StageZadolg($row_getKalplan['kodkalplan'])), 2, '.', ' ').$__dened; ?></td>
				</tr>
<?php
		// ----- ----- -----
...
?>
			...
			<tfoot>
				<tr>
					...
					<th class="docview-details-tab2-summastage text-right"><?php echo number_format((float)($totalStage), 2, '.', ' ').$__dened; ?></th>
					<th class="docview-details-tab2-summastage text-right"><?php echo number_format((float)($totalSumChf), 2, '.', ' ').$__dened; ?></th>
					<th class="docview-details-tab2-summastage <?php echo ($totalZadolg <> 0 ? 'text-danger' : 'text-default'); ?> text-right"><?php echo number_format((float)($totalZadolg), 2, '.', ' ').$__dened; ?></th>
				</tr>
			</tfoot>
		</table>
		<table id="compact-legend-table" class="table table-responsive" cellspacing="0" width="100%">
			<tbody>
				...
				<tr>
					...
					<td width="2%"><span class=""></span></td>
					<td>
						<span>1. Если срок выполнения этапа указан в виде даты (ДД/ММ/ГГГГ), то этот срок равен этой дате.</span><br>
						<span>2. Если срок выполнения этапа указан в днях, то этот срок равен дате 1-го аванса + количество дней.</span><br>
						<span>3. Если срок выполнения этапа не указан, то этот срок равен планируемой дате либо дате конца года (31/12/ГГГГ).</span>
					</td>
				</tr>
			</tbody>
		</table>
...
?>
				<tr>
					<td colspan="6" class="docview-details-tab2-txt text-left text-danger" style="text-align:left">По данному договору этапы не определены</td>
				</tr>
			</tbody>
		</table>
<?php
