...
table_sub_child_avans = $('#docview-sub-child-avans').DataTable( {
		dom: "<'row'<'col-sm-5'><'col-sm-4'><'col-sm-3'>>" + "<'row'<'col-sm-12'tr>>" + "<'row'<'col-sm-1'><'col-sm-5'><'col-sm-6'>>",
		language: {
...
/* 								$(row).css({ 'color':'#FFF'	}); */
		...
		columnDefs: [
          ...
          var selectedChf = table_tab6_chfsubpodr.row( { selected: true } );
...
table_sub_child_chetf.on( 'deselect', function () {
	table_sub_child_oplatachf.row( { selected: true } ).deselect();
  table_sub_child_oplatachf.ajax.reload(null, false);
	table_sub_child_avans.row( { selected: true } ).deselect();
  table_sub_child_avans.ajax.reload(null, false);
} );
// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
} );
...
//
?>
<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/docview/docview-details/restr_5/tabs/css/docview-details-common-tab6_sub_child_avans.css">
<h3 class="docview-details-title2 text-right">Авансовые платежи</h3>
<div class="demo-html"></div>
<table id="docview-sub-child-avans" class="table table-responsive table-bordered display compact" cellspacing="0" width="100%">
	<thead>
		<tr>
			<th></th>
			<th>ID аванса</th>
