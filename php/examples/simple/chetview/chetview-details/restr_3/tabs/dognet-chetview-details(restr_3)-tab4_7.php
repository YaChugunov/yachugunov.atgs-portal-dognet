<style>
	#chetview-details-tab4_7>thead {
		display: none
	}

	#chetview-details-tab4_7>tbody>tr>td:first-child {
		width: 20px
	}
	...
	#chetview-details-tab4_7>thead>tr>th {
		border: none
	}

	#chetview-details-tab4_7>tbody>tr>td>a {
		color: #3B7087;
		text-decoration: underline
	}

	#chetview-details-tab4_7>tbody>tr>td>a:hover {
		color: #333;
				...
				render: function(data, type, row, meta) {
					if (data) {
						if (row.dognet_docpaper.paperfull != "") {
							return '<a href="' + row.dognet_docpaper_files.file_url +
								'" target="_blank">' + row.dognet_docpaper.paperfull +
								'.<span style="text-transform:lowercase">' + row
								.dognet_docpaper_files.file_extension + '</span></a>';
						} else {
							return '<a href="' + row.dognet_docpaper_files.file_url +
								'" target="_blank">' + data +
								'.<span style="text-transform:lowercase">' + row
								.dognet_docpaper_files.file_extension +
								'</span></a> (описания файла не найдено, использовано системное имя файла)';
						}
					} else {
						return "";
					}
