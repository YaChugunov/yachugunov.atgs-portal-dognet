...
if (isset($_SESSION['password']) && isset($_SESSION['login'])) {
    if (checkUserAuthorization($_SESSION['login'], $_SESSION['password']) == -1) {
        ...
        if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
            if (!empty($uid)) {
                $_reqDoc = mysqli_fetch_array(mysqlQuery("SELECT * FROM dognet_docbase WHERE koddoc='{$uid}' AND koddel!='99' AND numberchet!=''"));
                //*
                //* Номер договора
                $docnumber = !empty($_reqDoc['docnumber']) ? $_reqDoc['docnumber'] : '';
                $chetnumber = !empty($_reqDoc['numberchet']) ? $_reqDoc['numberchet'] : '';
                //*
                ...
                $_reqCntChf = mysqli_fetch_array(mysqlQuery("SELECT COUNT(kodchfact) as cntChf FROM dognet_kalplanchf WHERE kodkalplan IN (SELECT koddoc FROM dognet_docbase WHERE koddoc='{$uid}' AND koddel!='99') AND koddel!='99'"));
                $cntChf = !empty($_reqCntChf) ? $_reqCntChf['cntChf'] : 0;
                $cntStages = $cntChf;
                //*
                //* Считаем количество авансов
                $_reqCntAv = mysqli_fetch_array(mysqlQuery("SELECT COUNT(kodavans) as cntAv FROM dognet_docavans WHERE koddoc IN (SELECT koddoc FROM dognet_docbase WHERE koddoc='{$uid}' AND koddel!='99') AND koddel!='99'"));
                $cntAv = !empty($_reqCntAv) ? $_reqCntAv['cntAv'] : 0;
                //*
                //* Считаем количество субподрядных договоров
                $_reqCntSubs = mysqli_fetch_array(mysqlQuery("SELECT COUNT(koddocsubpodr) as cntSubs FROM dognet_docsubpodr WHERE koddoc IN (SELECT koddoc FROM dognet_docbase WHERE koddoc='{$uid}' AND koddel!='99') AND koddel!='99'"));
