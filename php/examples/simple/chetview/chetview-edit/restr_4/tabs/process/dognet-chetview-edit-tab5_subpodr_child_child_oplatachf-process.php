...
# ----- ----- ----- 
function updateFields_oplchfsubpodr( $db, $action, $id, $values ) { 

// ID счета фактуры
	$_QRY_1 = $db->sql( "SELECT kodchfsubpodr FROM dognet_docoplchfsubpodr WHERE id=".$id )->fetchAll();
	if ( $_QRY_1 ) { $__kodchfsubpodr = $_QRY_1[0]['kodchfsubpodr']; }
#
#
	if ( $action == 'CRT' ) { 
...
function update_chfZadol( $db, $id, $kodchfsubpodr ) { 
		...
		if ( $_QRY_1 ) { $__sumchfsubpodr = $_QRY_1[0]['sumchfsubpodr']; }
	// ----- 
		...
		if ( $_QRY_2 ) { $__sumoplata = $_QRY_2[0]['sumoplata']; }
	// ----- 
...
function update_otherChfZadol( $db, $id, $koddocsubpodr, $kodchfsubpodr ) { 
	...
	// ----- 
		...
		if ( $_QRY_2 ) { $__sumoplata = $_QRY_2[0]['sumoplata']; }
	// ----- 
		$_QRY_3 = $db->sql( "SELECT SUM(sumavsubpodr) as sumavans FROM dognet_docavsubpodr WHERE kodchfsubpodr=".$kod )->fetchAll();
...
// ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- 		
			...
			->on( 'postCreate', function ( $editor, $id, $values, $row ) { 
				updateFields_oplchfsubpodr( $editor->db(), 'CRT', $id, $values );
				update_chfZadol( $editor->db(), $id, $_POST['kodchfsubpodr_oplchf'] );
				update_otherChfZadol( $editor->db(), $id, $_POST['koddocsubpodr2'], $_POST['kodchfsubpodr_oplchf'] );
			} )
			->on( 'postEdit', function ( $editor, $id, $values, $row ) {
				updateFields_oplchfsubpodr( $editor->db(), 'UPD', $id, $values );
				update_chfZadol( $editor->db(), $id, $_POST['kodchfsubpodr_oplchf'] );
				update_otherChfZadol( $editor->db(), $id, $_POST['koddocsubpodr2'], $_POST['kodchfsubpodr_oplchf'] );
			} )
			->on( 'preRemove', function ( $editor, $id, $values ) {
				updateFields_oplchfsubpodr( $editor->db(), 'DEL', $id, $values );
			} )
