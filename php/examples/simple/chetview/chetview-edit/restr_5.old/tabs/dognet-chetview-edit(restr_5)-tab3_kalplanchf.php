...
// Изменяем размер диалогового окна редактирования договора субподряда
	editor_tab3_kalplanchf.on( 'open', function () { $(".modal-dialog").css({
		"width":"60%",
		"min-width":"800px",
		"max-width":"1170px"
		});
	} );
	editor_tab3_kalplanchf.on( 'close', function () { $(".modal-dialog").css({
		"width":"60%",
		"min-width":"none",
		"max-width":"none"
		});
	...
	var table_tab3_kalplanchf = $('#chetview-edit-tab3_kalplanchf').DataTable( {
	      ...
	      { data: "dognet_docbase.kodshab",
	        render: function ( data, type, row ) {
				...
				{ data: "dognet_kalplanchf.kodchfact", className: "" },
				{ data: "dognet_kalplanchf.chetfnumber", className: "" },
				{ data: "dognet_kalplanchf.chetfdate", className: "" },
				{ data: "dognet_kalplanchf.chetfsumma", className: "" },
				{ data: "dognet_kalplanchf_zadol.chetfsumzadol", className: "" }
			],
			columnDefs: [
				{ orderable: true, searchable: false, visible: false, targets: 0 },
	    ...
	    { text:	'<span class="glyphicon glyphicon-refresh"></span>', action:
	      function ( e, dt, node, config ) {
			...
			{ extend: "create", editor: editor_tab3_kalplanchf, text: "НОВЫЙ СЧЕТ",
				formButtons:
					[ 'Создать',
						{ text: 'Отмена', action: function () { this.close(); } }
					]
			},
			{ extend: "edit", editor: editor_tab3_kalplanchf, text: "ИЗМЕНИТЬ",
				formButtons:
					[ 'Изменить',
						{ text: 'Отмена', action: function () { this.close(); } }
					]
			},
			{ extend: "remove", editor: editor_tab3_kalplanchf, text: "УДАЛИТЬ",
				formButtons:
					[ 'Удалить',
						{ text: 'Отмена', action: function () { this.close(); } }
					]
...
// ОПЛАТА СЧЕТОВ-ФАКТУР : форма редактирования
	var editor_tab3_oplatachf = new $.fn.dataTable.Editor( {
      ...
      data: function ( d ) {
          var selected = table_tab3_kalplanchf.row( { selected: true } );
          if ( selected.any() ) {
    ...
    i18n: {
        create: { title: "<h3>Добавить новую оплату по счету-фактуре</h3>" },
        edit: { title: "<h3>Изменить оплату</h3>" },
        remove: {
            button: "Удалить",
...
//
// Изменяем размер диалогового окна редактирования договора субподряда
	editor_tab3_oplatachf.on( 'open', function () { $(".modal-dialog").css({
		"width":"65%",
		"min-width":"800px",
		"max-width":"1170px"
		});
	} );
	editor_tab3_oplatachf.on( 'close', function () { $(".modal-dialog").css({
		"width":"60%",
		"min-width":"none",
		"max-width":"none"
		});
...
// 		dom: "Bfrtip",
      ...
      data: function ( d ) {
          ...
          if ( selected.any() ) {
                d.kodchfact = selected.data().dognet_kalplanchf.kodchfact;
                d.kodkalplan = selected.data().dognet_kalplanchf.kodkalplan;
                console.log("KalplanChf ("+selected.id()+") :: kodchfact: "+d.kodchfact+", kodkalplan: "+d.kodkalplan);
          }
          var selected2 = table_tab3_oplatachf.row( { selected: true } );
          if ( selected2.any() ) {
	  ...
	  select: { style: 'single' },
		columns: [
			{ data: "dognet_oplatachf.kodoplata", className: "" },
			{ data: "dognet_oplatachf.dateopl", className: "" },
			{ data: "dognet_oplatachf.summaopl", className: "" }
		],
		columnDefs: [
		...
		buttons: [
			{ extend: "create", editor: editor_tab3_oplatachf, text: '<span class="glyphicon glyphicon-plus"></span>',
				formButtons:
					[ 'Создать',
						{ text: 'Отмена', action: function () { this.close(); } }
					]
			},
			{ extend: "edit", editor: editor_tab3_oplatachf, text: '<span class="glyphicon glyphicon-pencil"></span>',
				formButtons:
					[ 'Изменить',
						{ text: 'Отмена', action: function () { this.close(); } }
					]
			},
			{ extend: "remove", editor: editor_tab3_oplatachf, text: '<span class="glyphicon glyphicon-remove"></span>' }
		]
	} );
//
// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
...
// АВАНСЫ ЗАЧТЕННЫЕ ПО СЧЕТАМ-ФАКТУРАМ : форма редактирования
	var editor_tab3_chfavans = new $.fn.dataTable.Editor( {
      ...
      data: function ( d ) {
          var selected_chfavans = table_tab3_kalplanchf.row( { selected: true } );
    ...
    i18n: {
        ...
        edit: { title: "<h3>Изменить зачтенную сумму</h3>" },
        remove: {
            button: "Удалить",
...
//
// Изменяем размер диалогового окна редактирования договора субподряда
	editor_tab3_chfavans.on( 'open', function () { $(".modal-dialog").css({
		"width":"70%",
		"min-width":"800px",
		"max-width":"1170px"
		});
	} );
	editor_tab3_chfavans.on( 'close', function () { $(".modal-dialog").css({
		"width":"60%",
		"min-width":"none",
		"max-width":"none"
		});
...
// АВАНСЫ ЗАЧТЕННЫЕ ПО СЧЕТАМ-ФАКТУРАМ : таблица
	var table_tab3_chfavans = $('#chetview-edit-tab3_avans').DataTable( {
	  ...
	  select: { style: 'single' },
		columns: [
			{ data: "dognet_chfavans.kodavans", className: "" },
			{ data: "dognet_docavans.dateavans", className: "" },
			{ data: "dognet_chfavans.summaoplav", className: "" }
		],
		columnDefs: [
			{ orderable: false, searchable: false, targets: 0 },
			{
		...
		buttons: [
			{ extend: "create", editor: editor_tab3_chfavans, text: '<span class="glyphicon glyphicon-plus"></span>',
				formButtons:
					[ 'Создать',
						{ text: 'Отмена', action: function () { this.close(); } }
					]
			},
			{ extend: "edit", editor: editor_tab3_chfavans, text: '<span class="glyphicon glyphicon-pencil"></span>',
				formButtons:
					[ 'Изменить',
						{ text: 'Отмена', action: function () { this.close(); } }
					]
			},
			{ extend: "remove", editor: editor_tab3_chfavans, text: '<span class="glyphicon glyphicon-remove"></span>' }
		]
	} );
// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
//
...
// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	table_tab3_kalplanchf.on( 'select', function () {
	    ...
	    editor_tab3_oplatachf
	        ...
	        .def( table_tab3_kalplanchf.row( { selected: true } ).data().dognet_kalplanchf.kodchfact );
...
// :::
include($_SERVER['DOCUMENT_ROOT']."/dognet/php/examples/simple/chetview/chetview-edit/restr_5/tabs/forms/chetview-edit_tab3_kalplanchf-customForm.php");
include($_SERVER['DOCUMENT_ROOT']."/dognet/php/examples/simple/chetview/chetview-edit/restr_5/tabs/forms/chetview-edit_tab3_kalplanchf_oplata-customForm.php");
include($_SERVER['DOCUMENT_ROOT']."/dognet/php/examples/simple/chetview/chetview-edit/restr_5/tabs/forms/chetview-edit_tab3_kalplanchf_avans-customForm.php");
?>
<section>
<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/chetview/chetview-edit/restr_5/tabs/css/chetview-edit-tab3_kalplanchf.css">
	<div class="" style="padding-left:5px; padding-right:5px">
		<div class="space30"></div>
		<h3 class="parent-title space20">Счета-фактуры</h3>
		<div class="demo-html"></div>
		<table id="chetview-edit-tab3_kalplanchf" class="table table-responsive table-bordered display compact" cellspacing="0" width="100%">
			<thead>
				<tr>
					<th></th>
					<th>ID счета</th>
...
// :::
?>
<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/chetview/chetview-edit/restr_5/tabs/css/chetview-edit-tab3_kalplanchf_oplata.css">
...
<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/chetview/chetview-edit/restr_5/tabs/css/chetview-edit-tab3_kalplanchf_avans.css">
		<div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
			<div class="space30"></div>
