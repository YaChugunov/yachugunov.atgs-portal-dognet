...
Editor::inst($db, 'dognet_docblankwork')
	->fields(
		...
		Field::inst('dognet_docblankwork.kodsubpodr')
			->options(
				Options::inst()
					->table('dognet_spsubpodr')
					->value('kodsubpodr')
					->label(array('kodsubpodr', 'namesubpodrshot'))
					->render(function ($row) {
						return $row['namesubpodrshot'];
					})
			),
		// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	...
	->on('preGet', function ($editor, $id) {
		$editor->where(function ($q) {
			$q->and_where(function ($q1) {
				$q1->where('dognet_docblankwork.kodstatusblank', "RD");
				$q1->where('dognet_docblankwork.kodblankdone', "1");
				$q1->where('dognet_docblankwork.numberdoccr', "");
				$q1->where('dognet_docblankwork.yearblankwork', date('Y'));
			});
			$q->or_where(function ($q2) {
				$q2->where('dognet_docblankwork.kodstatusblank', "CR");
				$q2->where('dognet_docblankwork.kodblankdone', "0");
				$q2->where('dognet_docblankwork.numberdoccr', "");
				$q2->where('dognet_docblankwork.yearblankwork', date('Y'));
			});
			$q->order('dognet_docblankwork.dateblankorder desc', 'dognet_docblankwork.numberblankwork desc');
		});
	})
	# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	...
	->leftJoin('dognet_spsubpodr', 'dognet_spsubpodr.kodsubpodr', '=', 'dognet_docblankwork.kodsubpodr')
