...
?>
	<script type="text/javascript" language="javascript" class="init">
		...
		$(document).ready(function() {
			...
			table_gipwork_newoplchf = $('#gipwork-main-newoplchf-table').DataTable({
				...
				ajax: {
					url: "php/examples/simple/main/main/restr_4/tabs/process/gipwork-main-newoplchf-process.php",
					type: "POST"
				},
				serverSide: true,
				columns: [{
				...
				columnDefs: [{
						...
						render: function(data, type, row, meta) {
							if (row.dognet_docbase.kodshab == '0') {
								return '<a href="/dognet/dognet-chetview.php?chetview_type=details&uniqueID=' +
									row.dognet_docbase.koddoc +
									'" class="docview-details-link" title="Перейти к карточке счета">' +
									data + '</a>';
							} else if (row.tmp1.kodshab == '1' || row.tmp1.kodshab == '3') {
								return '<a href="/dognet/dognet-docview.php?docview_type=details&uniqueID=' +
									...
									row.tmp1.docnumber + '</a>';
							} else if (row.tmp2.kodshab == '2' || row.tmp2.kodshab == '4') {
								return '<a href="/dognet/dognet-docview.php?docview_type=details&uniqueID=' +
									row.tmp2.koddoc +
									'" class="docview-details-link" title="Перейти к карточке договора">' +
									row.tmp2.docnumber + '</a>';
							} else {
								return '-';
							}
						},
	...
	// :::
	?>
	<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/main/main/restr_4/tabs/css/gipwork-main-newoplchf.css">
