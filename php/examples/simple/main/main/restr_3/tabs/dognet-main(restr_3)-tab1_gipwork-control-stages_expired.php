...
if (checkDognetMainpageViewBlock(4, 'control_stages')>0) {
?>
<script type="text/javascript" language="javascript" class="init">
var table_gipwork_control_stages_expired;
// ----- ----- -----
$(document).ready(function() {
		table_gipwork_control_stages_expired = $('#gipwork-control-stages_expired-table').DataTable( {
			dom: "<'row'<'col-sm-5'><'col-sm-4'><'col-sm-3'>>" + "<'row'<'col-sm-12'tr>>" + "<'row'<'col-sm-4'i><'col-sm-8'p>>",
			language: {
				url: "php/examples/simple/main/main/dt_russian-control-stages_expired.json"
			},
			ajax: {
				url: "php/examples/simple/main/main/restr_3/tabs/process/gipwork-control-stages_expired-process.php",
				type: "POST"
			},
			serverSide: true,
...
// 					$(row).css('background-color','rgb(255, 240, 240)');
		...
		var detailRows_stages_expired = [];
		$('#gipwork-control-stages_expired-table tbody').on( 'click', 'tr td.details-control', function () {

		  var tr = $(this).closest('tr');
		  var row = table_gipwork_control_stages_expired.row( tr );
		  var idx = $.inArray( tr.attr('id'), detailRows_stages_expired );
		...
		if ( row.child.isShown() ) {
			...
			detailRows_stages_expired.splice( idx, 1 );
		}
		else {
			tr.addClass( 'details' );
			rowData = table_gipwork_control_stages_expired.row( row );
			d = row.data();
			...
			rowData.child( <?php include('templates/gipwork-control-stages_expired.tpl'); ?> ).show();
		// Add to the 'open' array
		  if ( idx === -1 ) {
		      detailRows_stages_expired.push( tr.attr('id') );
		  }
		}
		} );
		// On each draw, loop over the `detailRows_stages_expired` array and show any child rows
		table_gipwork_control_stages_expired.on( 'draw', function () {
			$.each( detailRows_stages_expired, function ( i, id ) {
...
// :::
?>
<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/main/main/restr_3/tabs/css/gipwork-control-stages_expired.css">
<section>
	<div id="gipwork-control-stages_expired" class="" style="">
		<h3 class="docview-details-title2 space10">Просроченные этапы</h3>
		<div class="demo-html"></div>
		<table id="gipwork-control-stages_expired-table" class="table table-bordered table-responsive display compact" cellspacing="0" width="100%">
			<thead id="gipwork-control-stages_expired-table-header">
				<tr>
...
?>
<section>
	<div id="gipwork-main-stages_expired" class="">
