...
if (checkDognetMainpageViewBlock(3, 'control_chf')>0) {
?>
...
$(document).ready(function() {
	table_gipwork_control_stages_oplataexpired = $('#gipwork-control-stages_oplataexpired-table').DataTable( {
		...
		ajax: {
			url: "php/examples/simple/main/main/restr_3/tabs/process/gipwork-control-stages_oplataexpired-process.php",
			type: "POST"
		},
		serverSide: true,
		...
		columnDefs: [
			...
			{ orderable: false, searchable: false, render: function ( data, type, row, meta ) {
				numberdayoplstage = row.dognet_dockalplan.numberdayoplstage;
				dateoplall = row.dognet_dockalplan.dateoplall;
				dateplan = row.dognet_dockalplan.dateplan;
				chetfdate = row.dognet_kalplanchf.chetfdate;
				srokopl = row.dognet_dockalplan.srokopl;
				idsrokopl = row.dognet_dockalplan.idsrokopl;
				if (srokopl!=""&&srokopl!=null) {
					if (idsrokopl!==0&&idsrokopl!==2) {
						if (chetfdate!=""&&chetfdate!=null) {
							date1 = moment(chetfdate, 'YYYY-MM-DD');
							out = date1.add(srokopl, 'days').format('DD.MM.YYYY')+" ("+srokopl+")";
						}
						else {
							out = "--- ("+srokopl+")";
						}
					...
					date2 = moment(dateplan, 'YYYY-MM-DD');
					tmp = date2.add(numberdayoplstage, 'days').format('YYYY-MM-DD');
					out = "!? ("+moment(tmp, 'YYYY-MM-DD').format('DD.MM.YYYY')+")";
				}
				return out;
				}, targets: 8
			}
