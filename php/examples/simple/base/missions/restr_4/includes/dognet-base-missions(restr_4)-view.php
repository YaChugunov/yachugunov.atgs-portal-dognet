...
<link rel="stylesheet" type="text/css"
      href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/docview/css/docview-table-current-tab2-custom.css">
<!-- Custom Form Editor style -->
<link rel="stylesheet" type="text/css"
      href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/docview/css/docview-customForm-current-tab2-Editor.css">
...
$(document).ready(function() {
    ...
    table_tab7_report_missions = $('#report-missions').DataTable({
        dom: "<'row'<'col-sm-5'><'col-sm-3'<'#toolbar_msg'>><'col-sm-4'f>>" + "<'row'<'col-sm-12'tr>>" +
        ...
        ajax: {
            url: "php/examples/simple/base/missions/restr_4/includes/process/dognet-base-missions(restr_4)-view-process.php",
            type: "POST"
        },
        serverSide: true,
        columns: [{
        ...
        columnDefs: [{
            ...
            },
            {
                ...
                render: function(data, type, row, meta) {
                    if (data) {
                        return row.sp_contragents_opf.abbr + ' "' + data + '"';
                    } else {
                        return "-";
                    }
                },
        ...
        drawCallback: function(settings) {
            ...
            api.column(1, {
                ...
                if (last !== group) {
                    $(rows).eq(i).before(
                        '<tr class="group"><td style="text-align:left; white-space:nowrap">3-4/' +
                        group + '</td><td colspan="2" style="text-align:left">' + api
                        .row(i).data().dognet_docbase.yearnachdoc +
                        '</td><td colspan="3" class="docname-search" style="text-align:left">' +
                        api.row(i).data().dognet_docbase.docnameshot + '</td></tr>'
                    );
                    last = group;
                }
            });
...
<?php
    ...
    // :::
    include $_SERVER['DOCUMENT_ROOT'] . "/dognet/php/examples/simple/base/missions/restr_4/includes/forms/base-missions-filters.php";
    // ----- ----- ----- ----- -----
?>
<link rel="stylesheet"
      href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/base/missions/restr_4/includes/css/base-missions-main.css">
<section>
    <p class="text-danger space10">Имейте в виду, что в реестре договоров содержится список только тех договоров
        (определяемый Отделом договоров), по которым подразумеваются командировки. Полный список смотрите в разделе <a
           href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/dognet-docview.php?docview_type=current">Текущие
            договора</a></p>
    <div class="demo-html"></div>
    <table id="report-missions" class="table table-responsive table-bordered display compact" cellspacing="0"
           width="100%">
        <thead>
            <tr>
