...
<link rel="stylesheet"
      href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/docview/docview-current/restr_4/css/docview-current-common-tables.css">
<link rel="stylesheet"
      href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/docview/docview-current/restr_4/css/docview-current-common-customForm.css">
...
function ajaxRequest_sendMail_docStatus(docnumber, namedoc, ispolname, docstatus, summadoc, koddoc, kodstatus, kodispol,
    ...
    request = $.ajax({
        url: "php/examples/simple/docview/docview-current/restr_4/php/sendMail_onSubmit-changeDocstatus.php",
        type: "post",
        cache: false,
        data: {
            docnumber: docnumber,
...
function ajaxRequest_check_docNumber(docnumber, responseHandler) {
    ...
    // Fire off the request to /form.php
    request = $.ajax({
        url: "php/examples/simple/docview/docview-current/restr_4/php/dognet_reqAjax_onSubmit_checkDocNumber.php",
        type: "post",
        cache: false,
        data: {
            docnumber: docnumber
...
$(document).ready(function() {
    ...
    editor_doc_main = new $.fn.dataTable.Editor({
        display: "bootstrap",
        ajax: "php/examples/simple/docview/docview-current/restr_4/process/dognet-docview-doc(restr_4)-main-process.php",
    ...
    editor_doc_main
        .on('open', function() {
            ...
            $('#kodagent_filter').val('');
            if (($('#DTE_Field_dognet_docbase-kodagent').value) != editor_doc_main.field(
                    'dognet_docbase.kodagent').get()) {}
            // Поиск в ниспадающем списке по содержимому текстового поля для ответного документа
            $('#DTE_Field_dognet_docbase-kodagent').filterByText(editor_doc_main, $('#kodagent_filter'),
                'dognet_docbase.kodagent', false);
            ...
            editor_doc_main.on('preSubmit', function(e, data, action) {
                ...
                if (action == 'create') {
                    ...
                    console.log("-> " + new Date().toLocaleString() +
                        " : reqResponseHandler2.checkDocnumber(): " + reqResponseHandler2
                        .checkDocnumber());
        ...
        if (action == 'create') {
            ...
            editor_doc_main.dependent('dognet_docbase.usedoczayv', function(val) {
                if (val == 1) {
                    ...
                    editor_doc_main.dependent('dognet_docbase.kodblankwork', function(val, data,
                        callback) {
                        if ($("#DTE_Field_dognet_docbase-kodblankwork option:selected")
                            .val() != "") {

                            // Serialize the data in the form
                            // var serializedData = $form.serialize();
                            var kodblankwork = $(
                                "#DTE_Field_dognet_docbase-kodblankwork").val();
                            // Fire off the request to /form.php
                            request2 = $.ajax({
                                url: "php/examples/simple/docview/docview-current/restr_4/process/dognet-docview-doc(restr_4)-main_fetch-process.php",
                                type: "post",
                                cache: false,
                                data: {
                            ...
                            request2.done(function(response, textStatus, jqXHR) {
                                ...
                                console.log("Request 2 response : " + response
                                    .replace(/\r?\n/g, ""));
                                ...
                                if (resp != '') {
                                    ...
                                    console.log("ZAK " + zak + " / ISP " + isp +
                                        " / RUK " + ruk + " / OBJ " + obj);
                                    editor_doc_main.field(
                                        'dognet_docbase.kodzakaz').set(zak);
                                    editor_doc_main.field(
                                        'dognet_docbase.kodispol').set(isp);
                                    editor_doc_main.field(
                                        'dognet_docbase.kodispolruk').set(
                                        ruk);
                                    editor_doc_main.field(
                                        'dognet_docbase.docnameshot').set(
                                        doc);
                                    editor_doc_main.field(
                                        'dognet_docbase.docnamefullm').set(
                                        doc);
                                    editor_doc_main.field(
                                        'dognet_docbase.kodobject').set(obj);
                                }
    ...
    table_doc_main = $('#docview-doc-main').DataTable({
        ...
        ajax: {
            url: "php/examples/simple/docview/docview-current/restr_4/process/dognet-docview-doc(restr_4)-main-process.php",
            type: "POST"
        },
        serverSide: true,
            ...
            },
            {
                ...
                render: function(data, type, row, meta) {
                    return '<span style="padding:0 5px"><a href="dognet-docview.php?docview_type=details&uniqueID=' +
                        ...
                        '<span style="padding:0 5px"><a href="dognet-docview.php?docview_type=edit&uniqueID=' +
                        row.dognet_docbase.koddoc +
    ...
    editor_doc_main.on('submitSuccess', function(e, json, data, action) {
        if (action === "create") {
            ...
            _docpartnernumber = newObj.data[0].dognet_docbase.docpartnernumberSTR;
    ...
    $('#docview-doc-main tbody').on('click', 'tr td.details-control', function() {
        ...
        if (row.child.isShown()) {
            ...
            kodagent = row.data().dognet_docbase.kodagent;
            kodtipdoc = row.data().dognet_docbase.kodtip;
            letterToChangeSrok = '';
            ...
            if (kodtipdoc == "245287841599652" || kodtipdoc == "245287841608965") {
                if (userid == "999") {
                    letterToChangeSrok =
                        '<br><span style="float:left; font-size:0.8em; font-weight:300" class="text-danger"><a href="dognet-docview.php?docview_type=current&export=yes&tip=letter_transfer&uniqueID1=' +
                        koddoc + '&zak=' + kodzakaz +
                        '" class="">Письмо о переносе срока</a></span>';
                } else {
                    // letterToChangeSrok =
                    //     '<br><span style="float:left; font-size:0.8em; font-weight:300" class="text-danger"><a href="#noanchor" class="nonactive">Письмо о переносе срока</a></span>';
                    letterToChangeSrok =
                        '<br><span style="float:left; font-size:0.8em; font-weight:300" class="text-danger"><a href="dognet-docview.php?docview_type=current&export=yes&tip=letter_transfer&uniqueID1=' +
                        koddoc + '&zak=' + kodzakaz +
                        '" class="">Письмо о переносе срока</a></span>';
...
<link rel="stylesheet"
      href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/docview/docview-current/restr_4/css/docview-current-main.css">
<style>
...
<section>
    ...
    <table id="docview-doc-main" class="table table-responsive table-bordered display compact" cellspacing="0"
           width="100%">
        <thead>
            <tr>
    ...
    $('#columnSearch_btnApply').click(function() {
        ...
        if ($("#checkbox_saveFiltersPanelState").prop("checked")) {
            ...
            $('#docview-current-filters').find('input, select').each(function() {
                ...
                $.post("/dognet/php/examples/simple/docview/docview-current/restr_4/php/dognet_reqAjax_saveFiltersToSession.php", {
