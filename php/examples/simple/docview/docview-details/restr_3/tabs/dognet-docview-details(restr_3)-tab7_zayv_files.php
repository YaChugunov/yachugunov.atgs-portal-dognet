...
// Обработчик таблицы счетов
	table_zayv_files = $('#docview-zayv-files').DataTable( {
		dom: "<'row'<'col-sm-5'><'col-sm-4'><'col-sm-3'<'test_msg'>>>" + "<'row'<'col-sm-12'tr>>" + "<'row'<'col-sm-1'B><'col-sm-5'i><'col-sm-6'p>>",
		language: {
			url: "php/examples/simple/docview/docview-details/dt_russian-tab7_zayv_files.json"
		},
		ajax: {
			url: "php/examples/php/docview/docview-details/dognet-docview-details-tab7_zayv_files-process.php",
			type: "POST",
      data: function ( d ) {
...
// Array to track the ids of the details displayed rows
  ...
  $('#docview-zayv-files tbody').on( 'click', 'tr td.details-control', function () {
      ...
      if ( row.child.isShown() ) {
		...
		d = row.data();
		rowData.child( <?php include('templates/docview-details_tab7_zayv_files.tpl'); ?> ).show();
...
//
?>
<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/docview/docview-details/restr_3/tabs/css/docview-details-common-tab7_zayv_files.css">
<h3 class="docview-details-title2 text-right">Спецификации к заявке (в виде прикреплённых файлов)</h3>
<div class="demo-html"></div>
<table id="docview-zayv-files" class="table table-responsive table-bordered display compact" cellspacing="0" width="100%">
	<thead>
		<tr>
