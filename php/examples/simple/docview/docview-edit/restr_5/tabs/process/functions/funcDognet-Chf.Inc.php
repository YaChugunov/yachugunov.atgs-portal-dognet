...
function UPDATE_CHFZADOL($db, $action, $id, $kodCHF_selected, $kodCHF_field) {
  ...
  $_QRY_CHF = $db->sql("SELECT sumchfsubpodr, kodchfsubpodr FROM dognet_docchfsubpodr WHERE id='{$id}'")->fetchAll();
  $kodchfsubpodrDB = !empty($_QRY_CHF[0]['kodchfsubpodr']) ? $_QRY_CHF[0]['kodchfsubpodr'] : "";
  $sumchfsubpodrDB = !empty($_QRY_CHF[0]['sumchfsubpodr']) ? $_QRY_CHF[0]['sumchfsubpodr'] : 0;
  $sumzadolchfsubpodrDB = !empty($_QRY_CHF[0]['sumzadolchfsubpodr']) ? $_QRY_CHF[0]['sumzadolchfsubpodr'] : 0;
  ...
  if (!empty($_POST['kodchfsubpodr_field']) && !empty($_POST['kodchfsubpodr_selected'])) {
    ...
    if ($action == 'UPD' or $action == 'CRT') {
      ...
      $_NEW_sumzadolchfsubpodr = $__sumchfsubpodr - (CALC_SUMOPLCHF($kodCHF_field) + $__sumavans +
        $__sumavsplit);
      $_var1 = "-1";
    }
    if ($action == 'PREDEL') {
  ...
  if (empty($_POST['kodchfsubpodr_field']) && !empty($_POST['kodchfsubpodr_selected'])) {
    ...
    if ($action == 'UPD' or $action == 'CRT') {
      ...
      $_NEW_sumzadolchfsubpodr = $__sumchfsubpodr - (CALC_SUMOPLCHF($kodCHF_selected) + $__sumavans +
        $__sumavsplit);
      $_var1 = "-3";
    }
    if ($action == 'PREDEL') {
