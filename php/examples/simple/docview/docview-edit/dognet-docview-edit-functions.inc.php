...
#
function StageOplAvans($varKodkalplan) {
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
# Функция расчета незакрытия этапа с кодом kodkalplan
#
function StageNezakr($varKodkalplan) {
// Из таблицы dognet_dockalplan определяем код(ы) этапа(ов) - kodkalplan
	...
	$query_getSummaChf = mysqlQuery( "SELECT SUM(chetfsumma) as summachf FROM dognet_kalplanchf WHERE kodkalplan = ".$varKodkalplan." AND koddel <> '99'" );
	$row_getSummaChf = mysqli_fetch_array($query_getSummaChf, MYSQLI_ASSOC);
	$varSummaChf = $row_getSummaChf['summachf'];
	return $varSummastage - $varSummaChf;
}
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
...
// Из таблицы dognet_dockalplan определяем код(ы) этапа(ов) - kodkalplan
	...
	$row_getSummastage = mysqli_fetch_array($query_getSummastage, MYSQLI_ASSOC);
	$varSummastage = $row_getSummastage['summastage'];
	return $varSummastage - StageOplata($varKodkalplan);
}
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
