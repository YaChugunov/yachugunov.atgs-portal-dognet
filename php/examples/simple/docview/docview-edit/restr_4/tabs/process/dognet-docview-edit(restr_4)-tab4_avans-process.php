...
function updateFields_docavans($db, $action_docavans, $id, $values, $__kodshab, $__uniqueID)
{
    ...
    if ($action_docavans == 'CRT') {
        ...
        $sum1           = ($_QRY && $_QRY[0]['sum1'] != "") ? $_QRY[0]['sum1'] : 0;
        $sumoplavstage  = ($_QRY_C && $_QRY_C[0]['sumoplavstage'] != "") ? $_QRY_C[0]['sumoplavstage'] : 0;
        $db->update('dognet_dockalplan_progress', [
            'firstdateavans' => $firstdateavans,
            'srokstage_date' => $expiry_date,
            'sumavstage'     => $sum1,
            'zadolsum_av'    => $sum1 - $sumoplavstage,
        ], ['kodkalplan' => $_QRY1[0]['koddoc']]);
        //
        $db->update('dognet_dockalplan', [
            'srokstage_date' => $expiry_date,
        ], ['kodkalplan' => $_QRY1[0]['koddoc']]);
            ...
            Field::inst('dognet_docavans.koddoc')
                ->options(
                    Options::inst()
                        ...
                        ->value('kodkalplan')
                        ->label(['koddoc', 'kodkalplan', 'numberstage', 'nameshotstage'])
