...
# ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### 
#
function CALC_SUMOPLCHF($kodchfsubpodr) {
...
function CALC_SUMAVOST($db, $action, $id, $useavans, $kodavsubpodr) {
  ...
  if (!empty($kodavsubpodr)) {
    if (empty($useavans)) {
      ...
      if ($action != "DEL") {
        $_QRY_ID_SPLIT = $db->sql("SELECT sumavsplit, kodchfsubpodr FROM dognet_docavsplitsubpodr WHERE id='{$id}'")->fetchAll();
        $sumsplit = (!empty($_QRY_ID_SPLIT[0]['sumavsplit']) && !empty($_QRY_ID_SPLIT[0]['kodchfsubpodr'])) ? $_QRY_ID_SPLIT[0]['sumavsplit'] : 0;
      }
...
# ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### 
#
function UPDATE_CHFZADOL($db, $action, $id, $kodCHF_selected, $kodCHF_field, $sumavsplit_selected, $sumavsplit_field) {

  $_QRY_CHF = $db->sql("SELECT sumavsplit, kodchfsubpodr FROM dognet_docavsplitsubpodr WHERE id='{$id}'")->fetchAll();
  $kodchfsubpodrDB = !empty($_QRY_CHF[0]['kodchfsubpodr']) ? $_QRY_CHF[0]['kodchfsubpodr'] : "";
  $sumavsplitDB = !empty($_QRY_CHF[0]['sumavsplit']) ? $_QRY_CHF[0]['sumavsplit'] : 0;
  ...
  if (!empty($_POST['kodchfsubpodr_field']) && !empty($_POST['kodchfsubpodr_selected'])) {
    ...
    if ($action == 'UPD' or $action == 'CRT') {
      ...
      $_NEW_sumzadolchfsubpodr = $__sumchfsubpodr - (CALC_SUMOPLCHF($kodCHF_field) + $__sumavans + $__sumavsplit);
    }
    if ($action == 'PREDEL') {
      // $_NEW_sumzadolchfsubpodr = '-10';
      $_NEW_sumzadolchfsubpodr = $__sumchfsubpodr - (CALC_SUMOPLCHF($kodCHF_field) + $__sumavans + $__sumavsplit) + $sumavsplitDB;
    }
    // Пишем новую задолженность по СФ
    $db->update('dognet_docchfsubpodr', array(
      'sumzadolchfsubpodr'    => $_NEW_sumzadolchfsubpodr,
      'comment'               => 'SPLIT ' . $_var1 . ' / ' . $_var2,
  ...
  if (empty($_POST['kodchfsubpodr_field']) && !empty($_POST['kodchfsubpodr_selected'])) {
    ...
    if ($action == 'UPD' or $action == 'CRT') {
      ...
      $_NEW_sumzadolchfsubpodr = $__sumchfsubpodr - (CALC_SUMOPLCHF($kodCHF_selected) + $__sumavans + $__sumavsplit);
    }
    if ($action == 'PREDEL') {
      // $_NEW_sumzadolchfsubpodr = '-30';
      $_NEW_sumzadolchfsubpodr = $__sumchfsubpodr - (CALC_SUMOPLCHF($kodCHF_selected) + $__sumavans + $__sumavsplit) + $sumavsplitDB;
    }
    // Пишем новую задолженность по СФ
    $db->update('dognet_docchfsubpodr', array(
      'sumzadolchfsubpodr'    => $_NEW_sumzadolchfsubpodr,
...
function UPDATE_CHFZADOL_OTH($db, $action, $id, $koddocsubpodr, $kodCHF_selected, $kodCHF_field) {
  ...
  while ($_ROW = mysqli_fetch_assoc($_QRY)) {
    ...
    // -----
    $_QRY_2 = $db->sql("SELECT SUM(sumchfavsubpodr) as sumavans FROM dognet_docavsubpodr WHERE kodchfsubpodr=" . $kod)->fetchAll();
    if ($_QRY_2) {
      $__sumavans = $_QRY_2[0]['sumavans'];
    }
    // -----
    $_QRY_3 = $db->sql("SELECT SUM(sumavsplit) as sumavsplit FROM dognet_docavsplitsubpodr WHERE kodchfsubpodr=" . $kod)->fetchAll();
    if ($_QRY_3) {
      $__sumavsplit = $_QRY_3[0]['sumavsplit'];
    }
    // -----
    $_NEW_sumzadolchfsubpodr = $__sumchfsubpodr - (CALC_SUMOPLCHF($kod) + $__sumavans + $__sumavsplit);
    // -----
    if ($kod != $kodCHF_field) {
      $db->update('dognet_docchfsubpodr', array(
        'sumzadolchfsubpodr'    => $_NEW_sumzadolchfsubpodr
