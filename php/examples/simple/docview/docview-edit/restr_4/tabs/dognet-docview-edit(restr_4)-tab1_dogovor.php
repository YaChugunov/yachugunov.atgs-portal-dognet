...
function ajaxRequest_sendMail_docStatus(docnumber, namedoc, ispolname, docstatus, summadoc, koddoc, kodstatus, kodispol,
    ...
    request = $.ajax({
        url: "php/examples/simple/docview/docview-edit/restr_4/tabs/php/sendMail_onSubmit-changeDocstatus.php",
        type: "post",
        cache: false,
        data: {
            docnumber: docnumber,
...
$(document).ready(function() {
    ...
    editor_tab1_dogovor
        .on('open', function() {
            ...
            $('#kodagent_filter').val('');
            if (($('#DTE_Field_dognet_docbase-kodagent').value) != editor_tab1_dogovor.field(
                    'dognet_docbase.kodagent').get()) {}
            // Поиск в ниспадающем списке по содержимому текстового поля для ответного документа
            $('#DTE_Field_dognet_docbase-kodagent').filterByText(editor_tab1_dogovor, $('#kodagent_filter'),
...
<section>
    <div id="docview-tab1_dogovor" class="" style="padding:0 5px">
        ...
        <table id="docview-edit-tab1_dogovor" class="table table-responsive table-bordered display compact"
