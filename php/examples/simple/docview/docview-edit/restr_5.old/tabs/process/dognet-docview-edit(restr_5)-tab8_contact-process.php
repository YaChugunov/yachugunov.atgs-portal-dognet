...
#
#
function updateFields($db, $action, $id, $values, $koddoc) {
	#
	#
...
Editor::inst($db, 'dognet_spcontact')
	...
	->on('preGet', function ($editor, $id) use ($__uniqueID) {
		$editor->where(function ($q) use ($__uniqueID) {
			$q->where('dognet_spcontact.koddoc', $__uniqueID);
		});
	})
	//
	->on('postCreate', function ($editor, $id, $values, $row) use ($__uniqueID) {
		updateFields($editor->db(), 'CRT', $id, $values, $__uniqueID);
	})
	->on('postEdit', function ($editor, $id, $values, $row) use ($__uniqueID) {
		// updateFields( $editor->db(), 'UPD', $id, $values, $__uniqueID );
	})
