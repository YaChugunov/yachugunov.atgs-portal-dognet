...
//
var reqField1 = { kodtip: function (response1) { $('#ajaxResponseOut1').html(response1.replace(new RegExp("\\r?\\n", "g"), "")); } };
var reqField2 = { sumdiff: function (response2) { $('#ajaxResponseOut2').html(response2.replace(new RegExp("\\r?\\n", "g"), "")); } };
var reqField3 = { firstavans: function (response) { } };
//
function ajaxRequest_kodtip(data, responseHandler) {
    ...
    request1 = $.ajax({
        url: "php/examples/simple/docview/docview-edit/restr_5/tabs/process/dognet-docview-edit(restr_5)-tab2_kalplans_fetch-process.php",
        type: "post",
        cache: false,
        data: { koddoc: data },
...
function ajaxRequest_sumdiff(data1, data2, data3, data4, responseHandler) {
    ...
    request2 = $.ajax({
        ...
        type: "post",
        cache: false,
        data: { action: data1, koddoc: data2, kodkalplan: data3, sum: data4 },
    ...
    request2.done(function (response2, textStatus, jqXHR){
		...
		var stage_summa = x[1];
		var diff_summa = x[2];
		var diff = (parseFloat(diff_summa) < 0) ? '<span class="" style="color:#fff; padding:2px 5px; background-color:#951302">'+diff_summa+'</span>' : '<span class="" style="color:#fff; padding:2px 5px; background-color:#46a512">'+diff_summa+'</span>';
...
function ajaxRequest_firstavans(data, responseHandler) {
    ...
    // Fire off the request to /form.php
    request = $.ajax({
        url: "php/examples/simple/docview/docview-edit/restr_5/tabs/php/dognet-reqAjax-getFromDB-firstavans.php",
        type: "post",
        cache: false,
        data: { koddoc: data },
    ...
    request.done(function (response, textStatus, jqXHR){
		...
		var av1summa = x[1];
		if (av1date!="" && av1summa!="") {
	        $('#ajaxFirstAvansOut').html("1-ый аванс поступил "+moment(av1date, 'YYYY-MM-DD').format('DD.MM.YYYY')+" на сумму "+av1summa);
...
// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	editor_tab2_kalplans = new $.fn.dataTable.Editor( {
	    ...
	    ajax: "php/examples/simple/docview/docview-edit/restr_5/tabs/process/dognet-docview-edit(restr_5)-tab2_kalplans-process.php",
		table: "#docview-edit-tab2_kalplans",
    i18n: {
        create: { title: "<h3>Создать новый этап</h3>" },
        edit: { title: "<h3>Редактировать этап</h3>" },
        remove: {
...
// ----- ----- ----- ----- -----
			    ...
			    options: [
			        { label: "ПКЗ", value: 0 },
			        { label: "По сроку (в кал. днях)", value: 1 },
			        { label: "По дате", value: 2 }, 
			        { label: "По сроку (в раб. днях)", value: 3 }
			    ]
			}, {
			    label: "Новый срок",
...
// ----- -- ----- -- -----
	editor_tab2_kalplans
		.on( 'open', function () {
		    ...
		    const tips = ['245773055697212', '245773052282168', '245287843809309', '245287841599652', '245267756588371'];
	        ...
	        editor_tab2_kalplans.on( 'preSubmit', function ( e ) {
				if ( editor_tab2_kalplans.field( 'dognet_dockalplan.numberstage' ).val()=='' || editor_tab2_kalplans.field( 'dognet_dockalplan.kodobject' ).val()=='' || editor_tab2_kalplans.field( 'dognet_dockalplan.nameshotstage' ).val()=='' ) {
				...
				if ( (editor_tab2_kalplans.field( 'dognet_dockalplan.srokstage' ).val()=='' && editor_tab2_kalplans.field( 'dognet_dockalplan.srokstage_date' ).val()=='') || editor_tab2_kalplans.field( 'dognet_dockalplan.srokopl' ).val()=='' ) {
				...
				if ( editor_tab2_kalplans.field( 'dognet_dockalplan.dateplan' ).val()=='' || editor_tab2_kalplans.field( 'dognet_dockalplan.dateoplall' ).val()=='' ) {
					document.getElementById("doc-editor-menu-tab-4-errmsg").innerHTML = '<span class="glyphicon glyphicon-exclamation-sign"></span>';
    ...
    editor_tab2_kalplans.dependent( 'dognet_dockalplan.idsrokopl', function ( val ) {
			...
			if ( val == 2 ) {
				...
				$('#DTE_Field_dognet_dockalplan-srokopl').change(function() {
					...
					if ($(this).val().substring(3, 2) > 12 || $(this).val().substring(3, 2) == "00") {
...
// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	table_tab2_kalplans = $('#docview-edit-tab2_kalplans').DataTable( {
		dom: "<'row'<'col-sm-5'B><'col-sm-4'><'col-sm-3'>>" + "<'row'<'col-sm-12'tr>>" + "<'row'<'col-sm-6'i><'col-sm-6'p>>",
...
// 		dom: "t",
		...
		ajax: {
			url: "php/examples/simple/docview/docview-edit/restr_5/tabs/process/dognet-docview-edit(restr_5)-tab2_kalplans-process.php",
			type: "POST"
		},
		serverSide: true,
		columns: [
...
// Выводим уведомление (цифру) о количестве этапов
	table_tab2_kalplans.on( 'draw', function () {
		...
		selected = table_tab2_kalplans.row( { selected: true } );
...
// ----- -- ----- -- -----
	editor_tab2_kalplans.on( 'open', function (e, mode, action) {
		...
		// ----- -- ----- -- -----
			...
			selected = table_tab2_kalplans.row( { selected: true } );
			if (selected.any()) { kodkalplan = selected.data().dognet_dockalplan.kodkalplan; }
			else { kodkalplan = null; }
...
<section>
	<div id="docview-tab2_kalplans" class="" style="padding:0 5px">
		...
		<h3 class="parent-title space20">Этапы основного договора</h3>
