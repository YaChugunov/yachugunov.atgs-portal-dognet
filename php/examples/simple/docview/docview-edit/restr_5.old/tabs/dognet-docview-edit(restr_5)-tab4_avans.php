...
//
//
var reqField_sendMail = { sendMail: function (response) {  } };
function ajaxRequest_sendMail(koddoc, docnumber, namedoc, numberstage, namestage, dateavans, summaavans, responseHandler) {
	var response = false;
    // Fire off the request to /form.php
    request = $.ajax({
        url: "php/examples/simple/docview/docview-edit/restr_5/tabs/php/sendMail_onSubmit-newAvans.php",
        type: "post",
        cache: false,
        data: {
	        koddoc: koddoc,
...
// АВАНСЫ : форма редактирования
	editor_tab4_avans = new $.fn.dataTable.Editor( {
		display: "bootstrap",
		ajax: "php/examples/simple/docview/docview-edit/restr_5/tabs/process/dognet-docview-edit(restr_5)-tab4_avans-process.php",
...
// 		dom: "t",
		...
		ajax: {
			url: "php/examples/simple/docview/docview-edit/restr_5/tabs/process/dognet-docview-edit(restr_5)-tab4_avans-process.php",
			type: "POST"
		},
		serverSide: true,
		columns: [
...
// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	editor_tab4_avans.on( 'submitSuccess', function (e, json, data, action) {
		if (action==="create") {
			...
			console.log( "newObj.data > "+newObj.data[0] );
			...
			_summaavans = $.fn.dataTable.render.number(' ', ',', 2, '').display( _summaavans )+newObj.data[0].dognet_spdened.short_code

			ajaxRequest_sendMail(_koddoc, _docnumber, _namedoc, _numberstage, _namestage, _dateavans, _summaavans, 'sendMail');

		}
		table_tab3_kalplanchf.row( { selected: true } ).deselect();
	} );
//
// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
//
...
// :::
include($_SERVER['DOCUMENT_ROOT']."/dognet/php/examples/simple/docview/docview-edit/restr_5/tabs/forms/docview-edit_tab4_avans-customForm.php");
?>
<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/docview/docview-edit/restr_5/tabs/css/docview-edit-tab4_avans.css">
<section>
	<div id="docview-tab4_avans" class="" style="padding:0 5px">
		<div class="space30"></div>
		<h3 class="parent-title space20">Авансы по основному договору</h3>
		<div class="demo-html"></div>
		<table id="docview-edit-tab4_avans" class="table table-responsive table-bordered display compact" cellspacing="0" width="100%">
			<thead>
				<tr>
