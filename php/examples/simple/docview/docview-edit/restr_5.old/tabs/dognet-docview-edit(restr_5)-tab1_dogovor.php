...
//
//
var reqField_sendMail_docStatus = { sendMail: function (response) {  } };
...
// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	...
	.on( 'open', function () {
		...
		$('#kodzakaz_filter').val('');
		if (($('#DTE_Field_dognet_docbase-kodzakaz').value) != editor_tab1_dogovor.field('dognet_docbase.kodzakaz').get()) {
		}
		// Поиск в ниспадающем списке по содержимому текстового поля для ответного документа
		$('#DTE_Field_dognet_docbase-kodzakaz').filterByText(editor_tab1_dogovor, $('#kodzakaz_filter'), 'dognet_docbase.kodzakaz', false);

		$('#kodobject_filter').val('');
		if (($('#DTE_Field_dognet_docbase-kodobject').value) != editor_tab1_dogovor.field('dognet_docbase.kodobject').get()) {
		}
		// Поиск в ниспадающем списке по содержимому текстового поля для ответного документа
		$('#DTE_Field_dognet_docbase-kodobject').filterByText(editor_tab1_dogovor, $('#kodobject_filter'), 'dognet_docbase.kodobject', false);
...
// Изменяем размер диалогового окна редактирования договора субподряда
	editor_tab1_dogovor.on( 'close', function () { $(".modal-dialog").css({
		"width":"60%",
		"min-width":"none",
		"max-width":"none"
		});
...
// 		dom: "t",
		...
		lengthChange: false,
		buttons: [
	    { text:	'<span class="glyphicon glyphicon-refresh"></span>', className: 'tab1-refreshButton', action:
	      function ( e, dt, node, config ) {
					table_tab1_dogovor.columns().search('').draw();
				}
			},
			{ extend: "edit", editor: editor_tab1_dogovor, text: "РЕДАКТИРОВАТЬ",
				formButtons:
					[ 'Сохранить изменения',
...
// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	editor_tab1_dogovor.on( 'preOpen', function () {
		...
		var yearnachdoc = addZero(2, editor_tab1_dogovor.field('dognet_docbase.yearnachdoc').val());
			editor_tab1_dogovor.field( 'docDateBegin' ).set(daynachdoc+"."+monthnachdoc+"."+yearnachdoc);
