      ...
      <div class="modal-body">
				...
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
				  ...
				  <div class="media-body media-middle bodytext" style="padding:15px 20px">
				    <span class="text-justify">Коллеги! Я планирую на время отладки работы сервиса ДОГОВОР использовать email-рассылки для оповещения обо всех обновлениях и изменениях в работе сервиса (другие способы оповещения что-то не приживаются), и не хочу напрягать рассылками на общий email сотрудников, которые данным сервисом не пользуются. Все просто, если вы хотите получать эти рассылки ответьте ДА, если не хотите - НЕТ, и это сообщение не будет появляться некоторое время, на всякий случай, вдруг Вы передумаете :)</span>
				  </div>
				</div>
...
$('#formx').submit(function(){
    $.post(
        '_assets/includes/subscribe_handler/dognetNewsletter-subscribe-ajax-post-handler.php', // адрес обработчика
	...
	function call() {
	  var msg = $('#formx').serialize();
		$.ajax({
			type: 'POST',
			url: '_assets/includes/subscribe_handler/dognetNewsletter-subscribe-ajax-post-handler.php',
      data: msg,
