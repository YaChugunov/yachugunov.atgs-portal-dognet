...
#
#
if (mysqli_num_rows($_QRY_RECIPIENT)>0&&$_ROW_RECIPIENT['email2']!="") {
	$email_to = $_ROW_RECIPIENT['email2'];
}
else {
	$email_to = $_ROW_ZAYVTEL['email'];
}
?>
		...
		<table id="mail-preview-info" class="table table-responsive">
			<tbody>
				<tr style="">
					...
					<td width="70%" class="text-left"><span><?php echo "Сообщение выбранному пользователю"; ?></span></td>
				</tr>
				<tr style="">
				...
				<tr style="">
					...
					<td width="70%" class="text-left"><span>
						<?php
							if (mysqli_num_rows($_QRY_RECIPIENT)>0) {
								echo $_ROW_RECIPIENT['lastname']." ".mb_substr($_ROW_RECIPIENT['firstname'],0,1,'utf-8').". ".mb_substr($_ROW_RECIPIENT['middlename'],0,1,'utf-8').". / ".$email_to." (из списка пользователей)";
							}
							else {
								echo $_ROW_ZAYVTEL['namezayvtelshot']." / ".$email_to." (из справочника)";
							}
							?>
					</span></td>
				</tr>
...
# ----- ----- ----- ----- -----
 	...
 	$mail3->addCC('chugunov@atgs.ru', 'copy to Y.Chugunov');
# ----- ----- ----- ----- -----
