	...
	$(document).ready(function() {
		...
		editor_chet_main = new $.fn.dataTable.Editor({
			...
			fields: [{
				...
				label: "Срок поставки (в неделях)",
				name: "dognet_doczayvchet.srokpostavki"
			}, {
				label: "",
		...
		table_chet_main = $('#zayvview-chet-main').DataTable({
			dom: "<'row'<'col-sm-6'B><'col-sm-3'><'col-sm-3'l>>" + "<'row'<'col-sm-12'tr>>" + "<'row'<'col-sm-4'i><'col-sm-8'p>>",
			// 		dom: "<'space50'r>tip",
			language: {
			...
			*/
			...
			columns: [{
				...
				{
					data: "dognet_doczayvchet.srokpostavki",
					defaultContent: ''
				}
			],
			select: {
			...
			columnDefs: [{
				...
				},
				{
					...
					render: function(data, type, row, meta) {
						comment = (row.dognet_doczayv.zayvchetcom != "") ? "<span style='float:right'><a href='#qwerty' data-toggle='tooltip' data-placement='top' title='" + row.dognet_doczayv.zayvchetcom + "' style='color:#999'><span class='glyphicon glyphicon-comment'></span></a></span>" : "";
						return "<abbr title=''><a href='#qwerty' data-toggle='tooltip' data-placement='top' title='" + row.dognet_doczayv.namerabfilespec + "'>" + row.dognet_sptipzayvall.nametipzayvshotall + "-" + data + "</a></abbr>" + comment;
					},
					targets: 3
				},
				{
				...
				},
				{
					...
					render: function(data, type, row, meta) {
						if (data != null) {
							if (row.dognet_spdened.html_code != null && row.dognet_spdened.html_code != "") {
								return '<span style="text-align:left">' + $.fn.dataTable.render.number(' ', ',', 2, '').display(data) + '</span><span style="float:right">' + row.dognet_spdened.html_code + '</span>';
							} else {
								return '<span style="text-align:left">' + $.fn.dataTable.render.number(' ', ',', 2, '').display(data) + '</span><span style="float:right">--.</span>';
							}
						} else {
							if (row.dognet_spdened.html_code != null && row.dognet_spdened.html_code != "") {
								return '<span style="text-align:left">0.00</span><span style="float:right">' + row.dognet_spdened.html_code + '</span>';
							} else {
				...
				},
				{
					...
					render: function(data, type, row, meta) {
						// Дедлайн
						var srok_postavki = row.dognet_doczayvchet.srokpostavki;
						var zadolzh_pr = row.dognet_doczayvchet.zayvchetpr;
						var date_chet = moment(row.dognet_doczayvchet.zayvchetdate, 'DD.MM.YYYY').format('YYYY-MM-DD');
						if (srok_postavki > 0 && srok_postavki != null && srok_postavki != "" && zadolzh_pr < 100) {
							var date_dl = moment(date_chet).add(srok_postavki, 'weeks');
							var date_now = moment();
							var diff = date_dl.diff(date_now, 'days')
							console.log(diff);
							if (diff < 7) {
								return "<a href='#qwerty' data-toggle='tooltip' data-placement='top' title='До расчетной даты поставки менее одной недели' style='color:#999'><span style='color:#ee4044'><span class='glyphicon glyphicon-fire'></span></span></a>";
							} else if (diff < 14 && diff >= 7) {
								return "<a href='#qwerty' data-toggle='tooltip' data-placement='top' title='До расчетной даты поставки менее двух недель' style='color:#999'><span style='color:#f0ad4e'><span class='glyphicon glyphicon-exclamation-sign'></span></span>";
							} else {
								return "<span>" + row.dognet_doczayvchet.srokpostavki + "</span>";
							}
						} else if (srok_postavki > 0 && srok_postavki != null && srok_postavki != "" && zadolzh_pr == 100) {
							return "<span>" + row.dognet_doczayvchet.srokpostavki + "</span>";
						} else {
							return "<span>--</span>";
						}
			...
			buttons: [{
					...
					action: function(e, dt, node, config) {
						...
						table_chet_main.order([1, "desc"], [12, "asc"]).draw();
					}
				},
				{
					extend: "create",
		...
		$('#zayvview-chet-main tbody').on('click', 'tr td.details-control', function() {
			...
			if (row.child.isShown()) {
				...
				var srok_postavki = "---";
				var deadline_icon = "";
				var date_chet = moment(d.dognet_doczayvchet.zayvchetdate, 'DD.MM.YYYY').format('YYYY-MM-DD');
				var zadolzh_pr = d.dognet_doczayvchet.zayvchetpr;
				console.log(date_chet, d.dognet_doczayvchet.srokpostavki, date_dl);
				if (d.dognet_doczayvchet.srokpostavki > 0) {
					var date_dl = moment(date_chet).add(d.dognet_doczayvchet.srokpostavki, 'weeks').format('DD.MM.YYYY');
					var srok_postavki = d.dognet_doczayvchet.srokpostavki + " нед.";
					console.log(srok_postavki, date_dl);
					//
					var date_dl1 = moment(date_chet).add(d.dognet_doczayvchet.srokpostavki, 'weeks');
					var date_now = moment();
					var diff = date_dl1.diff(date_now, 'days');
					if (diff < 7 && zadolzh_pr < 100) {
						deadline_icon = "<span style='color:#ee4044; padding-left:10px; padding-right:10px'><span class='glyphicon glyphicon-fire'></span></span>";
					} else if (diff < 14 && diff >= 7 && zadolzh_pr < 100) {
						deadline_icon = "<span style='color:#f0ad4e; padding-left:10px; padding-right:10px'><span class='glyphicon glyphicon-exclamation-sign'></span></span>";
					} else {
						deadline_icon = "";
					}
				}
				// 
				d.srokpostavki = srok_postavki;
				d.deadline = date_dl + '' + deadline_icon;
				//
		...
		editor_chet_main.on('open', function(e) {
			...
			editor_chet_main.field('deadline_mailing_enbl').disable();
		...
		table_chet_chetf.on('draw', function() {
			if (table_chet_chetf.page.info().recordsTotal > 0) {
				$('#zayvview-chet-chetf thead').css('display', '');
			} else {
				$('#zayvview-chet-chetf thead').css('display', 'none');
			}
		});
...
// :::
?>
<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/zayvview/zayvview-chet/restr_3/css/zayvview-chet-main-customform.css">
				...
				<div class="Section">
					<div class="Block100">
						...
						<div class="Block100 fieldset-table-row">
							<div class="fieldset-table-cell" style="padding-bottom:0px">
								<fieldset>
									<editor-field name="kodzayv_transfer_enbl"></editor-field>
								</fieldset>
							</div>
							<div class="fieldset-table-cell" style="width:100%; padding-bottom:0px">
								<div>
					...
					<div class="Block100">
						<fieldset class="field30">
							<editor-field name="dognet_doczayvchet.srokpostavki"></editor-field>
						</fieldset>
						<fieldset class="field30">
						...
						<div class="Block100 fieldset-table-row">
							<div class="fieldset-table-cell" style="padding-bottom:0px">
								<fieldset>
									<editor-field name="deadline_mailing_enbl"></editor-field>
								</fieldset>
							</div>
							<div class="fieldset-table-cell" style="width:100%; padding-bottom:0px">
								<div>
									<div class="chekbox-inline-text text-muted">Уведомить на snab@atgs.ru, если до дедлайна срока поставки осталось 7 и менее дней</div>
								</div>
							</div>
						</div>
					</div>
...
// :::
?>
<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/zayvview/zayvview-chet/restr_3/css/zayvview-chet-chetf-customform.css">
...
<div id="tab-1" class="tab-pane fade in active">
	...
	<div class="col-xs-12 space30" style="padding:10px; background-color:#fafafa">
		...
		<div class="col-xs-3 col-sm-3 col-md-2 col-lg-2">
			<div class="input-group space10" style="width:100%">
				<label for="chZadolSearch_text"><b>Задолженность :</b></label>
	...
	<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/zayvview/zayvview-chet/restr_3/css/zayvview-chet-main.css">
	<?php // ----- ----- ----- ----- ----- 
	?>
	...
	<div class="" style="margin-top:15px; margin-bottom:60px">
		<div style="float: left">
			<p><span style="color: #f0ad4e" class="label label-icon"><span class="glyphicon glyphicon-exclamation-sign" title="До расчетной даты поставки менее 2-ух недель"></span></span><span style="font-size:0.9em; font-family: 'HelliosCond', sans-serif; font-weight: 400; color:#8d8d8d">До расчетной даты поставки менее 2-ух недель</span></p>
		</div>
		<div style="float: left">
			<p><span style="color: #ee4044" class="label label-icon"><span class="glyphicon glyphicon-fire" title="До расчетной даты поставки менее 1-ой недели"></span></span><span style="font-size:0.9em; font-family: 'HelliosCond', sans-serif; font-weight: 400; color:#8d8d8d">До расчетной даты поставки менее 1-ой недели</span></p>
		</div>
	</div>

	<?php // ----- ----- ----- ----- ----- 
	?>
	<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/zayvview/zayvview-chet/restr_3/css/zayvview-chet-chetf.css">
	<div class="space30"></div>
	<div class="col-xs-12 space30" style="padding:10px; background-color:#fafafa">
...
<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/zayvview/zayvview-chet/restr_3/css/zayvview-chet-transfer-zayvfrom.css">
<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/zayvview/zayvview-chet/restr_3/css/zayvview-chet-transfer-zayvto.css">
...
<div id="tab-3" class="tab-pane fade">
	...
	<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/zayvview/zayvview-chet/restr_3/css/zayvview-chet-archive.css">
	<?php // ----- ----- ----- ----- ----- 
	?>
	<div class="demo-html"></div>
	...
	<?php // ----- ----- ----- ----- ----- 
	?>
	<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/zayvview/zayvview-chet/restr_3/css/zayvview-chet-archive-chetf.css">
	<div class="space30"></div>
	<div class="col-xs-12 space30" style="padding:10px; background-color:#fafafa">
		<h4 class="text-uppercase" style="color:#31708f; font-family:'Oswald', sans-serif; font-weight:400; letter-spacing:0.4em">Счета-фактуры к выбранному счету</h4>
	</div>
