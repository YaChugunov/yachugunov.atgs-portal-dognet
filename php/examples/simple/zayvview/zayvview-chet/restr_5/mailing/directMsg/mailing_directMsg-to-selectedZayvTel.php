...
# ----- ----- -----
// Определяем Заявителя по справочнику Заявителей
...
// Определяем Получателя по таблице пользователей Портала
if (isset($_GET['recipient']) && !empty($_GET['recipient'])) {
	$_QRY_RECIPIENT = mysqlQuery("SELECT * FROM users WHERE kodzayvtel = " . $_GET['recipient']);
	$_ROW_RECIPIENT = mysqli_fetch_assoc($_QRY_RECIPIENT);
}
# ----- ----- -----
