...
if (isset($_GET['uniqueID'])) {
	$_QRY_CH = mysqlQuery("SELECT * FROM dognet_doczayvchet WHERE kodzayvchet = " . $_GET['uniqueID']);
	$_ROW_CH = mysqli_fetch_assoc($_QRY_CH);
}
// Выбираем поставщика выставившего счет
$_QRY_POST = mysqlQuery("SELECT * FROM sp_contragents WHERE kodcontragent = " . $_ROW_CH['kodpost']);
$_ROW_POST = mysqli_fetch_assoc($_QRY_POST);

// Выбираем связанный счет-фактуру
$_QRY_CHF = mysqlQuery("SELECT * FROM dognet_doczayvchetf WHERE kodzayvchet = " . $_ROW_CH['kodzayvchet']);
...
$message3 .= "
		<h3>Обновление по заявке № " . $_ROW_TIPZAYV['nametipzayvshotall'] . "-" . $_ROW_ZAYV['numberzayv'] . " от " . date('d.m.Y', strtotime($_ROW_ZAYV['datezayv'])) . "</h3>
...
if (isset($_GET['msgSubject'])) {
	switch ($_GET['msgSubject']) {
		...
		case 1:
			$message3 .= "
				<span><b>Поставщик '" . $_ROW_POST['nameshort'] . "' выставил счет № " . $_ROW_CH['zayvchetnumber'] . " от " . date('d.m.Y', strtotime($_ROW_CH['zayvchetdate'])) . "</b></span><br>
				<span>Сумма счета : " . number_format((float)($_ROW_CH['zayvchetsumma']), 2, '.', ' ') . " руб.</span><br>";
			break;
		case 2:
			$message3 .= "
				<span><b>Был произведен перенос счета № " . $_ROW_CH['zayvchetnumber'] . " от " . date('d.m.Y', strtotime($_ROW_CH['zayvchetdate'])) . " на сумму " . number_format((float)($_ROW_CH['zayvchetsumma']), 2, '.', ' ') . " руб. на другую заявку</b></span><br>
				<span>Текущая заявка - № " . $_ROW_TIPZAYV['nametipzayvshotall'] . "-" . $_ROW_ZAYV['numberzayv'] . "</span><br>";
			break;
		case 3:
			$message3 .= "
				<span><b>Был изменена сумма задолженности по счету № " . $_ROW_CH['zayvchetnumber'] . " от " . date('d.m.Y', strtotime($_ROW_CH['zayvchetdate'])) . "</b></span><br>
				<span>Текущая задолженность по счету - <span style='color:red'>" . number_format((float)($_ROW_CH['zayvchetzadol']), 2, '.', ' ') . " ( " . $_ROW_CH['zayvchetpr'] . "% закрытия )</span></span><br>";
			break;
		case 4:
			$message3 .= "
				<span><b>Произошли изменения в регистрационных данных выбранного счета</b></span><br>
				<span>Дата создания счета - " . date('d.m.Y', strtotime($_ROW_CH['zayvchetdate'])) . "</span><br>
				<span>№ счета : " . $_ROW_CH['zayvchetnumber'] . "</span><br>
				<span>Сумма счета : " . number_format((float)($_ROW_CH['zayvchetsumma']), 2, '.', ' ') . " руб.</span><br>
				<span>Задолженность по счету : " . number_format((float)($_ROW_CH['zayvchetzadol']), 2, '.', ' ') . " руб. ( " . $_ROW_CH['zayvchetpr'] . "% закрытия )</span><br>
				<span>Примечание : " . ((!empty($_ROW_CH['zayvchetcomment'])) ? $_ROW_CH['zayvchetcomment'] : '---') . "</span><br>";
			break;
		default:
			$message3 .= "";
	}
	...
	<table id="mail-preview-info" class="table table-responsive">
		<tbody>
			<tr style="">
				...
				<td width="70%" class="text-left"><span><?php echo "Тестовое сообщение (chugunov@atgs.ru)"; ?></span></td>
			</tr>
			<tr style="">
			...
			<tr style="">
				...
				<td width="70%" class="text-left"><span><?php echo "Чугунов Я. М. / " . $email_to; ?></span></td>
			</tr>
		</tbody>
	</table>
</div>
