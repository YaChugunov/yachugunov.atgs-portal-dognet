...
// Обработчик формы редактирование счета
	editor_zayv_child_dop = new $.fn.dataTable.Editor( {
	    ...
	    i18n: {
	        create: { title: "<h3>Новая позиция спецификации</h3>" },
	        edit: { title: "<h3>Изменить позицию спецификации</h3>" },
	        remove: {
	            button: "Удалить",
...
// Управление размером диалогового окна редактирования счета
	editor_zayv_child_dop.on( 'open', function () { $(".modal-dialog").css({
		"width":"50%",
		"min-width":"640px",
		"max-width":"800px"
		});
	} );
	editor_zayv_child_dop.off( 'close', function () { $(".modal-dialog").css({
		"width":"80%",
		"min-width":"none",
		"max-width":"none"
		});
		...
		columns: [
      {
	    ...
	    { text:	'<span class="glyphicon glyphicon-refresh"></span>', action:
	      function ( e, dt, node, config ) {
			...
			{ extend: "create", editor: editor_zayv_child_dop, text: '<span class="glyphicon glyphicon-plus"></span>',
				formButtons:
					[ 'Создать',
						{ text: 'Отмена', action: function () { this.close(); } }
					]
			},
			{ extend: "edit", editor: editor_zayv_child_dop, text: '<span class="glyphicon glyphicon-pencil"></span>',
				formButtons:
					[ 'Изменить',
						{ text: 'Отмена', action: function () { this.close(); } }
					]
			},
			{ extend: "remove", editor: editor_zayv_child_dop, text: '<span class="glyphicon glyphicon-remove"></span>' }
		],
		drawCallback: function () {
...
// ----- ----- -----
					...
					type: "upload",
					display: function ( id ) { return '<a target="_blank" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet'+editor_zayv_child_chet.file( 'dognet_doczayvchet_files', id ).file_webpath+'"><h4>СКАЧАТЬ ФАЙЛ</h4></a>'; },
					dragDrop: false,
					dragDropText: "",
					clearText: "",
					fileReadText: "Файл загружается",
...
// ----- ----- -----
// Управление размером диалогового окна редактирования счета
	editor_zayv_child_chet.on( 'open', function () { $(".modal-dialog").css({
		"width":"50%",
		"min-width":"640px",
		"max-width":"800px"
		});
	} );
	editor_zayv_child_chet.off( 'close', function () { $(".modal-dialog").css({
		"width":"80%",
		"min-width":"none",
		"max-width":"none"
		});
			...
			{ data: "dognet_doczayvchet.zayvchetzadol" },
      {
	    ...
	    { text:	'<span class="glyphicon glyphicon-refresh"></span>', action:
	      function ( e, dt, node, config ) {
			...
			{ extend: "create", editor: editor_zayv_child_chet, text: '<span class="glyphicon glyphicon-plus"></span>',
				formButtons:
					[ 'Создать',
						{ text: 'Отмена', action: function () { this.close(); } }
					]
			},
			{ extend: "edit", editor: editor_zayv_child_chet, text: '<span class="glyphicon glyphicon-pencil"></span>',
				formButtons:
					[ 'Изменить',
						{ text: 'Отмена', action: function () { this.close(); } }
					]
			},
			{ extend: "remove", editor: editor_zayv_child_chet, text: '<span class="glyphicon glyphicon-remove"></span>' }
		],
		drawCallback: function () {
...
// Обработчик формы редактирование счета-фактуры
	editor_chet_child_chetf = new $.fn.dataTable.Editor( {
	    ...
	    i18n: {
	        create: { title: "<h3>Новый счет-фактура</h3>" },
...
// ----- ----- -----
					...
					type: "upload",
					display: function ( id ) { return '<a target="_blank" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet'+editor_chet_child_chetf.file( 'dognet_doczayvchetf_files', id ).file_webpath+'"><h4>СКАЧАТЬ ФАЙЛ</h4></a>'; },
					dragDrop: false,
					dragDropText: "",
					clearText: "",
					fileReadText: "Файл загружается",
...
// Управление размером диалогового окна редактирования счета-фактуры
	editor_chet_child_chetf.on( 'open', function () { $(".modal-dialog").css({
		"width":"50%",
		"min-width":"640px",
		"max-width":"800px"
		});
	} );
	editor_chet_child_chetf.off( 'close', function () { $(".modal-dialog").css({
		"width":"80%",
		"min-width":"none",
		"max-width":"none"
		});
	...
	table_chet_child_chetf = $('#zayvview-chet-child-chetf').DataTable( {
	    ...
	    select: {
			...
			columns: [
	      {
				...
				{ data: "dognet_doczayvchetf.zayvchetfdate", className: "" },
				{ data: "dognet_doczayvchetf.zayvchetfnumber", className: "" },
				{ data: "dognet_doczayvchetf.zayvchetfcomment", className: "" },
				{ data: "dognet_doczayvchetf.zayvchetfsumma", className: "" },
	      {
          ...
          defaultContent: "",
		    ...
		    { text:	'<span class="glyphicon glyphicon-refresh"></span>', action:
		      function ( e, dt, node, config ) {
				...
				{ extend: "create", editor: editor_chet_child_chetf, text: '<span class="glyphicon glyphicon-plus"></span>',
					formButtons:
						[ 'Создать',
							{ text: 'Отмена', action: function () { this.close(); } }
						]
				},
				{ extend: "edit", editor: editor_chet_child_chetf, text: '<span class="glyphicon glyphicon-pencil"></span>',
					formButtons:
						[ 'Изменить',
							{ text: 'Отмена', action: function () { this.close(); } }
						]
				},
				{ extend: "remove", editor: editor_chet_child_chetf, text: '<span class="glyphicon glyphicon-remove"></span>' }
			]
		} );
// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
//
...
// Обработчики событий таблицы заявок
	...
	table_zayv_main.on( 'deselect', function () {
		...
		table_zayv_child_chet.row( { selected: true } ).deselect();
    table_zayv_child_chet.ajax.reload(null, false);
		table_zayv_child_dop.buttons().disable();
		table_zayv_child_dop.row( { selected: true } ).deselect();
...
// Обработчики событий таблицы счетов
	...
	table_zayv_child_chet.on( 'deselect', function () {
		...
		table_chet_child_chetf.row( { selected: true } ).deselect();
    table_chet_child_chetf.ajax.reload(null, false);
	} );
// ----- ----- -----
// Обработчики событий редактора счета-фактуры
