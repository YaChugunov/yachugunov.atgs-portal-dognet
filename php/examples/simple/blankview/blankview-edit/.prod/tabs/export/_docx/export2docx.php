...
//
require ($_SERVER['DOCUMENT_ROOT'] . '/dognet/php/examples/simple/blankview/blankview-edit/.prod/tabs/export/_docx/export2docx_styles.inc.php');
// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
//
...
$_BlankRowID   = !empty($_ROW_DOCBLANK['blank_rowID']) ? $_ROW_DOCBLANK['blank_rowID'] : '';
//
if ($_GET['blankType'] !== 'SUB') {
	$_QRY_ZAKAZ = mysqlQuery("SELECT nameshort FROM sp_contragents WHERE kodcontragent = '" . $_ROW_DOCBLANK['kodzakaz'] . "'");
	$_ROW_ZAKAZ = mysqli_fetch_assoc($_QRY_ZAKAZ);
	$_ZAKNAME   = !empty($_ROW_ZAKAZ['nameshort']) ? $_ROW_ZAKAZ['nameshort'] : '';
} else {
	$_QRY_ZAKAZ = mysqlQuery("SELECT nameshort FROM sp_contragents WHERE kodcontragent = '" . $_ROW_DOCBLANK['kodsubpodr'] . "'");
	$_ROW_ZAKAZ = mysqli_fetch_assoc($_QRY_ZAKAZ);
	$_ZAKNAME   = !empty($_ROW_ZAKAZ['nameshort']) ? $_ROW_ZAKAZ['nameshort'] : '';
}
//
...
switch ($_GET['blankType']) {
	...
	case 'SUB':
		...
		break;
	case 'PIR':
		$cell_S1->addTextRun($_TXTRUN_Align_Left)->addText('Бланк требований на ПИР № ' . $_NumberBlank, $_FONT_H13_UB);
		$_QRY_BLANKWORK = mysqlQuery("SELECT * FROM dognet_blankdocpir WHERE id = '" . $_BlankRowID . "' AND kodblankwork='" . $_GET['blankID'] . "' AND kodtipblank='DO'");
		$_ROW_BLANKWORK = mysqli_fetch_assoc($_QRY_BLANKWORK);
		break;
	default:
...
if ($_GET['blankType'] !== 'SUB') {
	...
	$cell = $table->addCell(11000, $_TBL_Body_Cell_B);
	$cell->addText($_ZAKNAME, $_FONT_P10_B, $_TXT_INCELL_1L);
	// ----- ----- -----
	// 3.2 Исполнитель (поставщик)
	//
	$table = $section->addTable($_TBL_Common_Fixed);
...
if ($_GET['blankType'] !== 'SUB') {
	...
	$cell    = $table->addCell(18500, $_TBL_Body_Cell_Clear);
	$textrun = $cell->addTextRun();
	if ($_ROW_DOCBLANK['kodusetender'] == '1') {
		$textrun->addFormField('checkbox')->setDefault(true);
		$textrun->addText(' По итогам выигранного тендера', $_FONT_P10, $_TXT_INCELL_1L);
	} else {
		$textrun->addFormField('checkbox');
		$textrun->addText(' По итогам выигранного тендера', $_FONT_P10, $_TXT_INCELL_1L);
	}
...
$cell = $table->addCell(18500, $_TBL_Body_Cell_Clear);
$cell->addText('Срок исполнения договорных обязательств', $_FONT_P11_B, $_TXT_INCELL_1L);
// ----- ----- -----
// 9.1 Дата
//
$table = $section->addTable($_TBL_Common_Fixed);
...
//
// ДЛЯ БЛАНКА НА ПИР ($_GET['blankType']=="PIR"
//
elseif ($_GET['blankType'] == 'PIR') {
	//
	// ----- ----- -----
	// 11.1 Входят в стоимость договора
	//
	...
	$cell = $table->addCell(1500, $_TBL_Body_Cell_Clear);
	$cell->addText('3)', $_FONT_P10, $_TXT_INCELL_1L);
	$cell    = $table->addCell(11500, $_TBL_Body_Cell_Clear);
	$textrun = $cell->addTextRun();
	if ($_ROW_BLANKWORK['kodusekomrasx3'] == '1') {
		$textrun->addFormField('checkbox')->setDefault(true);
		$textrun->addText(' входят в стоимость договора с установленным лимитом', $_FONT_P10, $_TXT_INCELL_1L);
	} else {
		$textrun->addFormField('checkbox');
		$textrun->addText(' входят в стоимость договора с установленным лимитом', $_FONT_P10, $_TXT_INCELL_1L);
	}
	$cell = $table->addCell(3000, $_TBL_Body_Cell_B);
	$cell->addText(($_ROW_BLANKWORK['summalimitmis'] != 0.0) ? $_ROW_BLANKWORK['summalimitmis'] : '', $_FONT_P10_B, $_TXT_INCELL_1C);
	$cell = $table->addCell(4000, $_TBL_Body_Cell_Clear);
	//
	$textrun->addText('', array('name' => 'Arial', 'size' => 1));
	//
	// ----- ----- -----
...
// $_QRY_DOC = mysqlQuery("SELECT docnumber FROM dognet_docbase WHERE koddoc = '" . $_ROW_DOCBLANK['koddoc'] . "'");
// $_ROW_DOC = mysqli_fetch_assoc($_QRY_DOC);

// $_QRY_STAGE = mysqlQuery("SELECT numberstage FROM dognet_dockalplan WHERE kodkalplan = '" . $_ROW_DOCBLANK['kodkalplan'] . "'");
...
elseif ($_GET['blankType'] == 'PIR') {
	...
	if ($_ROW_BLANKWORK['koduserisk1'] == '1') {
		$textrun->addFormField('checkbox')->setDefault(true);
		$textrun->addText(' осуществимость выполнения договора по срокам', $_FONT_P10, $_TXT_INCELL_1L);
	} else {
		$textrun->addFormField('checkbox');
		$textrun->addText(' осуществимость выполнения договора по срокам', $_FONT_P10, $_TXT_INCELL_1L);
	}
	...
	if ($_ROW_BLANKWORK['koduserisk2'] == '1') {
		$textrun->addFormField('checkbox')->setDefault(true);
		$textrun->addText(' полнота исходных данных', $_FONT_P10, $_TXT_INCELL_1L);
	} else {
		$textrun->addFormField('checkbox');
		$textrun->addText(' полнота исходных данных', $_FONT_P10, $_TXT_INCELL_1L);
	}
	...
	if ($_ROW_BLANKWORK['koduserisk4'] == '1') {
		$textrun->addFormField('checkbox')->setDefault(true);
		$textrun->addText(' сезон технического обследования объекта', $_FONT_P10, $_TXT_INCELL_1L);
	} else {
		$textrun->addFormField('checkbox');
		$textrun->addText(' сезон технического обследования объекта', $_FONT_P10, $_TXT_INCELL_1L);
	}
	...
	//
	// ----- ----- -----
	// 16.5 Соблюдение сроков оплаты
	//
	$table = $section->addTable($_TBL_Common_Fixed);
	$table->addRow(350, $_TBL_Body_Row);
	$cell = $table->addCell(1500, $_TBL_Body_Cell_Clear);
	$cell->addText('5)', $_FONT_P11, $_TXT_INCELL_1L);
	$cell    = $table->addCell(18500, $_TBL_Body_Cell_Clear);
	$textrun = $cell->addTextRun();
	if ($_ROW_BLANKWORK['koduserisk5'] == '1') {
		$textrun->addFormField('checkbox')->setDefault(true);
		$textrun->addText(' соблюдение срока оплаты по договору', $_FONT_P10, $_TXT_INCELL_1L);
	} else {
		$textrun->addFormField('checkbox');
		$textrun->addText(' соблюдение срока оплаты по договору', $_FONT_P10, $_TXT_INCELL_1L);
	}
	...
	$table->addRow(350, $_TBL_Body_Row);
	$cell = $table->addCell(1500, $_TBL_Body_Cell_Clear);
	$cell->addText('6)', $_FONT_P10, $_TXT_INCELL_1L);
	$cell    = $table->addCell(3500, $_TBL_Body_Cell_Clear);
	$textrun = $cell->addTextRun();
	if ($_ROW_BLANKWORK['koduserisk6'] == '1') {
		$textrun->addFormField('checkbox')->setDefault(true);
		$textrun->addText(' иное', $_FONT_P10, $_TXT_INCELL_1L);
	} else {
		$textrun->addFormField('checkbox');
...
function countDown() {
    ...
    timer = setInterval(function() {
        ...
        blankID = '<?php echo $_GET['blankID']; ?>';
        url1 = 'http://' + host + '/dognet/dognet-blankview.php?blankview_type=edit';
        url2 = 'htp://' + host + '/dognet/dognet-blankview.php?blankview_type=edit&export=yes&blankID=' +
            blankID;
        if (i < 0) {
            clearInterval(timer);
            document.getElementById("link").innerHTML =
                "<div class='space20'><span style='padding:0 15px'><a class='return-link' href=" + url1 +
                ">Вернуться в Бланки</a></span></div><div class='space20'><span style='padding:0 15px'><a class='return-link' href=" +
