...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	...
	// Выбираем идентификаторы создаваемого документа
		...
		if ($_QRY_blankpaper) {
			...
			$_QRY_blankRowID = $db->sql( "SELECT id FROM dognet_blankdocpnr WHERE kodblankwork=".$_kodblankwork )->fetchAll();
#
#
			if ($_docFileID != "") {
			...
			// Переименовываем файл
				...
				if ($_QRY_paperfiles[0]['flag'] == "0") {
					...
					$__newFileName = "NEW-B-{$_numberpril}-PNR-DO-{$_kodblankwork}";
...
if ( !isset($_POST['kodblankwork_tab3_blankpril']) || !is_numeric($_POST['kodblankwork_tab3_blankpril']) ) {
	echo json_encode( [ "data" => [] ] );
}
else {
$_KODBLANKWORK = $_POST['kodblankwork_tab3_blankpril'];
...
// 	->leftJoin( 'dognet_blankdocpnr', 'dognet_blankdocpnr.kodblankwork', '=', 'dognet_blankworkpril.kodblankwork' )
