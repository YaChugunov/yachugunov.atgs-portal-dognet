...
// ПОЛНОЕ РЕДАКТИРОВАНИЕ БЛАНКА ::: Редактор
	var editor_blankview_edit = new $.fn.dataTable.Editor( {
		display: "bootstrap",
		ajax: "php/examples/php/blankview/blankview-current/dognet-blankview-currentGIP-process.php",
		table: "#blankview-current-table",
    i18n: {
...
// ----- ----- ----- ----- -----
/*
	$('#blankview-current-table').on( 'dblclick', 'tbody tr', function () {
	    editor_blankview_edit.edit( this, {
	        focus: 0
	    } );
	} );
*/
...
// РЕДАКТИРОВАНИЕ ГОТОВОГО БЛАНКА ::: Редактор
	var editor_blankview_current = new $.fn.dataTable.Editor( {
		display: "bootstrap",
		ajax: "php/examples/php/blankview/blankview-current/dognet-blankview-currentGIP-process.php",
		table: "#blankview-current-table",
    i18n: {
...
// ----- ----- ----- ----- -----
	var openVals;
  editor_blankview_current.on( 'preClose', function ( e ) {
      // On close, check if the values have changed and ask for closing confirmation if they have
      if ( openVals !== JSON.stringify( editor_blankview_current.get() ) ) {
          return confirm( 'Вы изменили данные формы. Уверены, что хотите выйти из редактирования?' );
      }
  } )
// ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
// БЛАНК ГОТОВЫЙ ::: Таблица данных
	var table_blankview_current = $('#blankview-current-table').DataTable( {
		dom: "<'row'<'col-sm-9'B><'col-sm-3'>>" + "<'row'<'col-sm-12'tr>>" + "<'row'<'col-sm-4'i><'col-sm-8'p>>",
// 		dom: "<'space50'r>tip",
		language: {	url: "russian.json" },
		ajax: {
			url: "php/examples/php/blankview/blankview-current/dognet-blankview-currentGIP-process.php",
			type: "POST"
		},
		serverSide: true,
		columns: [
		...
		buttons: [
			{ text:	'<span class="glyphicon glyphicon-refresh"></span>', action:
				function ( e, dt, node, config ) {
					...
					table_blankview_current.columns().search('');
					table_blankview_current.draw();
				}
			},
			{ extend: "create", editor: editor_blankview_edit, text: "НОВЫЙ БЛАНК (ПОСТАВКА)",
				formButtons:
					[ 'Создать',
						{ text: 'Отмена', action: function () { this.close(); } }
					]
			},
			{ extend: "create", editor: editor_blankview_edit, text: "НОВЫЙ БЛАНК (ПНР)",
				formButtons:
					[ 'Создать',
						{ text: 'Отмена', action: function () { this.close(); } }
					]
			},
			{ extend: "create", editor: editor_blankview_edit, text: "НОВЫЙ БЛАНК (СУБПОДРЯД)",
				formButtons:
					[ 'Создать',
						{ text: 'Отмена', action: function () { this.close(); } }
					]
			},
			{ extend: "edit", editor: editor_blankview_current, text: "ИЗМЕНИТЬ",
				formButtons:
					[ 'Изменить',
						{ text: 'Отмена', action: function () { this.close(); } }
					]
			},
			{ extend: "remove", editor: editor_blankview_current, text: "УДАЛИТЬ" }
		],
    rowGroup: {
...
// ----- ----- ----- ----- -----
		$('#globalSearch_button').click(function(e){
			table_blankview_current.search($("#globalSearch_text").val()).draw();
		});
		$('#clearSearch_button').click(function(e){
			table_blankview_current.search('').draw();
			$('#globalSearch_text').val('');
		});
		$('#columnSearch_btnApply').click(function(e){
			table_blankview_current
				.columns(1)
...
// Array to track the ids of the edit displayed rows
    ...
    $('#blankview-current-table tbody').on( 'click', 'tr td.details-control', function () {
        var tr = $(this).closest('tr');
        var row = table_blankview_current.row( tr );
        ...
        if ( row.child.isShown() ) {
            ...
            tr.addClass( 'edit' );
			rowData = table_blankview_current.row( row );
			d = row.data();
			rowData.child( <?php include('templates/blankview-current-table.tpl'); ?> ).show();
...
// On each draw, loop over the `detailRows` array and show any child rows
	table_blankview_current.on( 'draw', function () {
...
// БЛАНК РАБОЧИЙ ::: Редактор
	var editor_blankview_docfiles = new $.fn.dataTable.Editor( {
		display: "bootstrap",
		ajax: "php/examples/php/blankview/blankview-current/dognet-blankview-docfiles-process.php",
...
// 		dom: "<'space50'r>tip",
		language: {	url: "russian.json" },
		ajax: {
			url: "php/examples/php/blankview/blankview-current/dognet-blankview-docfiles-process.php",
			type: 'post',
      data: function ( d ) {
          var selected = table_blankview_current.row( { selected: true } );
          if ( selected.any() ) {
...
// ПРИЛОЖЕНИЯ БЛАНКА ::: Редактор
	var editor_blankview_blankpril = new $.fn.dataTable.Editor( {
		display: "bootstrap",
		ajax: "php/examples/php/blankview/blankview-current/dognet-blankview-blankpril-process.php",
...
// 		dom: "<'space50'r>tip",
		language: {	url: "russian.json" },
		ajax: {
			url: "php/examples/php/blankview/blankview-current/dognet-blankview-blankpril-process.php",
			type: 'post',
      data: function ( d ) {
          var selected = table_blankview_current.row( { selected: true } );
          if ( selected.any() ) {
...
//
// ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
	table_blankview_current.on( 'select', function () {
			table_blankview_current.buttons().enable();
	...
	table_blankview_current.on( 'deselect', function () {
...
/*
/* Общее для таблицы */
#blankview-current-table {  }
#blankview-current-table .details-control:hover { cursor:hand }
#blankview-current-table > tbody > tr.selected, #docview-details-tab3 > tbody > tr > .selected { color:#000;	background-color:#e9e9e9 }
/*
/*
/* Заголовок таблицы */
#blankview-current-table > thead {	background-color:#111;	color:#fff; font-family:'Oswald', sans-serif; border-bottom:none;	border-top:none }
#blankview-current-table > thead > tr > th { border-bottom:none; font-weight:400; font-size:1.0em; text-transform:uppercase }
#blankview-current-table .sorting:after, #blankview-current-table .sorting_asc:after, #blankview-current-table .sorting_desc:after { display:none }
#blankview-current-table > thead > tr > th.sorting_asc { padding-right:5px }
#blankview-current-table > thead > tr > th.sorting_desc { padding-right:5px }
/*
/*
/* Тело таблицы */
#blankview-current-table {  }
#blankview-current-table > tbody { font-family:'Helioscond', sans-serif; font-size:1.0em; font-weight:300; color:#666; border-bottom:none; border-top:none }
#blankview-current-table > tbody > tr > td { padding:5px; line-height:1.42857143; vertical-align:middle; border-top:none }
#blankview-current-table > thead > tr > th { padding:8px 5px; border-bottom:none }
#blankview-current-table > tbody > tr > td {  }
#blankview-current-table > tbody > tr > td:last-child { text-align:right }

#blankview-current-table > tfoot > tr > td { padding:5px 4px }
#blankview-current-table > tfoot > tr > td:last-child { text-align:right }
#blankview-current-table > tfoot {
...
----- ----- ----- ----- ----- ----- ----- ----- ----- -----
*/
	#blankview-current-table > thead > tr > th:first-child, #blankview-current-table > tbody > tr > td:first-child { width:1%; text-align:center }
	#blankview-current-table > thead > tr > th:nth-child(2), #blankview-current-table > tbody > tr > td:nth-child(2) { width:3%; text-align:center; border-left:1px #fff solid }
	#blankview-current-table > thead > tr > th:nth-child(3), #blankview-current-table > tbody > tr > td:nth-child(3) { width:4%; text-align:center; border-left:1px #fff solid }
	#blankview-current-table > thead > tr > th:nth-child(4), #blankview-current-table > tbody > tr > td:nth-child(4) { width:45%; text-align:left; border-left:1px #fff solid }
	#blankview-current-table > thead > tr > th:nth-child(5), #blankview-current-table > tbody > tr > td:nth-child(5) { width:19%; text-align:left; border-left:1px #fff solid }
	#blankview-current-table > thead > tr > th:nth-child(6), #blankview-current-table > tbody > tr > td:nth-child(6) { width:5%; text-align:center; border-left:1px #fff solid }
	#blankview-current-table > thead > tr > th:nth-child(7), #blankview-current-table > tbody > tr > td:nth-child(7) { width:4%; text-align:center; border-left:1px #fff solid }
	#blankview-current-table > thead > tr > th:nth-child(8), #blankview-current-table > tbody > tr > td:nth-child(8) { width:7%; text-align:center; border-left:1px #fff solid }
	#blankview-current-table > thead > tr > th:nth-child(9), #blankview-current-table > tbody > tr > td:nth-child(9) { width:8%; text-align:center; border-left:1px #fff solid }
	#blankview-current-table > thead > tr > th:nth-child(10), #blankview-current-table > tbody > tr > td:nth-child(10) { width:7%; text-align:center; border-left:1px #fff solid }
	#blankview-current-table > thead > tr > th:nth-child(11), #blankview-current-table > tbody > tr > td:nth-child(11) { width:5%; text-align:center; border-left:1px #fff solid }
	#blankview-current-table .details-control:hover { cursor:pointer }

/*
----- ----- ----- ----- ----- ----- ----- ----- ----- -----
*/
	#blankview-current-table .sorting_asc:after { display:none }
	#blankview-current-table > thead > tr > th.sorting_asc { padding-right:8px }
/*
----- ----- ----- ----- ----- ----- ----- ----- ----- -----
*/
	...
	#blankview-current-filters { padding:15px 0; background-color:#f1f1f1 }
...
#blankview-docfiles-table .sorting:after, #blankview-current-table .sorting_asc:after, #blankview-current-table .sorting_desc:after { display:none }
...
#blankview-blankpril-table .sorting:after, #blankview-current-table .sorting_asc:after, #blankview-current-table .sorting_desc:after { display:none }
...
#blankview-current-table .group { font-family:'Oswald', sans-serif; font-size:1.3em; font-weight:400; color:#000 }
#blankview-current-table > tbody > tr.dtrg-start.dtrg-level-0 > td { font-family:'Oswald', sans-serif; font-size:1.3em; font-weight:400; text-align:left; background-color:#d9d9d9; color:#111; padding:5px }
#blankview-current-table > tbody > tr.dtrg-end.dtrg-level-0 > td { font-family:'Oswald', sans-serif; font-size:1.4em; font-weight:500; color:#000; background-color:#f1f1f1 }
#blankview-current-table > tbody > tr.dtrg-end.dtrg-level-0 > td { font-family:'Oswald', sans-serif; font-size:1.4em; font-weight:500; color:#000; background-color:#f1f1f1 }
.itogo-lvl0 {  }
  ...
  <div id="blanksearch-filters-block" class="panel-group">
    <div class="panel panel-default">
      ...
      <div id="blanksearch-filters" class="panel-collapse collapse">
				...
				<div id="blankview-current-filters" class="panel-body space30">
...
?>
	...
	<table id="blankview-current-table" class="table table-striped" cellspacing="0" width="100%">
			<thead>
				<tr>
