<script type="text/javascript" language="javascript" class="init">
	...
	function ajaxRequest_sendMailSUB(blanktip, blankname, ispolname, ispolrukname, objname, zakname, summablank, responseHandler) {
		var response = false;
		// Fire off the request to /form.php
		request = $.ajax({
			url: "php/examples/simple/blankview/blankview-gip/restr_5/tabs/php/sendMail_onSubmit-newBlankSUB.php",
			type: "post",
			cache: false,
			data: {
				blanktip: blanktip,
	...
	$(document).ready(function() {
		...
		var detailRows_sub = [];
		$('#blankview-gip-sub-table tbody').on('click', 'tr td.details-control', function() {
			...
			if (row.child.isShown()) {
				...
				d = row.data();
				rowData.child(<?php include('templates/blankview-gip-sub-blanktemplate-table.tpl'); ?>).show();
		...
		var editor_blankview_gip_sub_blankpril = new $.fn.dataTable.Editor({
			...
			fields: [{
				...
				type: "upload",
				display: function(id) {
					return '<h4><a target="_blank" href="' + editor_blankview_gip_sub_blankpril.file('dognet_blankworkpril_files', id).file_webpath + '"><span class="glyphicon glyphicon-eye-open"></span></a></h4>';
				},
				dragDrop: false,
				clearText: "",
		...
		editor_blankview_gip_sub_blankpril.on('initCreate', function(e) {
			...
			editor_blankview_gip_sub_blankpril.field('dognet_blankworkpril.msgDocFileID').val('Сначала создайте запись!');
		...
		//
		// ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
		table_blankview_gip_sub.on('select', function() {
		...
		editor_blankview_edit_sub.on('preSubmit', function(e, data, action) {
			...
			if ((action === "create") && editor_blankview_edit_sub.field('dognet_blankdocsub.kodblankcreate').val() == 1 && editor_blankview_edit_sub.field('sendEmailSUB').val() == 1) {
				...
				ajaxRequest_sendMailSUB(_blanktip, _blankname, _ispolname, _ispolrukname, _objname, _zakname, _summablank, 'sendMail');

			}
			// Осуществляем глубокое копирование объекта и инициализируем переменную
			else if ((action === "edit") && editor_blankview_edit_sub.field('dognet_blankdocsub.kodblankcreate').val() == 1 && editor_blankview_edit_sub.field('sendEmailSUB').val() == 1) {
...
// :::
?>
<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/blankview/blankview-gip/restr_5/tabs/css/blankview-gip-tab4_sub.css">
<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/blankview/blankview-gip/restr_5/tabs/css/blankview-gip-tab4_summadop.css">
<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/blankview/blankview-gip/restr_5/tabs/css/blankview-gip-tab4_doc_files.css">
<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/blankview/blankview-gip/restr_5/tabs/css/blankview-gip-tab4_pril_files.css">
<?php
// ----- ----- ----- ----- -----
				...
				<div class="Block80">
					<div class="Block100 fieldset-table-row">
						...
						<div class="fieldset-table-cell" style="width:100%">
							<div>
								<div class="chekbox-inline-text">Отправить заявку в Отдел договоров. После этого редактирование бланка будет уже невозможно!</div>
							</div>
						</div>
					</div>
				</div>
