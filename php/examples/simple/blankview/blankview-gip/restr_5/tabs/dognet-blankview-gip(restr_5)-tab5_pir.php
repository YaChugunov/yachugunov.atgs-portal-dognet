<script type="text/javascript" language="javascript" class="init">
	...
	function ajaxRequest_sendMailPIR(blanktip, blankname, ispolname, ispolrukname, objname, zakname, summablank, responseHandler) {
		var response = false;
		// Fire off the request to /form.php
		request = $.ajax({
			url: "php/examples/simple/blankview/blankview-gip/restr_5/tabs/php/sendMail_onSubmit-newBlankPIR.php",
			type: "post",
			cache: false,
			data: {
				blanktip: blanktip,
	...
	$(document).ready(function() {
		...
		var editor_blankview_edit_pir = new $.fn.dataTable.Editor({
			...
			i18n: {
				create: {
					title: "<h3>Создать новую заявку на договор ПИР2</h3>"
				},
				edit: {
		...
		$('#blankview-gip-pir-table tbody').on('click', 'tr td.details-control', function() {
			...
			if (row.child.isShown()) {
				...
				d = row.data();
				rowData.child(<?php include('templates/blankview-gip-pir-blanktemplate-table.tpl'); ?>).show();
		...
		var editor_blankview_gip_pir_blankpril = new $.fn.dataTable.Editor({
			...
			fields: [{
				...
				type: "upload",
				display: function(id) {
					return '<h4><a target="_blank" href="' + editor_blankview_gip_pir_blankpril.file('dognet_blankworkpril_files', id).file_webpath + '"><span class="glyphicon glyphicon-eye-open"></span></a></h4>';
				},
				dragDrop: false,
				clearText: "",
		...
		editor_blankview_gip_pir_blankpril.on('initCreate', function(e) {
			...
			editor_blankview_gip_pir_blankpril.field('dognet_blankworkpril.msgDocFileID').val('Сначала создайте запись!');
		...
		//
		// ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
		table_blankview_gip_pir.on('select', function() {
		...
		editor_blankview_edit_pir.on('preSubmit', function(e, data, action) {
			...
			if ((action === "create") && editor_blankview_edit_pir.field('dognet_blankdocpir.kodblankcreate').val() == 1 && editor_blankview_edit_pir.field('sendEmailPIR').val() == 1) {
				...
				ajaxRequest_sendMailPIR(_blanktip, _blankname, _ispolname, _ispolrukname, _objname, _zakname, _summablank, 'sendMail');

			}
			// Осуществляем глубокое копирование объекта и инициализируем переменную
			else if ((action === "edit") && editor_blankview_edit_pir.field('dognet_blankdocpir.kodblankcreate').val() == 1 && editor_blankview_edit_pir.field('sendEmailPIR').val() == 1) {
...
// :::
?>
<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/blankview/blankview-gip/restr_5/tabs/css/blankview-gip-tab5_pir.css">
<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/blankview/blankview-gip/restr_5/tabs/css/blankview-gip-tab5_summadop.css">
<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/blankview/blankview-gip/restr_5/tabs/css/blankview-gip-tab5_doc_files.css">
<link rel="stylesheet" href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/dognet/php/examples/simple/blankview/blankview-gip/restr_5/tabs/css/blankview-gip-tab5_pril_files.css">
<?php
// ----- ----- ----- ----- -----
