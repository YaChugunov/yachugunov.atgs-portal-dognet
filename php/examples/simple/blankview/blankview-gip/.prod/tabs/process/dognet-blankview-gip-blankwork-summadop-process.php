...
#
#
	->on( 'preGet', function ( $editor_blanksummadop, $id ) use ($_KODTIPBLANK) {
		if ($_KODTIPBLANK == "CR") {
			$editor_blanksummadop
				->where( function ( $q ) use ($_KODTIPBLANK) {
			    $q->where( 'dognet_blanksummadop.koddel', '99', '!=' );
			    $q->and_where( 'dognet_blanksummadop.kodtipblank', $_KODTIPBLANK, '=' );
				} );
		}
		if ($_KODTIPBLANK == "RD") {
			$editor_blanksummadop
...
// 	->leftJoin( 'dognet_docblankwork', 'dognet_docblankwork.kodblankwork', '=', 'dognet_blanksummadop.kodblankwork' )
	...
	->where( 'dognet_blanksummadop.kodblankwork', $_KODBLANKWORK )
	->process( $_POST )
	->json();
