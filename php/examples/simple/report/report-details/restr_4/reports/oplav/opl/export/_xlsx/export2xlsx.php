...
// require($_SERVER['DOCUMENT_ROOT'] . "/dognet/_assets/_PHPExcel/PHPExcel.php");
//
//
//
// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
...
$properties->setTitle('Справка об оплатах по всем договорам');
$properties->setDescription('Справка об оплатах по всем договорам');
...
$properties->setSubject('Справка об оплатах по всем договорам');
...
if (checkIsItSuperadmin($_SESSION['id']) == 1) {
    include($_SERVER['DOCUMENT_ROOT'] . "/dognet/php/examples/simple/report/report-details/restr_4/reports/oplav/opl/export/_xlsx/export2xlsx_opl_v3.php");
} else {
    include($_SERVER['DOCUMENT_ROOT'] . "/dognet/php/examples/simple/report/report-details/restr_4/reports/oplav/opl/export/_xlsx/export2xlsx_opl_v3.php");
}
...
// $xmlWriter = new PHPExcel_Writer_Excel2007($objPHPExcel);
// $xmlWriter->save('php://output');
$filepath = $_SERVER['DOCUMENT_ROOT'] . "/dognet/tmp/";
$filename = "SPRAVKA-OPLCHF_" . date('YmdHis') . ".XLSX";
//$xmlWriter->setPreCalculateFormulas(true);
$xmlWriter->save($filepath . $filename);

// Делаем запись в системный лог
// Все параметры в таблице portal_log_messages
// PORTAL_SYSLOG('99942200', '0000001', null, $_GET['reportview'], "Оплаты по всем договорам за период", "EXCEL");
...
function countDown() {
    ...
    timer = setInterval(function() {
        ...
        url2 = 'http://' + host + '/dognet/dognet-report.php?reportview=opl&export=yes';
        if (i < 0) {
            clearInterval(timer);
