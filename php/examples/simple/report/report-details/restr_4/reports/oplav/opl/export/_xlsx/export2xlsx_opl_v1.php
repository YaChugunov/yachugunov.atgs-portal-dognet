...
while ($_ROW_dockalplan = mysqli_fetch_assoc($_QRY_dockalplan)) {
	$_QRY_kalplanchf = mysqlQuery("SELECT * FROM dognet_kalplanchf WHERE koddel<>'99' AND kodkalplan = '" . $_ROW_dockalplan['kodkalplan'] . "' AND kodchfact IN (SELECT kodchfact FROM dognet_oplatachf WHERE koddel<>'99' AND (dateopl BETWEEN '" . $start_date . "' AND '" . $end_date . "'))");
	$_SUM_ONSTAGE = 0.0;
	//
	while ($_ROW_kalplanchf = mysqli_fetch_assoc($_QRY_kalplanchf)) {
		...
		$line++;
		//
		$_QRY_oplatachf = mysqlQuery("SELECT * FROM dognet_oplatachf WHERE koddel<>'99' AND (dateopl BETWEEN '" . $start_date . "' AND '" . $end_date . "') AND kodchfact = '" . $_ROW_kalplanchf['kodchfact'] . "'");
		$_SUM_ONCHF = 0.0;
		//
		while ($_ROW_oplatachf = mysqli_fetch_assoc($_QRY_oplatachf)) {
			...
			$activeSheet->setCellValue("F{$line}", 'Оплата');
			// Выравнивание по горизонтали - центр
			$activeSheet->getStyle("G{$line}")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			$activeSheet->setCellValue("G{$line}", $_OPLDATE);
			...
			$activeSheet->setCellValue("H{$line}", $_OPLSUMMA);
			$activeSheet->getStyle("H{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
			$activeSheet->setCellValue("I{$line}", $_OPLCOMM);
			// Оформляем границы
			// 				$activeSheet->getStyle("A{$line}:I{$line}")->applyFromArray($_BORDER_INSIDE);
			$activeSheet->getStyle("A{$line}:I{$line}")->applyFromArray($_BORDER_BOTTOM_THIN);
			// ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
			// Суммируем счета по этапу
			$_SUM_ONCHF += $_OPLSUMMA;
			// Следующая строка
			$line++;
		}
		// Суммируем всего
		$_SUM_ONSTAGE += $_SUM_ONCHF;
	}
	// Суммируем всего
...
$activeSheet->setCellValue("A{$line}", "ИТОГО ПОЛУЧЕНО ОПЛАТ: ");
