...
$properties->setTitle('Контрактная база за период');
$properties->setDescription('Контрактная база за период');
...
$properties->setSubject('Контрактная база за период');
$properties->setKeywords('Портал, Договор, Отчеты');
#
#
include ($_SERVER['DOCUMENT_ROOT']."/dognet/php/examples/simple/report/report-details/restr_4/reports/bankform/form2/export/_xlsx/export2xlsx_bankform2.php");
#
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
/*
...
$filename = "SPRAVKA-BANKFORM2_".$_SESSION['id']."_".date('YmdHis').".XLSX";
$xmlWriter->save($filepath.$filename);

// Делаем запись в системный лог
// Все параметры в таблице portal_log_messages
	PORTAL_SYSLOG('99942200', '0000001', null, $_GET['reportview'], "Контрактная база за период", "EXCEL");
...
function countDown(){
	//время в сек.
		...
		timer = setInterval(function(){
			...
			url2 = 'http://'+host+'/dognet/dognet-report.php?reportview=bankform2&export=yes';
			if (i < 0) {
				clearInterval(timer);
