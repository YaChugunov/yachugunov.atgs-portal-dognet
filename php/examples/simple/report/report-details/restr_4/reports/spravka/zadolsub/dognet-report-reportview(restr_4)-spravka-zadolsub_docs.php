	...
	$(document).ready(function() {
		...
		table_reports_spravka_zadolsub_docs = $('#reports-spravka-zadolsub-docs-table').DataTable({
			...
			columns: [{
				...
				{
					orderable: false,
					data: "dognet_reports_zadolsub_chfincoming.date_incoming",
					className: "text-left"
				},
				{
				...
				{
					data: "dognet_reports_zadolsub_chfincoming.type_incoming",
					className: "text-left"
				},
				{
					orderable: false,
					data: "dognet_reports_zadolsub_chfincoming.sum_incoming",
					className: "text-left"
				},
				{
			...
			rowGroup: {
				startRender: function(rows, group, level) {
					...
					if (level == 1) {
						...
						return '<span style="text-align:left; white-space:nowrap; margin-left:10px">Основной договор № 3-4/' + docnumber + '&nbsp;:&nbsp;' + group + '</span>';
					}
					if (level == 2) {
						var namestage = rows.data().pluck("dognet_dockalplan").pluck("nameshotstage")[0];
						if (group) {
							return '<span style="text-align:left; white-space:nowrap; margin-left:20px">Этап ' + group + ' : ' + namestage + '</span>';
						} else {
							return '<span style="text-align:left; white-space:nowrap; margin-left:20px">Этап не определен либо отсутствует</span>';
						}
					}
					if (level == 3) {

						var datedocsubpodr = rows.data().pluck("dognet_docsubpodr").pluck("datedocsubpodr")[0];
						var sumdocsubpodr = rows.data().pluck("dognet_docsubpodr").pluck("sumdocsubpodr")[0];
						if (group) {
							return '<table width="100%"><tbody><tr><td width="40%">Договор субподряда № ' + group + '</td><td width="20%">' + datedocsubpodr + '</td><td width="20%">' + $.fn.dataTable.render.number(' ', ',', 2, '').display(sumdocsubpodr) + '' + koddened + '</td><td width="20%"></td></tr></tbody></table>';
						} else {
							return '<table width="100%"><tbody><tr><td colspan="4">Договор субподряда не определен либо отсутствует</td></tr></tbody></table>';
						}
					}
					if (level == 4) {
						var numberchfsubpodr = rows.data().pluck("dognet_docchfsubpodr").pluck("numberchfsubpodr")[0];
						var datechfsubpodr = rows.data().pluck("dognet_docchfsubpodr").pluck("datechfsubpodr")[0];
						var sumchfsubpodr = rows.data().pluck("dognet_docchfsubpodr").pluck("sumchfsubpodr")[0];
						var sumzadolchfsubpodr = rows.data().pluck("dognet_docchfsubpodr").pluck("sumzadolchfsubpodr")[0];
						if (group) {
							return '<table width="100%"><tbody><tr><td width="40%">Счет-фактура # ' + numberchfsubpodr + '</td><td width="20%">' + datechfsubpodr + '</td><td width="20%"><span id="chf-' + rows.data().pluck("dognet_docchfsubpodr").pluck("kodchfsubpodr")[0] + '">' + $.fn.dataTable.render.number(' ', ',', 2, '').display(sumchfsubpodr) + '' + koddened + '</span></td><td width="20%"></td></tr></tbody></table>';
						} else {
							return '<span style="text-align:left; white-space:nowrap; margin-left:40px">Номер счета не указан</span>';
						}
					}
				},
				endRender: function(rows, group, level) {
					...
					if (level == 0) {
						...
						for (let i = 0; i < rows.count(); i++) { // выведет 0, затем 1, затем 2
							var flag_1 = rows.data().pluck("dognet_reports_zadolsub_chfincoming").pluck("flag_1")[i];
							sm = Number(sm) + Number(flag_1) * Number(rows.data().pluck("dognet_docchfsubpodr").pluck("sumchfsubpodr")[i]);
						}
						var itogo0 = rows.data().pluck("dognet_reports_zadolsub_chfincoming").pluck("sum_incoming")
							.reduce(function(a, b) {
								return a + b * 1;
							}, 0);
						return '<span class="itogo-lvl0"><span class="itogo-lvl0-title">Итого по субподрядчику</span><span class="itogo-lvl0-text"><span class="itogo-lvl-lbltext1">' + $.fn.dataTable.render.number(' ', ',', 2, '').display(sm) + '' + koddened + '</span><span class="glyphicon glyphicon-option-vertical"></span><span class="itogo-lvl-lbltext2">' + $.fn.dataTable.render.number(' ', ',', 2, '').display(itogo0) + '' + koddened + '</span><span class="glyphicon glyphicon-option-vertical"></span><span class="itogo-lvl-lbltext3">' + $.fn.dataTable.render.number(' ', ',', 2, '').display(sm - itogo0) + '' + koddened + '</span></span></span>';
					...
					if (level == 3) {
						...
						var itogo3 = rows.data().pluck("dognet_reports_zadolsub_chfincoming").pluck("sum_incoming")
							.reduce(function(a, b) {
								return a + b * 1;
							}, 0);

						console.log('flag_1: ' + flag_1);
						return '<span class="itogo-lvl3"><span class="itogo-lvl3-title">Итого по договору субподряда</span><span class="itogo-lvl3-text"><span class="itogo-lvl-lbltext1">' + $.fn.dataTable.render.number(' ', ',', 2, '').display(sm) + '' + koddened + '</span><span class="glyphicon glyphicon-option-vertical"></span><span class="itogo-lvl-lbltext2">' + $.fn.dataTable.render.number(' ', ',', 2, '').display(itogo3) + '' + koddened + '</span><span class="glyphicon glyphicon-option-vertical"></span><span class="itogo-lvl-lbltext3">' + $.fn.dataTable.render.number(' ', ',', 2, '').display(sm - itogo3) + '' + koddened + '</span></span></span>';
					...
					if (level == 4) {
						...
						var sumchfsubpodr0 = rows.data().pluck("dognet_docchfsubpodr").pluck("sumchfsubpodr")[0];
						var type = rows.data().pluck("dognet_reports_zadolsub_chfincoming").pluck("type_incoming")[0];
						var itogo4 = rows.data().pluck("dognet_reports_zadolsub_chfincoming").pluck("sum_incoming")
							.reduce(function(a, b) {
								return a + b * 1;
							}, 0);
						return '<span class="itogo-lvl3"><span class="itogo-lvl3-title">Итого по счету</span><span class="itogo-lvl3-text"><span class="itogo-lvl-lbltext2">' + $.fn.dataTable.render.number(' ', ',', 2, '').display(itogo4) + '' + koddened + '</span><span class="glyphicon glyphicon-option-vertical"></span><span class="itogo-lvl-lbltext3">' + $.fn.dataTable.render.number(' ', ',', 2, '').display(sumchfsubpodr0 - itogo4) + '' + koddened + '</span></span></span>';
				...
				dataSrc: ["dognet_spsubpodr.namesubpodrshot", "dognet_docbase.docnameshot", "dognet_dockalplan.numberstage", "dognet_docsubpodr.numberdocsubpodr", "dognet_docchfsubpodr.kodchfsubpodr", "dognet_reports_zadolsub_chfincoming.type_incoming"],
			...
			buttons: [{
				...
				action: function(e, dt, node, config) {
					...
					table_reports_spravka_zadolsub_docs.order([1, "asc"], [2, "asc"], [4, "asc"]).draw();
				}
			}],
				...
				<div id="reports-spravka-zadolsub-docs-filters" class="panel-body space30">
					...
					<div class="col-xs-12 col-sm-6 col-md-2 col-lg-2">
						<div class="form-group space10" style="width:100%">
							...
							<input type="text" id="reportDocNumberSearch_text" class="form-control" placeholder="####" name="reportDocNumberSearch_text">
						</div>
					</div>
					<div class="col-xs-12 col-sm-6 col-md-2 col-lg-2">
						<div class="form-group space10" style="width:100%">
							<label for="reportSubdocNumberSearch_text"><b>Субподряд № :</b></label>
							<input type="text" id="reportSubdocNumberSearch_text" class="form-control" placeholder="####" name="reportSubdocNumberSearch_text">
						</div>
					</div>
					<div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
						<div class="input-group space10" style="width:100%">
							<label for="reportSubpodrSearch_text"><b>Субподрядчик :</b></label>
							<input type="text" id="reportSubpodrSearch_text" class="form-control" placeholder="Все объекты" name="reportSubpodrSearch_text">
						</div>
					</div>
					<!--
...
-->
					...
					<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 text-right">
						<div class="input-group-btn">
							<button id="columnSearch_btnApply1" class="btn btn-default" type="button">Применить фильтры</button>
							<button id="columnSearch_btnClear1" class="btn btn-default" type="button"><i class="glyphicon glyphicon-remove"></i></button>
							<button id="columnSearch_btnRefresh1" class="btn btn-default" type="button"><i class="glyphicon glyphicon-refresh"></i></button>
						</div>
					</div>
					<?php // ----- ----- ----- ----- ----- 
					?>
