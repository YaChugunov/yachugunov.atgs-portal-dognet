...
SELECT kodcontragent, nameshort
FROM sp_contragents
WHERE koddel<>'99' AND kodcontragent IN 
...
while ($_ROW_spzakaz = mysqli_fetch_assoc($_QRY_spzakaz)) {
	...
	(SELECT koddoc FROM dognet_docavans a WHERE a.dateavans <= '" . $on_date . "' AND a.summaavans > (SELECT coalesce(SUM(summaoplav), 0) FROM dognet_chfavans b WHERE b.dateoplav <= '" . $on_date . "' AND b.kodavans = a.kodavans)) AND kodzakaz = '" . $_ROW_spzakaz['kodcontragent'] . "' AND koddel <> '99') 
	OR koddoc IN 
	(SELECT koddoc FROM dognet_docavans a WHERE a.dateavans <= '" . $on_date . "' AND a.summaavans > (SELECT coalesce(SUM(summaoplav), 0) FROM dognet_chfavans b WHERE b.dateoplav <= '" . $on_date . "' AND b.kodavans = a.kodavans) AND kodzakaz = '" . $_ROW_spzakaz['kodcontragent'] . "' AND koddel <> '99')
	");
	//
		...
		if ($_ROW_docbase['kodshab'] == 1 || $_ROW_docbase['kodshab'] == 3) {
			...
			");
				...
				while ($_ROW_docavans = mysqli_fetch_assoc($_QRY_docavans)) {
					...
					$SUMOPLAV = $db_handle->runQuery("SELECT SUM(summaoplav) as sumoplav FROM dognet_chfavans WHERE kodavans = '" . $_ROW_docavans['kodavans'] . "' AND koddel<>'99'");
