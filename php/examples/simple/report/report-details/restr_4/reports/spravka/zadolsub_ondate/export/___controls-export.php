        ...
        <div class="col-xs-12 col-sm-12 col-md-8 col-md-offset-2 col-lg-8 col-lg-offset-2">
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 space20">
                <div id="report-settings" class="text-center">
                    <h3 class="space10">Справка о задолженности по субподрядчикам на дату</h3>
                    <div id="export-format-block" class="center-block" style="">
                        <form class="form-inline" method="GET" action="/dognet/dognet-report.php">
                            <input type="hidden" name="reportview" value="zadolsub_ondate">
                            ...
                            <div id="report-info" class="">
                                ...
                                <ol>
                                    <li>Учитываются только счета-фактуры с датой ранее или равной выбранной и имеющие
                                        больше нулевой
                                        рассчитанную
                                        задолженность на дату ранее или равной выбранной.</li>
                                    <li>В расчет берутся только полностью зачтенные авансы с датой ранее или равной
                                        выбранной в зачет
                                        выбранных счетов-фактур (п.1).</li>
                                    <li>В расчет берутся также частично зачтенные авансы (сплиты), но только с датой
                                        ранее или равной
                                        выбранной в зачет выбранных счетов-фактур (п.1).</li>
                                    <li>В расчет берутся оплаты с датой ранее или равной выбранной выбранных
                                        счетов-фактур (п.1).</li>
                                    <li>Задолженность по счету-фактуре рассчитывается как разность между суммой
                                        счета-фактуры и суммой
                                        всех
                                        поступлений в его оплату (зачтенные авансы, зачтенные сплиты, оплаты).</li>
                                    <li>Все суммы, приходы и задолженности по каждому счету-фактуре циклически
                                        суммируются в рамках
                                        договора
                                        субподряда,
                                        этапа (если есть), основного договора (если есть), субподрядчика, и затем
                                        формируется итоговая сумма
                                        по всем субподрядным организациям.</li>
                                    <li><span class="text-danger">*Вкладка с общей сводкой по субподрядчикам из экспорта
                                            пока убрана.
                                            Методика расчета требует обновления и проверки. Если она действительно
                                            необходима, то ее можно
                                            будет
                                            вернуть.</span></li>
                                    <li>Методика расчета задолженности была значительно переработана. Был также добавлен
                                        альтернативный
                                        вариант подсчета задолженности. Он происходит в фоне и сверяется с основным для
                                        дополнительной
                                        проверки. Данная методика будет постепенно перенесена на все другие отчеты
                                        задолженностей с учетом
                                        даты.</li>
                                </ol>
                            ...
                            <div>
                                <button class="btn btn-export" type="submit" name="done"
...
if (isset($_GET['format'])) {
    switch ($_GET['format']) {
    case "xls":
        include $_SERVER['DOCUMENT_ROOT'] . "/dognet/php/examples/simple/report/report-details/restr_4/reports/spravka/zadolsub_ondate/export/_xlsx/export2xlsx.php";
        break;
    default:
