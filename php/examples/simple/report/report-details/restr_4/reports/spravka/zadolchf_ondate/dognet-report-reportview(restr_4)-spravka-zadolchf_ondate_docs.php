<script type="text/javascript" language="javascript" class="init">
	...
	$(document).ready(function() {
		...
		table_reports_spravka_zadolchf_docs = $('#reports-spravka-zadolchf-docs-table').DataTable({
			...
			ajax: {
				url: "php/examples/simple/report/report-details/restr_4/reports/spravka/zadolchf_ondate/process/dognet-report-reportview(restr_4)-spravka-zadolchf_ondate-docs-process.php",
				type: "POST"
			},
			serverSide: true,
			columns: [{
					...
					render: function(data, type, row, meta) {
						if (!isNaN(data)) {
							if (row.dognet_dockalplan.idsrokopl == 3) {
								...
								var par2 = moment(row.dognet_reports_zadolchf_ondate.chetfdate, 'DD.MM.YYYY').format('YYYY-MM-DD');
								var par3 = row.dognet_reports_zadolchf_ondate.kodchfact;
								var par4 = data;
			...
			rowGroup: {
				startRender: function(rows, group, level) {
					...
					if (level == 1) {
						...
						return '<div style="text-align:left; margin-left:10px">Договор 3-4/' + docnumber + '&nbsp;:&nbsp;' + group + '</div>';
					}

					if (level == 2) {
						var namestage = rows.data().pluck("dognet_dockalplan").pluck("nameshotstage")[0];
						if (group) {
							return '<div style="text-align:left; margin-left:20px">Этап ' + group + ' : ' + namestage + '</div>';
						} else {
