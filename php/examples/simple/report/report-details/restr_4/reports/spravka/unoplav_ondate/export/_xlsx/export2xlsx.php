...
include($_SERVER['DOCUMENT_ROOT'] . "/dognet/php/examples/simple/report/report-details/funcReport.unoplav.inc.php");
...
$properties->setTitle('Справка о незакрытых авансах на дату');
$properties->setDescription('Справка о незакрытых авансах на дату');
...
$properties->setSubject('Справка о незакрытых авансах на дату');
...
#
#
include($_SERVER['DOCUMENT_ROOT'] . "/dognet/php/examples/simple/report/report-details/restr_4/reports/spravka/unoplav_ondate/export/_xlsx/export2xlsx_unoplav_ondate.php");
#
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
/*
...
$filename = "SPRAVKA-AVANS_ONDATE-" . date('YmdHis') . ".XLSX";
$xmlWriter->save($filepath . $filename);

// Делаем запись в системный лог
// Все параметры в таблице portal_log_messages
PORTAL_SYSLOG('99942200', '0000001', null, $_GET['reportview'], "Справка о незакрытых авансах на дату", "EXCEL");
...
<script type="text/javascript">
  ...
  function countDown() {
    ...
    timer = setInterval(function() {
      ...
      url2 = 'http://' + host + '/dognet/dognet-report.php?reportview=unoplav_ondate&export=yes';
      if (i < 0) {
        clearInterval(timer);
