...
*/
#
$_queryStr = "SELECT * FROM sp_contragents WHERE kodcontragent IN ( SELECT kodsubpodr FROM dognet_docsubpodr WHERE koddel<>'99' AND koddocsubpodr IN (SELECT koddocsubpodr FROM dognet_docchfsubpodr WHERE koddel<>'99' AND datechfsubpodr<='{$_ONDATE}') ) ORDER BY nameshort ASC";
...
while ($_ROW = mysqli_fetch_assoc($_QRY)) {
	$_kodsubpodr = $_ROW['kodcontragent'];
	...
	if (F_CHECK_SUB_ZADOL($_kodsubpodr, $_ONDATE) == '1') {
		...
		# ::::: LVL 2
		#
		# ЗАПРОС : Определяем набор основных договоров для которых имеются этапы (koddoc IN dognet_dockalplan), для выполнения которых заключались договора-субподряда (kodkalplan IN dognet_docsubpodr) с организациями выбранными в запросе LVL 1 (kodsubpodr='".$_ROW['kodcontragent']), по которым имеются задолженности (sumzadolsubpodr<>'0.00')
		#
		$_MAINDOC_EXIST = 0;
		...
		if ($_MAINDOC_EXIST == 1) {
			...
			while ($_ROW_docbase = mysqli_fetch_assoc($_QRY_docbase)) {
				...
				if (F_CHECK_DOC_ZADOL($_ROW_docbase['koddoc'], $_ROW_docbase['kodshab'], $_ONDATE) == '1') {
								...
								if (F_CHECK_SUBDOC_ZADOL($_koddocsubpodr, $_ONDATE) == '1') {
									...
									while ($_ROW_docchfsubpodr = mysqli_fetch_assoc($_QRY_docchfsubpodr)) {
										...
										if (F_CHECK_CHF_ZADOL($selectedKodchf, $_ONDATE) == '1') {
											...
											$activeSheet->setCellValue("G{$line}", $_ROW_docchfsubpodr['sumchfsubpodr']);
											...
											*/
												...
												while ($_ROW_OPLCHF = mysqli_fetch_assoc($_QRY_OPLCHF)) {
													...
													$activeSheet->setCellValue("H{$line}", $_ROW_OPLCHF['sumoplchfsubpodr']);
											...
											*/
												...
												while ($_ROW_AVCHF = mysqli_fetch_assoc($_QRY_AVCHF)) {
													...
													$activeSheet->setCellValue("G{$line}", '');
													$activeSheet->setCellValue("H{$line}", $_ROW_AVCHF['sumavsubpodr']);
													$activeSheet->getStyle("H{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
													$activeSheet->setCellValue("I{$line}", '');
													# Оформляем границы
													$activeSheet->getStyle("A{$line}:I{$line}")->applyFromArray($_BORDER_BOTTOM_THIN);
											...
											*/
												...
												while ($_ROW_AVSPLITCHF = mysqli_fetch_assoc($_QRY_AVSPLITCHF)) {
													...
													$activeSheet->setCellValue("G{$line}", '');
													$activeSheet->setCellValue("H{$line}", $_ROW_AVSPLITCHF['sumavsplit']);
													$activeSheet->getStyle("H{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
													$activeSheet->setCellValue("I{$line}", '');
													# Оформляем границы
													$activeSheet->getStyle("A{$line}:I{$line}")->applyFromArray($_BORDER_BOTTOM_THIN);
											...
											$activeSheet->setCellValue("G{$line}", $_SUM_CHF);
											$activeSheet->getStyle("G{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
											$activeSheet->setCellValue("H{$line}", $_SUM_CHF_INCOMING);
											$activeSheet->getStyle("H{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
											$activeSheet->setCellValue("I{$line}", $_ZADOL_CHF);
									...
									$activeSheet->setCellValue("G{$line}", $_SUM_SUBDOC);
									$activeSheet->getStyle("G{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
									$activeSheet->setCellValue("H{$line}", $_SUM_SUBDOC_INCOMING);
									$activeSheet->getStyle("H{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
									$activeSheet->setCellValue("I{$line}", $_ZADOL_SUBDOC);
									$activeSheet->getStyle("I{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
									// Оформляем границы
									$activeSheet->getStyle("A{$line}:I{$line}")->applyFromArray($_BORDER_BOTTOM_THIN);
					...
					$activeSheet->setCellValue("G{$line}", $_SUM_DOC);
					$activeSheet->getStyle("G{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
					$activeSheet->setCellValue("H{$line}", $_SUM_DOC_INCOMING);
					$activeSheet->getStyle("H{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
					$activeSheet->setCellValue("I{$line}", $_ZADOL_DOC);
					$activeSheet->getStyle("I{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
					// Оформляем границы
					$activeSheet->getStyle("A{$line}:I{$line}")->applyFromArray($_BORDER_BOTTOM_THIN);
					$activeSheet->getStyle("A{$line}:I{$line}")->applyFromArray($_BORDER_INSIDE);
				...
				if (F_CHECK_SUBDOC_ZADOL($_koddocsubpodr, $_ONDATE) == '1') {
					...
					while ($_ROW_docchfsubpodr = mysqli_fetch_assoc($_QRY_docchfsubpodr)) {
						...
						if (F_CHECK_CHF_ZADOL($selectedKodchf, $_ONDATE) == '1') {
							...
							$activeSheet->setCellValue("I{$line}", F_CALC_CHF_ZADOL($_ROW_docchfsubpodr['kodchfsubpodr'], $_ONDATE));
							$activeSheet->getStyle("I{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
							// Оформляем границы
							$activeSheet->getStyle("A{$line}:I{$line}")->applyFromArray($_BORDER_BOTTOM_THIN);
							$activeSheet->getStyle("D{$line}:I{$line}")->applyFromArray($_BORDER_INSIDE);
		...
		$activeSheet->setCellValue("G{$line}", $_SUM_SUB);
		$activeSheet->getStyle("G{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
		$activeSheet->setCellValue("H{$line}", $_SUM_SUB_INCOMING);
		$activeSheet->getStyle("H{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
		$activeSheet->setCellValue("I{$line}", $_ZADOL_SUB);
		$activeSheet->getStyle("I{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
		// Оформляем границы
		$activeSheet->getStyle("A{$line}:I{$line}")->applyFromArray($_BORDER_TOP);
		$activeSheet->getStyle("A{$line}:I{$line}")->applyFromArray($_BORDER_INSIDE);
...
$activeSheet->setCellValue("G{$line}", $_ITOGO_SUM_SUB);
$activeSheet->getStyle("G{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
$activeSheet->setCellValue("H{$line}", $_ITOGO_SUM_SUB_INCOMING);
$activeSheet->getStyle("H{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
$activeSheet->setCellValue("I{$line}", $_ITOGO_ZADOL_SUB);
$activeSheet->getStyle("I{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
// Оформляем границы
$activeSheet->getStyle("A{$line}:I{$line}")->applyFromArray($_BORDER_TOP);
$activeSheet->getStyle("A{$line}:I{$line}")->applyFromArray($_BORDER_INSIDE);
