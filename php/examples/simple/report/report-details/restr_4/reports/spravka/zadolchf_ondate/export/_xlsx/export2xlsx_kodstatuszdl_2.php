...
$activeSheet->setCellValue("A{$line}", 'Судебная задолженность на ' . date('d.m.Y', strtotime($_ONDATE)));
...
//
$_queryStr .= "koddoc IN (SELECT koddoc FROM dognet_reports_zadolchf_ondate WHERE kodstatuszdl='2')) ORDER BY nameshort ASC";
//
...
while ($_ROW = mysqli_fetch_assoc($_QRY)) {
    ...
    //
    $_queryStr .= "koddoc IN (SELECT koddoc FROM dognet_reports_zadolchf_ondate WHERE kodstatuszdl='2')";
    //
