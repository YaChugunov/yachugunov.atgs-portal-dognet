...
#
// Название листа
$activeSheet->setTitle('Невозвратная');
#
// Колонтитулы
...
// Задаем свой формат
define("PRICE_FORMAT_3", PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1 . "[\$ р.-419]");
...
$activeSheet->setCellValue("A{$line}", 'Невозвратная задолженность на ' . date('d.m.Y', strtotime($_ONDATE)));
...
//
$_queryStr .= "koddoc IN (SELECT koddoc FROM dognet_reports_zadolchf_ondate WHERE kodstatuszdl='3')) ORDER BY nameshort ASC";
//
...
while ($_ROW = mysqli_fetch_assoc($_QRY)) {
    ...
    $activeSheet->mergeCells("A{$line}:H{$line}");
    // Вывод данных
    $activeSheet->setCellValue("A{$line}", $_ROW['nameshort']);
    // Оформляем границы
    ...
    //
    $_queryStr .= "koddoc IN (SELECT koddoc FROM dognet_reports_zadolchf_ondate WHERE kodstatuszdl='3')";
    //
    ...
    while ($_ROW_docbase = mysqli_fetch_assoc($_QRY_docbase)) {
        ...
        while ($_ROW_dockalplan = mysqli_fetch_assoc($_QRY_dockalplan)) {
            ...
            while ($_ROW_kalplanchf = mysqli_fetch_assoc($_QRY_kalplanchf)) {
                ...
                $activeSheet->getStyle("D{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_3);
                $activeSheet->setCellValue("E{$line}", $summaoplav);
                $activeSheet->getStyle("E{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_3);
                $activeSheet->setCellValue("F{$line}", $summaopl);
                $activeSheet->getStyle("F{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_3);
                $activeSheet->setCellValue("G{$line}", $summazadol);
                $activeSheet->getStyle("G{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_3);
        ...
        $activeSheet->getStyle("H{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_3);
        // Выравнивание по горизонтали - центр
    ...
    $activeSheet->mergeCells("A{$line}:G{$line}");
    // Вывод данных
    $activeSheet->setCellValue("A{$line}", 'Итого по заказчику (' . $_ROW["nameshort"] . ')');
