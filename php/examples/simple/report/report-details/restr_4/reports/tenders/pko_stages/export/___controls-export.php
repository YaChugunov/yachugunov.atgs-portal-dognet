		...
		<div class="col-xs-12 col-sm-12 col-md-8 col-md-offset-2 col-lg-8 col-lg-offset-2">
			<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 space20">
				<div id="report-settings" class="text-center">
					<h3 class="space10">Сведения об опыте выполнения работ по предмету ПКО<br>(с разбивкой по спецификациям)<br><span style="text-decoration:underline">за последние три года</span></h4>
			...
			<div class="col-xs-hidden col-sm-12 col-md-12 col-lg-12">
				<div id="export-format-block" class="center-block" style="">
					<form class="form-horizontal" method="GET" action="/dognet/dognet-report.php">
						<div id="docsearch-filters-block" class="">
							    <div id="docsearch-filters" class="">
									<div id="docview-search-filters" class="row">
										<div class="col-xs-12 col-sm-12 col-md-8 col-md-offset-2 col-lg-8 col-lg-offset-2 space30">
											<div class="checkbox text-left">
												<label><input id="chk_allDocs" type="checkbox" name="chk_allDocs" value="yes"><b>Все</b> / Все договора</label>
											</div>
											<div class="checkbox text-left">
												<label><input id="chk_currentDocs" type="checkbox" name="chk_currentDocs" value="yes"><b>Текущие</b> / Все текущие договора с любыми статусами, <u>кроме</u> "закрыт", "закрыт оригинала нет", "не использовать", "не состоялся", "для заказов"</label>
											</div>
											<div class="checkbox text-left">
												<label><input id="chk_closedDocs" type="checkbox" name="chk_closedDocs" value="yes"><b>Закрытые</b> / Договора со статусами "закрыт" и "закрыт, оригинала нет", все или с выбранными годами закрытия</label>
											</div>
										</div>
										<div id="divYearSelector" class="col-xs-12 col-sm-12 col-md-4 col-md-offset-4 col-lg-4 col-lg-offset-4" style="display:none">
											<div class="form-group space10" style="width:100%">
												<label for="YearSelector1" class="text-uppercase"><b>Год закрытия договора</b></label>
											...
											<div class="form-group space10" style="width:100%">
												...
												if (isset($_POST['YearSelector'])&&!empty($_POST['YearSelector'])) {
													for ($i=0;$i<count($_arrYear1);$i++) {
														...
														if ($selected!="") { echo "<option value='$_arrYear1[$i]' $selected>$_arrYear1[$i]</option>";	}
													}
												}
												?>
												</select>
										...
										<div id="divStatusSelector" class="col-xs-12 col-sm-12 col-md-6 col-md-offset-3 col-lg-6 col-lg-offset-3" style="display:none">
											<div class="form-group space10" style="width:100%">
												...
												if (isset($_POST['StatusSelector'])&&!empty($_POST['StatusSelector'])) {
													for ($i=0;$i<count($_arrStatus1);$i++) {
														...
														if ($selected!="") { echo "<option value='$_arrStatus1[$i]' $selected>$_arrStatus2[$i]</option>";	}
													}
												}
												?>
												</select>
						...
						<input type="hidden" name="reportview" value="pko_stages">
...
<?php
				if (isset($_GET['format'])) {
					switch ($_GET['format']) {
						case "xls":
							include($_SERVER['DOCUMENT_ROOT']."/dognet/php/examples/simple/report/report-details/restr_4/reports/tenders/pko_stages/export/_xlsx/export2xlsx.php");
							break;
						default:
...
$('#btn_removeFromList_YearSelector').click(function(){ $("#YearSelector option:selected").remove(); });
// Очистить список
$('#btn_clearList_YearSelector').click(function(){ $('#YearSelector').find('option').remove(); });
// ----- ----- ----- ----- -----
...
$('#chk_currentDocs').click(function(){
	if ($(this).is(':checked')) {
		...
		var list = [
			{'enbl': '1', 'kodstatus': '245267756667430', 'statusnameshot': 'ПОДПИСАНИЕ'},
			{'enbl': '1', 'kodstatus': '245287841353107', 'statusnameshot': 'ЗАВИС'},
			{'enbl': '0', 'kodstatus': '245287853877236', 'statusnameshot': 'ЗАКРЫТ'},
			{'enbl': '1', 'kodstatus': '245381842747296', 'statusnameshot': 'ТЕКУЩИЙ'},
			{'enbl': '1', 'kodstatus': '245381842145343', 'statusnameshot': 'ПРОЕКТ'},
			{'enbl': '0', 'kodstatus': '245316049282906', 'statusnameshot': 'НЕ ИСПОЛЬЗОВАТЬ'},
			{'enbl': '0', 'kodstatus': '245330847566343', 'statusnameshot': 'НЕ СОСТОЯЛСЯ'},
			{'enbl': '0', 'kodstatus': '245379861643048', 'statusnameshot': 'ДЛЯ ЗАКАЗОВ'},
			{'enbl': '1', 'kodstatus': '245494561881685', 'statusnameshot': 'ПРЕД-ПРОЕКТ'},
			{'enbl': '1', 'kodstatus': '245597345680479', 'statusnameshot': 'ЕСТЬ СКАН'},
			{'enbl': '0', 'kodstatus': '245600252070703', 'statusnameshot': 'ЗАКРЫТ, ОРИГИН. НЕТ'}
		];
		$('#StatusSelector option').remove();
		$('#YearSelector option').remove();
		for (i in list) {
			if (list[i].enbl=='1') {
				jQuery('#StatusSelector').append('<option value="'+list[i].kodstatus+'" selected>'+list[i].statusnameshot+'</option>');
			}
		}
	}
	else {
...
$('#chk_allDocs').click(function(){
	if ($(this).is(':checked')) {
		...
		var list = [
			{'enbl': '1', 'kodstatus': '245267756667430', 'statusnameshot': 'ПОДПИСАНИЕ'},
			{'enbl': '1', 'kodstatus': '245287841353107', 'statusnameshot': 'ЗАВИС'},
			{'enbl': '1', 'kodstatus': '245287853877236', 'statusnameshot': 'ЗАКРЫТ'},
			{'enbl': '1', 'kodstatus': '245381842747296', 'statusnameshot': 'ТЕКУЩИЙ'},
			{'enbl': '1', 'kodstatus': '245381842145343', 'statusnameshot': 'ПРОЕКТ'},
			{'enbl': '1', 'kodstatus': '245316049282906', 'statusnameshot': 'НЕ ИСПОЛЬЗОВАТЬ'},
			{'enbl': '1', 'kodstatus': '245330847566343', 'statusnameshot': 'НЕ СОСТОЯЛСЯ'},
			{'enbl': '1', 'kodstatus': '245379861643048', 'statusnameshot': 'ДЛЯ ЗАКАЗОВ'},
			{'enbl': '1', 'kodstatus': '245494561881685', 'statusnameshot': 'ПРЕД-ПРОЕКТ'},
			{'enbl': '1', 'kodstatus': '245597345680479', 'statusnameshot': 'ЕСТЬ СКАН'},
			{'enbl': '1', 'kodstatus': '245600252070703', 'statusnameshot': 'ЗАКРЫТ, ОРИГИН. НЕТ'}
		];
		$('#StatusSelector option').remove();
		$('#YearSelector option').remove();
		for (i in list) {
...
$('#chk_closedDocs').click(function(){
	if ($(this).is(':checked')) {
		...
		$('#chk_allDocs').prop('checked', false);
		var list = [
			{'enbl': '0', 'kodstatus': '245267756667430', 'statusnameshot': 'ПОДПИСАНИЕ'},
			{'enbl': '0', 'kodstatus': '245287841353107', 'statusnameshot': 'ЗАВИС'},
			{'enbl': '1', 'kodstatus': '245287853877236', 'statusnameshot': 'ЗАКРЫТ'},
			{'enbl': '0', 'kodstatus': '245381842747296', 'statusnameshot': 'ТЕКУЩИЙ'},
			{'enbl': '0', 'kodstatus': '245381842145343', 'statusnameshot': 'ПРОЕКТ'},
			{'enbl': '0', 'kodstatus': '245316049282906', 'statusnameshot': 'НЕ ИСПОЛЬЗОВАТЬ'},
			{'enbl': '0', 'kodstatus': '245330847566343', 'statusnameshot': 'НЕ СОСТОЯЛСЯ'},
			{'enbl': '0', 'kodstatus': '245379861643048', 'statusnameshot': 'ДЛЯ ЗАКАЗОВ'},
			{'enbl': '0', 'kodstatus': '245494561881685', 'statusnameshot': 'ПРЕД-ПРОЕКТ'},
			{'enbl': '0', 'kodstatus': '245597345680479', 'statusnameshot': 'ЕСТЬ СКАН'},
			{'enbl': '1', 'kodstatus': '245600252070703', 'statusnameshot': 'ЗАКРЫТ, ОРИГИН. НЕТ'}
		];
		$('#StatusSelector option').remove();
		$('#YearSelector option').remove();
