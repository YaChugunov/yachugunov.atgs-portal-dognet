...
#
#
$qryPart = "SELECT koddoc, kodshab, koddened, docnumber, docnameshot, docnamefull, docnamefullm, docsumma, yearnachdoc, monthnachdoc, daynachdoc, yearenddoc, monthenddoc, dayenddoc, docnamefullm, kodzakaz, kodobject, kodispol, kodtip, kodstatus, datezakr FROM dognet_docbase WHERE yearnachdoc >= YEAR( NOW() )-3 AND ";
...
while ($_ROW = mysqli_fetch_assoc($_QRY)) {
	...
	if ($_ROW['kodobject'] == "245530345307999") {
		...
		$DOCSTAGES_QRY = mysqlQuery("SELECT kodkalplan, numberstage, summastage FROM dognet_dockalplan WHERE koddoc = '" . $_ROW['koddoc'] . "'");
		while ($DOCSTAGES_ROW = mysqli_fetch_assoc($DOCSTAGES_QRY)) {
			...
			if ($_TMMQRY1) {
				if (!empty($_TMMQRY1['lastchetfdate'])) {
					$_DOCDATE_ZAK = date("d.m.Y", strtotime($_TMMQRY1['lastchetfdate'])) . "*";
					$_DOCYEAR_ZAK = date("Y", strtotime($_TMMQRY1['lastchetfdate'])) . "*";
				} else {
					$_DOCDATE_ZAK = "---";
					$_DOCYEAR_ZAK = "---";
				}
			...
			// Делаем заливку области ячеек
			$activeSheet->getStyle("A{$line}:K{$line}")->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
			$activeSheet->getStyle("A{$line}:K{$line}")->getFill()->getStartColor()->setRGB("E0F2F7");
			// 
			...
			$objRichText4->createTextRun("3-4/" . $_DOCNUM . " от " . $_DOCDATE_NACH . "\n" . $_DOCNAME . "\n" . $_DOCNAME . "\n\n");
			$objBold = $objRichText4->createTextRun("Спецификация № " . $_NUMBERSTAGE);
			$objBold->getFont()->setBold(true);
			$objBold->getFont()->setSize(11);
			$activeSheet->setCellValue("C{$line}", $objRichText4, PHPExcel_Cell_DataType::TYPE_STRING);
			// 
			...
			$activeSheet->setCellValue("E{$line}", $_SUMMASTAGE);
			$activeSheet->getStyle("E{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
			$activeSheet->setCellValue("F{$line}", ($_SUMMASTAGE - $STAGESUB_SUM));
