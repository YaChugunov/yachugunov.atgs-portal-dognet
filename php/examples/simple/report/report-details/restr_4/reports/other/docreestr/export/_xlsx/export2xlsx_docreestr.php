...
$activeSheet->getColumnDimension('C')->setWidth(50); // Источник финансирования
...
$activeSheet->getColumnDimension('S')->setWidth(15); // Сумма гарантии
$activeSheet->getColumnDimension('T')->setWidth(12); // Срок гарантии
$activeSheet->getColumnDimension('U')->setWidth(20); // Задолженность по кредиту
$activeSheet->getColumnDimension('V')->setWidth(12); // Срок кредита
$activeSheet->getColumnDimension('W')->setWidth(18); // Рентабельность
#
...
$activeSheet->setCellValue("K{$line}", 'ПОДПИСАНО' . "\n" . 'КС2, КС3');
...
$activeSheet->setCellValue("R{$line}", '*БАНК,' . "\n" . 'ОБСЛУЖИВАЮЩИЙ ДОГОВОР');
$activeSheet->setCellValue("S{$line}", '*СУММА' . "\n" . 'ГАРАНТИИ');
$activeSheet->setCellValue("T{$line}", '*СРОК' . "\n" . 'ГАРАНТИИ');
$activeSheet->setCellValue("U{$line}", '*ЗАДОЛЖЕННОСТЬ' . "\n" . 'ПО КРЕДИТУ');
$activeSheet->setCellValue("V{$line}", '*СРОК КРЕДИТА');
...
while ($_ROW_DOCBASE = mysqli_fetch_assoc($_QRY_DOCBASE)) {
	...
	$_QRY_ZAK = mysqlQuery("SELECT nameshort, namefull FROM sp_contragents WHERE kodcontragent='" . $_ROW_DOCBASE['kodzakaz'] . "'");
	$_ROW_ZAK = mysqli_fetch_assoc($_QRY_ZAK);
	$_colZAK = $_ROW_ZAK['nameshort'];
	#
	// Номер договора
