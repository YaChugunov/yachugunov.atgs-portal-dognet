...
$_SQL_dockalplan = "
	...
	dognet_dockalplan.kodkalplan as k2, dognet_dockalplan.kodobject as k3, dognet_dockalplan.nameshotstage as k10, dognet_dockalplan.numberstage as k11, dognet_dockalplan.idobjectready as k0,
	dognet_kalplanchf.kodkalplan as k7, dognet_kalplanchf.chetfdate as k8, dognet_kalplanchf.chetfnumber as k9, dognet_kalplanchf.kodchfact as kodchfact,
	dognet_docbase.koddoc as k12, dognet_docbase.docnameshot as k13, dognet_docbase.docnumber as k14, dognet_docbase.kodstatus as k15, dognet_docbase.kodobject as k16, dognet_docbase.kodzakaz as k17, dognet_docbase.kodtip as k18, dognet_docbase.daynachdoc as k19, dognet_docbase.monthnachdoc as k20, dognet_docbase.yearnachdoc as k21, dognet_docbase.kodtip as kodtip, dognet_docbase.kodstatus as kodstatus, dognet_docbase.docsumma as docsumma
    ...
    AND dognet_docbase.kodtip IN ('245287841599652','245287841608965'))
	OR
    (dognet_dockalplan_progress.kodkalplan IN (SELECT kodkalplan FROM dognet_kalplanchf WHERE YEAR(chetfdate)='" . $on_date . "' AND koddel!='99') AND
	dognet_dockalplan_progress.srokstage_date!='0000-00-00' AND dognet_dockalplan_progress.srokstage_date!='NULL' AND dognet_dockalplan_progress.srokstage_date!='' AND dognet_kalplanchf.chetfdate=(SELECT MAX(chetfdate) FROM dognet_kalplanchf WHERE kodkalplan=(SELECT MAX(kodkalplan) FROM dognet_dockalplan WHERE kodkalplan=dognet_dockalplan_progress.kodkalplan))
    AND dognet_docbase.kodtip NOT IN ('245287841599652','245287841608965')
    )
...
$_QRY_dockalplan = mysqlQuery("
	...
	LEFT JOIN dognet_docbase ON dognet_dockalplan_progress.koddoc = dognet_docbase.koddoc
	WHERE
	(dognet_dockalplan_progress.kodkalplan IN (SELECT kodkalplan FROM dognet_kalplanchf WHERE YEAR(chetfdate)='2023' AND koddel!='99') AND
	dognet_dockalplan_progress.srokstage_date!='0000-00-00' AND dognet_dockalplan_progress.srokstage_date!='NULL' AND dognet_dockalplan_progress.srokstage_date!=''
    AND dognet_docbase.kodtip IN ('245287841599652','245287841608965'))
	OR
    (dognet_dockalplan_progress.kodkalplan IN (SELECT kodkalplan FROM dognet_kalplanchf WHERE YEAR(chetfdate)='2023' AND chetfdate=(SELECT MAX(chetfdate) FROM dognet_kalplanchf) AND koddel!='99') AND
	dognet_dockalplan_progress.srokstage_date!='0000-00-00' AND dognet_dockalplan_progress.srokstage_date!='NULL' AND dognet_dockalplan_progress.srokstage_date!=''
    AND dognet_docbase.kodtip NOT IN ('245287841599652','245287841608965'))
    ORDER BY k14 DESC, k11 ASC
	");
...
while ($_ROW_dockalplan = mysqli_fetch_assoc($_QRY_dockalplan)) {
    ...
    $DOCZAK = $db_handle->runQuery("SELECT nameshort FROM sp_contragents WHERE kodcontragent = '" . $_ROW_dockalplan['k17'] . "'");
    // ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    ...
    #
    $koddoc = $_ROW_dockalplan['k12'];
    $docsumma = $_ROW_dockalplan['docsumma'];
    $kodstatus = $_ROW_dockalplan['kodstatus'];
    // Статус закрыт: 245287853877236
    $kodchfact = $_ROW_dockalplan['kodchfact'];
    //
    ...
    //
    $checkChfNY = (mysqli_num_rows($_reqChfNY) > 0) ? 1 : 0;
    #
    //** <<< UPDATE 17.09.2024

    if ($kodstatus = "245287853877236" && $checkChfNY == 0) {
        // ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
...
$activeSheet->setCellValue("A{$line}", $_SQL_dockalplan);
$line++;
