<?php
    ...
    $properties->setTitle('Отчет о договорах поставки с истёкшими сроками выполнения');
    $properties->setDescription('Отчет о договорах поставки с истёкшими сроками выполнения');
    ...
    $properties->setSubject('Отчет о договорах поставки с истёкшими сроками выполнения');
    ...
    if (checkIsItSuperadmin($_SESSION['id']) == 1) {
        include $_SERVER['DOCUMENT_ROOT'] . "/dognet/php/examples/simple/report/report-details/restr_4/reports/other/postexp/export/_xlsx/export2xlsx_postexp.v2.php";
    } else {
        include $_SERVER['DOCUMENT_ROOT'] . "/dognet/php/examples/simple/report/report-details/restr_4/reports/other/postexp/export/_xlsx/export2xlsx_postexp.v2.php";
    }
    if ($A1) {
    ...
    if ($A3) {
        ...
        //    include ($_SERVER['DOCUMENT_ROOT']."/dognet/php/examples/simple/report/report-details/restr_4/reports/spravka/zadolchf/export/_xlsx/export2xlsx_postexp.php");
    }
 ...
 */
    ...
    $filename = "SPRAVKA-DOCPOSTEXP_" . date('YmdHis') . ".XLSX";
    $xmlWriter->save($filepath . $filename);

    // Делаем запись в системный лог
    // Все параметры в таблице portal_log_messages
    // PORTAL_SYSLOG('99942200', '0000001', null, $_GET['reportview'], "Отчет о договорах поставки с истёкшими сроками выполнения", "EXCEL");
...
<div id="link">
    ...
    <div id="bottom-links"></div>
</div>

<script type="text/javascript">
host = '<?php echo $_SERVER["HTTP_HOST"]; ?>';
url1 = 'http://' + host + '/dognet/dognet-report.php';
url2 = 'http://' + host + '/dognet/dognet-report.php?reportview=postexp&export=yes';
document.getElementById("bottom-links").innerHTML =
