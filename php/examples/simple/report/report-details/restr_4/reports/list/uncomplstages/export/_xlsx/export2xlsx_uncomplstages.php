...
$activeSheet->getColumnDimension('L')->setWidth(20); // Не закрыто
#
...
$activeSheet->getStyle("G{$line}:L{$line}")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
//*
    if ($objectready === '1') {
        // Делаем заливку области ячеек
        $activeSheet->getStyle("A{$line}:L{$line}")->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
        $activeSheet->getStyle("A{$line}:L{$line}")->getFill()->getStartColor()->setRGB("FFF2CC");
    }
//* - - - - - - - - - -
...
$activeSheet->getStyle("A{$line}:L{$line}")->getFont()->setBold(true);
// Задаем цвет текста строки
$activeSheet->getStyle("A{$line}:L{$line}")->getFont()->getColor()->setRGB('111111');
// Объединяем ячейки по горизонтали
...
// Выравнивание по горизонтали - центр
$activeSheet->getStyle("J{$line}:L{$line}")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
