...
$_KODDOC = !empty($_GET['uniqueID']) ? $_GET['uniqueID'] : "";
...
#
// Название листа
$activeSheet->setTitle('Список счетов-фактур');
#
// Колонтитулы
$activeSheet->getHeaderFooter()->setOddHeader('&L&G&B&12СПИСОК СЧЕТОВ-ФАКТУР&R&G&B&12По выбранному договору');
...
$activeSheet->setCellValue("A{$line}", 'Список счетов-фактур по договору');
...
$start_table = $line;
// Шапка таблицы
$activeSheet->setCellValue("A{$line}", 'Договор / Этап');
...
$activeSheet->setCellValue("F{$line}", 'Оплачено');
...
#
#
$_queryStr = "SELECT * FROM dognet_docbase WHERE koddoc='{$_KODDOC}' AND koddel<>'99'";
...
while ($_ROW_docbase = mysqli_fetch_assoc($_QRY_docbase)) {
	...
	if ($_ROW_docbase['kodshab'] == 0) {
		$activeSheet->setCellValue("A{$line}", 'Счет №' . $_ROW_docbase['docnumber'] . " : " . $_ROW_docbase['docnameshot']);
	} else {
	...
	if (($_ROW_docbase['kodshab'] == 1) or ($_ROW_docbase['kodshab'] == 3)) {
		$_QRY_dockalplan_Str = "SELECT * FROM dognet_dockalplan WHERE koddel<>'99' AND koddoc='{$_KODDOC}'";
	}
	if (($_ROW_docbase['kodshab'] == 2) or ($_ROW_docbase['kodshab'] == 4)) {
		$_QRY_dockalplan_Str = "SELECT * FROM dognet_docbase WHERE koddel<>'99' AND numberchet='' AND koddoc='{$_KODDOC}' AND (kodshab='2' OR kodshab='4')";
	}
	if ($_ROW_docbase['kodshab'] == 0) {
		$_QRY_dockalplan_Str = "SELECT * FROM dognet_docbase WHERE koddel<>'99' AND numberchet<>'' AND koddoc='{$_KODDOC}' AND kodshab='0'";
	}
	#
	#
	...
	while ($_ROW_dockalplan = mysqli_fetch_assoc($_QRY_dockalplan)) {
		...
		#
		if (($_ROW_docbase['kodshab'] == 1) or ($_ROW_docbase['kodshab'] == 3)) {
			$_QRY_kalplanchf = mysqlQuery("SELECT dognet_kalplanchf.kodchfact, dognet_kalplanchf.chetfnumber, dognet_kalplanchf.chetfdate, dognet_kalplanchf.chetfsumma FROM dognet_kalplanchf WHERE dognet_kalplanchf.kodkalplan='{$_ROW_dockalplan['kodkalplan']}'");
		} elseif (($_ROW_docbase['kodshab'] == 0) or ($_ROW_docbase['kodshab'] == 2) or ($_ROW_docbase['kodshab'] == 4)) {
			$_QRY_kalplanchf = mysqlQuery("SELECT dognet_kalplanchf.kodchfact, dognet_kalplanchf.chetfnumber, dognet_kalplanchf.chetfdate, dognet_kalplanchf.chetfsumma FROM dognet_kalplanchf WHERE dognet_kalplanchf.kodkalplan='{$_ROW_docbase['koddoc']}'");
		}

		while ($_ROW_kalplanchf = mysqli_fetch_assoc($_QRY_kalplanchf)) {
			//
			$_CHFDATE = !empty($_ROW_kalplanchf['chetfdate']) ? $_ROW_kalplanchf['chetfdate'] : "";
			$_KODCHFACT = !empty($_ROW_kalplanchf['kodchfact']) ? $_ROW_kalplanchf['kodchfact'] : "";
			//
			$chetfsumma = $_ROW_kalplanchf['chetfsumma'];
			$summaoplav = !empty(DOCBASE_FN_SUM_AVANSCHF_CHETF($_KODCHFACT)) ? DOCBASE_FN_SUM_AVANSCHF_CHETF($_KODCHFACT) : 0.0;
			$summaopl = !empty(DOCBASE_FN_SUM_OPLATCHF_CHETF($_KODCHFACT)) ? DOCBASE_FN_SUM_OPLATCHF_CHETF($_KODCHFACT) : 0.0;
			$summazadol = $chetfsumma - ($summaoplav + $summaopl);
			...
			if (($_ROW_docbase['kodshab'] == 1) or ($_ROW_docbase['kodshab'] == 3)) {
				if ($_SROKOPL != "ПКЗ") {
					if ($_ROW_dockalplan['idsrokopl'] == 1) {
						...
						$srokopl = $nextDate->format($format);
					}
				} else {
					$srokopl = $_SROKOPL;
				}
			...
			$activeSheet->getStyle("D{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
	...
	$activeSheet->setCellValue("D{$line}", $_SUM_chetfsumma);
	$activeSheet->setCellValue("E{$line}", $_SUM_summaoplav);
	$activeSheet->setCellValue("F{$line}", $_SUM_summaopl);
	$activeSheet->setCellValue("G{$line}", $_SUM_summazadol_doc);
	$activeSheet->getStyle("D{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
	$activeSheet->getStyle("E{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
	$activeSheet->getStyle("F{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
	$activeSheet->getStyle("G{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
	// Выравнивание по горизонтали - центр
	$activeSheet->getStyle("D{$line}:G{$line}")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
	// Оформляем границы
	$activeSheet->getStyle("A{$line}:H{$line}")->applyFromArray($_BORDER_INSIDE);
	$activeSheet->getStyle("A{$line}:H{$line}")->applyFromArray($_BORDER_BOTTOM_THIN);
	#
