...
Editor::inst($db, 'dognet_dockalplan')
	...
	->on('preGet', function ($editor, $id) {
		$editor->where(function ($q) {
			$q->where(function ($r1) {
				$r1->where('dognet_docbase.kodstatus', '245381842747296'); // "Текущий"
				$r1->or_where('dognet_docbase.kodstatus', '245381842145343'); // "Проект"
				$r1->or_where('dognet_docbase.kodstatus', '245267756667430'); // "Подписание"
				$r1->or_where('dognet_docbase.kodstatus', '245597345680479'); // "Есть скан"
			});
		});
	})
	//
