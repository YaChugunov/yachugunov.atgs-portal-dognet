...
#
function SUM_SUBPODR_AVCHF($kod, $ondate) {
	// Считаем сумму сплитов по авансу без учета даты ( $ondate = NULL )
	if (!empty($kod) && empty($ondate)) {
		$_QRY = mysqli_fetch_assoc(mysqlQuery("SELECT SUM(sumavsplit) as sum FROM dognet_docavsplitsubpodr WHERE kodchfsubpodr='{$kod}' AND koddel<>'99'"));
		$result = $_QRY['sum'];
	}
	// Считаем сумму сплитов по авансу с учетом даты ( $ondate )
	elseif (!empty($kod) && !empty($ondate) && ((DateTime::createFromFormat('Y-m-d', $ondate) !== false))) {
		$_QRY = mysqli_fetch_assoc(mysqlQuery("SELECT SUM(sumavsplit) as sum FROM dognet_docavsplitsubpodr WHERE kodchfsubpodr='{$kod}' AND koddel<>'99' AND dateavsplit<='{$ondate}'"));
		$result = $_QRY['sum'];
	} else {
		$result = -1;
	}
...
# >>> $ondate	- Дата отсечки сплитов по дате (для справки о задолженности на дату). Если дата отсутствует (null), то суммируются ВСЕ сплиты
#
function SUM_SUBPODR_OPLCHF($kod, $ondate) {
	// Считаем сумму оплат без учета даты ( $ondate = NULL )
	if (!empty($kod) && empty($ondate)) {
		$_QRY = mysqli_fetch_assoc(mysqlQuery("SELECT SUM(sumoplchfsubpodr) as sum FROM dognet_docoplchfsubpodr WHERE kodchfsubpodr='{$kod}'"));
		$result = $_QRY['sum'];
	}
	// Считаем сумму сплитов по авансу с учетом даты ( $ondate )
	elseif (!empty($kod) && !empty($ondate) && ((DateTime::createFromFormat('Y-m-d', $ondate) !== false))) {
		$_QRY = mysqli_fetch_assoc(mysqlQuery("SELECT SUM(sumoplchfsubpodr) as sum FROM dognet_docoplchfsubpodr WHERE kodchfsubpodr='{$kod}' AND dateoplchfsubpodr<='{$ondate}'"));
		$result = $_QRY['sum'];
	} else {
		$result = -1;
	}
		...
		while ($_ROW_docbase = mysqli_fetch_assoc($_QRY_docbase)) {
			...
			*/
				...
				while ($_ROW_docsubpodr = mysqli_fetch_assoc($_QRY_docsubpodr)) {
					...
					#
					#
					$_QRY_docchfsubpodr_Str = "SELECT * FROM dognet_docchfsubpodr WHERE koddel<>'99' AND koddocsubpodr='" . $_ROW_docsubpodr['koddocsubpodr'] . "' AND sumzadolchfsubpodr<>'0.00' AND datechfsubpodr<='" . $_ONDATE . "'";
					...
					while ($_ROW_docchfsubpodr = mysqli_fetch_assoc($_QRY_docchfsubpodr)) {
						...
						#
						#
						$_QRY_chfincoming_Str = "SELECT * FROM dognet_reports_zadolsub_ondate_chfincoming WHERE kodchfsubpodr='" . $_ROW_docchfsubpodr['kodchfsubpodr'] . "' AND (type_incoming='1' OR type_incoming='2')";
						...
						if (mysqli_num_rows($_QRY_chfincoming) <= 0) {
							...
							while ($_ROW_chfincoming = mysqli_fetch_assoc($_QRY_chfincoming)) {
								...
								if ($_ROW_chfincoming['type_incoming'] == '1') {
									...
									$activeSheet->setCellValue("H{$line}", number_format($_ROW_chfincoming['sum_incoming'], 2, ',', ''));
									$activeSheet->setCellValue("I{$line}", '');
