...
$activeSheet->setCellValue("A{$line}", 'Текущая задолженность на ' . $_SPRAVKA_ONDATE);
...
//
$_queryStr .= "kodstatuszdl='1' AND koddoc IN (SELECT koddoc FROM dognet_reports_zadolchf)) ORDER BY nameshort ASC";
//
...
while ($_ROW = mysqli_fetch_assoc($_QRY)) {
    ...
    //
    $_queryStr .= "kodstatuszdl='1' AND koddoc IN (SELECT koddoc FROM dognet_reports_zadolchf)";
    //
