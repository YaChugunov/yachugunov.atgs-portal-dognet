...
$activeSheet->setCellValue("A{$line}", 'Текущая задолженность');
...
//
$_queryStr .= "kodstatuszdl='1' AND koddoc IN (SELECT koddoc FROM dognet_reports_zadolchf)) ORDER BY namezakshot ASC";
//
				...
				if (($_ROW_docbase['kodshab']==1)OR($_ROW_docbase['kodshab']==3)) {
					if ($_SROKOPL != "ПКЗ") {
						if ($_ROW_dockalplan['idsrokopl']==1) {
							...
							$srokopl = $date->format($format)." id=1";
						}
						elseif ($_ROW_dockalplan['idsrokopl']==2) {
							$srokopl = str_replace('/', '.', $_SROKOPL)." id=2";
						}
						elseif ($_ROW_dockalplan['idsrokopl']==3) {
							...
							$srokopl = $nextDate->format($format)." id=3";
						}
					}
					else {
						$srokopl = $_SROKOPL;
