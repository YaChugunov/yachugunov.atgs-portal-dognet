        ...
        rowGroup: {
            startRender: function(rows, group, level) {
                ...
                if (level == 1) {
                    ...
                    return '<span style="text-align:left; white-space:nowrap; margin-left:10px">Основной договор № 3-4/' +
                ...
                if (level == 2) {
                    ...
                    0];
                    if (group) {
                        return '<span style="text-align:left; white-space:nowrap; margin-left:20px">Этап ' +
                            group + ' : ' + namestage + '</span>';
                    } else {
                ...
                if (level == 3) {
                    ...
                    if (group) {
                        return '<table width="100%"><tbody><tr><td width="40%">Договор субподряда № ' +
                            group + '</td><td width="20%">' + datedocsubpodr +
                            '</td><td width="20%">' + $.fn.dataTable.render.number(' ', ',', 2, '')
                            .display(sumdocsubpodr) + '' + koddened +
                ...
                if (level == 4) {
                    ...
                    if (group) {
                        return '<table width="100%"><tbody><tr><td width="40%">Счет-фактура # ' +
                            numberchfsubpodr + '</td><td width="20%">' + datechfsubpodr +
                            '</td><td width="20%"><span id="chf-' + rows.data().pluck(
                                "dognet_docchfsubpodr").pluck("kodchfsubpodr")[0] + '">' + $.fn
                            .dataTable.render.number(' ', ',', 2, '').display(sumchfsubpodr) + '' +
                            koddened + '</span></td><td width="20%"></td></tr></tbody></table>';
                    } else {
            ...
            endRender: function(rows, group, level) {
                ...
                if (level == 0) {
                    ...
                    for (let i = 0; i < rows.count(); i++) { // выведет 0, затем 1, затем 2
                        var flag_1 = rows.data().pluck("dognet_reports_zadolsub_chfincoming").pluck(
                            "flag_1")[i];
                        sm = Number(sm) + Number(flag_1) * Number(rows.data().pluck(
                            "dognet_docchfsubpodr").pluck("sumchfsubpodr")[i]);
                    }
                    var itogo0 = rows.data().pluck("dognet_reports_zadolsub_chfincoming").pluck(
                            "sum_incoming")
                        .reduce(function(a, b) {
                            return a + b * 1;
                        }, 0);
                ...
                if (level == 3) {
                    ...
                    var itogo3 = rows.data().pluck("dognet_reports_zadolsub_chfincoming").pluck(
                            "sum_incoming")
                        .reduce(function(a, b) {
                            return a + b * 1;
                        }, 0);
                ...
                if (level == 4) {
                    ...
                    var sumchfsubpodr0 = rows.data().pluck("dognet_docchfsubpodr").pluck(
                        "sumchfsubpodr")[0];
                    var type = rows.data().pluck("dognet_reports_zadolsub_chfincoming").pluck(
                        "type_incoming")[0];
                    var itogo4 = rows.data().pluck("dognet_reports_zadolsub_chfincoming").pluck(
                            "sum_incoming")
                        .reduce(function(a, b) {
                            return a + b * 1;
                        }, 0);
                    return '<span class="itogo-lvl3"><span class="itogo-lvl3-title">Итого по счету</span><span class="itogo-lvl3-text"><span class="itogo-lvl-lbltext2">' +
                        ...
                        $.fn.dataTable.render.number(' ', ',', 2, '').display(sumchfsubpodr0 -
                            itogo4) + '' + koddened + '</span></span></span>';
            ...
            dataSrc: ["dognet_spsubpodr.namesubpodrshot", "dognet_docbase.docnameshot",
                ...
                "dognet_reports_zadolsub_chfincoming.type_incoming"
            ],
