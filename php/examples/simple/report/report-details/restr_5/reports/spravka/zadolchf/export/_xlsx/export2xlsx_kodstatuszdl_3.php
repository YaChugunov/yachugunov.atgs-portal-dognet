...
$activeSheet->setCellValue("A{$line}", 'Невозвратная задолженность на ' . $_SPRAVKA_ONDATE);
...
//
$_queryStr .= "kodstatuszdl='3' AND koddoc IN (SELECT koddoc FROM dognet_reports_zadolchf)) ORDER BY nameshort ASC";
//
...
while ($_ROW = mysqli_fetch_assoc($_QRY)) {
    ...
    //
    $_queryStr .= "kodstatuszdl='3' AND koddoc IN (SELECT koddoc FROM dognet_reports_zadolchf)";
    //
    ...
    while ($_ROW_docbase = mysqli_fetch_assoc($_QRY_docbase)) {
        ...
        $activeSheet->mergeCells("A{$line}:H{$line}");
        // Вывод данных
        $agentname = !empty($_ROW_docbase['nameagentshort']) ? " (агент: " . $_ROW_docbase['nameagentshort'] . ")" : "";
        if ($_ROW_docbase['kodshab'] == 0) {
            $activeSheet->setCellValue("A{$line}", 'Счет №' . $_ROW_docbase['docnumber'] . " : " . $_ROW_docbase['docnameshot']);
        } else {
            $activeSheet->setCellValue("A{$line}", 'Договор №3-4/' . $_ROW_docbase['docnumber'] . " : " . $_ROW_docbase['docnameshot'] . $agentname);
        }
        // Оформляем границы
        $activeSheet->getStyle("A{$line}:H{$line}")->applyFromArray($_BORDER_BOTTOM_THIN);
        #
