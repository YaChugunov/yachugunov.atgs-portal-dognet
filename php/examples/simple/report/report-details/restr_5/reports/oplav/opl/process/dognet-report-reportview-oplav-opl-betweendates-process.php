...
if (!isset($_POST['start_date']) || !isset($_POST['end_date'])) {
    ...
    $query = "(SELECT kodchfact FROM dognet_oplatachf WHERE ";
    $query .= "dateopl BETWEEN '" . $start_date . "' AND '" . $end_date . "' AND ";
    $query .= "koddel<>'99')";
    ...
    Editor::inst($db, 'dognet_kalplanchf')
        ...
        ->on('preGet', function ($editor, $id) use ($query, $query2) {
            $editor->where(function ($q1) use ($query) {
                $q1->where('dognet_oplatachf.kodchfact', $query, "IN", false);
            });
            /*
...
*/
        ...
        // ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
        ->leftJoin('dognet_oplatachf', 'dognet_oplatachf.kodchfact', '=', 'dognet_kalplanchf.kodchfact')
        ...
        ->where('dognet_oplatachf.dateopl', $start_date, '>=')
        ->where('dognet_oplatachf.dateopl', $end_date, '<=')
        ->process($_POST)
        ->json();
}
