...
$properties->setTitle('Выполнение работ по всем договорам за период');
$properties->setDescription('Выполнение работ по всем договорам за период');
...
$properties->setSubject('Выполнение работ по всем договорам за период');
...
#
#
	include ($_SERVER['DOCUMENT_ROOT']."/dognet/php/examples/simple/report/report-details/restr_5/reports/other/docprogress/export/_xlsx/export2xlsx_docprogress.php");
if ($A1) {
...
//	include ($_SERVER['DOCUMENT_ROOT']."/dognet/php/examples/simple/report/report-details/restr_5/reports/spravka/zadolchf/export/_xlsx/export2xlsx_docprogress.php");
}
...
$filename = "REPORT-DOCPROGRESS_".$_SESSION['id']."_".date('YmdHis').".XLSX";
$xmlWriter->save($filepath.$filename);

// Делаем запись в системный лог
// Все параметры в таблице portal_log_messages
	PORTAL_SYSLOG('99942200', '0000001', null, $_GET['reportview'], "Выполнение работ договорам за период", "EXCEL");
...
function countDown(){
	//время в сек.
		...
		timer = setInterval(function(){
			...
			url2 = 'http://'+host+'/dognet/dognet-report.php?reportview=docprogress&export=yes';
			if (i < 0) {
				clearInterval(timer);
