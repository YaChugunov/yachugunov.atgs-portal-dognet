...
if (!isset($_POST['on_date'])) {
	...
	// 	$on_date = date("Y", strtotime($_POST['on_date']));
	$on_date = $_POST['on_date'];
	$query = "(SELECT kodkalplan FROM dognet_dockalplan_progress ";
	$query .= "WHERE kodkalplan IN (SELECT kodkalplan FROM dognet_kalplanchf WHERE YEAR(chetfdate) = '" . $on_date . "' AND koddel != '99') ";
	$query .= "AND srokstage_date != '0000-00-00' AND srokstage_date != 'NULL' AND srokstage_date != '')";
	// 		$query2 = "(SELECT MAX(chetfdate) FROM dognet_kalplanchf WHERE koddel != '99')";
	// 		$query3 = "(SELECT kodzakaz FROM sp_contragents WHERE namezakshot LIKE '%".$_POST['zakname']."%')";
...
*/
		...
		->leftJoin('dognet_kalplanchf', 'dognet_kalplanchf.kodkalplan', '=', 'dognet_dockalplan_progress.kodkalplan')
		...
		->where('dognet_kalplanchf.koddel', '99', '!=')
		->process($_POST)
		->json();
}
