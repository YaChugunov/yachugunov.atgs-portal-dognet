...
$activeSheet->getColumnDimension('G')->setWidth(12); // Срок
$activeSheet->getColumnDimension('H')->setWidth(20); // Сумма этапа
$activeSheet->getColumnDimension('I')->setWidth(20); // Сумма СФ
$activeSheet->getColumnDimension('J')->setWidth(20); // Не закрыто
#
...
$activeSheet->setCellValue("H{$line}", 'СУММА ЭТАПА');
$activeSheet->setCellValue("I{$line}", 'СУММА СФ');
$activeSheet->setCellValue("J{$line}", 'НЕ ЗАКРЫТО');
...
$activeSheet->getStyle("F{$line}:J{$line}")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
while ($_ROW_dockalplan = mysqli_fetch_assoc($_QRY_dockalplan)) {
	...
	$activeSheet->setCellValue("G{$line}", $_STAGESROK);
	$activeSheet->setCellValue("H{$line}", $_STAGESUM);
	$activeSheet->getStyle("H{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
	$activeSheet->setCellValue("I{$line}", $_STAGECHFSUM);
	$activeSheet->getStyle("I{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
	$activeSheet->setCellValue("J{$line}", $_STAGEZADOL);
...
// Выравнивание по горизонтали - центр
$activeSheet->getStyle("H{$line}:J{$line}")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
$activeSheet->setCellValue("A{$line}", "ИТОГО: ");
$activeSheet->setCellValue("H{$line}", $_SUM_TOTAL);
$activeSheet->getStyle("H{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
$activeSheet->setCellValue("I{$line}", $_SUM_TOTAL_CHF);
$activeSheet->getStyle("I{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
$activeSheet->setCellValue("J{$line}", $_SUM_TOTAL_ZADOL);
$activeSheet->getStyle("J{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
// Оформляем границы
$activeSheet->getStyle("A{$line}:J{$line}")->applyFromArray($_BORDER_INSIDE);
$activeSheet->getStyle("A{$line}:J{$line}")->applyFromArray($_BORDER_BOTTOM_THIN);
