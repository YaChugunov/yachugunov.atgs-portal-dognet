...
$activeSheet->getColumnDimension('F')->setWidth(12); // Срок
$activeSheet->getColumnDimension('G')->setWidth(20); // Сумма этапа
$activeSheet->getColumnDimension('H')->setWidth(20); // Сумма СФ
$activeSheet->getColumnDimension('I')->setWidth(20); // Не закрыто
#
...
$activeSheet->setCellValue("F{$line}", 'СРОК');
$activeSheet->setCellValue("G{$line}", 'СУММА ЭТАПА');
$activeSheet->setCellValue("H{$line}", 'СУММА СФ');
$activeSheet->setCellValue("I{$line}", 'НЕ ЗАКРЫТО');
...
$_QRY_dockalplan = mysqlQuery("SELECT
	...
	FROM dognet_dockalplan INNER JOIN dognet_dockalplan_progress ON dognet_dockalplan.kodkalplan = dognet_dockalplan_progress.kodkalplan WHERE dognet_dockalplan.koddel<>'99' AND dognet_dockalplan.kodkalplan IN (SELECT kodkalplan FROM dognet_dockalplan_progress WHERE summastage > sumchfstage) AND dognet_dockalplan.koddoc IN (SELECT koddoc FROM dognet_docbase WHERE kodstatus='245381842747296' OR kodstatus='245381842145343' OR kodstatus='245267756667430')");
$_SUM_TOTAL = 0.0;
$_SUM_TOTAL_CHF = 0.0;
...
while ($_ROW_dockalplan = mysqli_fetch_assoc($_QRY_dockalplan)) {
	...
	$activeSheet->setCellValue("E{$line}", $_DOCSTATUS);
	$activeSheet->setCellValue("F{$line}", $_STAGESROK);
	$activeSheet->setCellValue("G{$line}", $_STAGESUM);
	$activeSheet->getStyle("G{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
	$activeSheet->setCellValue("H{$line}", $_STAGECHFSUM);
	$activeSheet->getStyle("H{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
	$activeSheet->setCellValue("I{$line}", $_STAGEZADOL);
...
$activeSheet->setCellValue("G{$line}", $_SUM_TOTAL);
$activeSheet->getStyle("G{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
$activeSheet->setCellValue("H{$line}", $_SUM_TOTAL_CHF);
$activeSheet->getStyle("H{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
$activeSheet->setCellValue("I{$line}", $_SUM_TOTAL_ZADOL);
$activeSheet->getStyle("I{$line}")->getNumberFormat()->setFormatCode(PRICE_FORMAT_1);
// Оформляем границы
$activeSheet->getStyle("A{$line}:I{$line}")->applyFromArray($_BORDER_INSIDE);
$activeSheet->getStyle("A{$line}:I{$line}")->applyFromArray($_BORDER_BOTTOM_THIN);
