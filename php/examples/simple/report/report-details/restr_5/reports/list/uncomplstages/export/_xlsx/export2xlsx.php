...
$properties->setTitle('Перечень незакрытых этапов');
$properties->setDescription('Перечень незакрытых этапов');
...
$properties->setSubject('Перечень незакрытых этапов');
...
#
#
	include ($_SERVER['DOCUMENT_ROOT']."/dognet/php/examples/simple/report/report-details/restr_5/reports/list/uncomplstages/export/_xlsx/export2xlsx_uncomplstages.php");
if ($A1) {
...
$filename = "LIST-STAGEUNCOMPL_".$_SESSION['id']."_".date('YmdHis').".XLSX";
...
function countDown(){
	//время в сек.
		...
		timer = setInterval(function(){
			...
			url2 = 'http://'+host+'/dognet/dognet-report.php?reportview=uncomplstages&export=yes';
			if (i < 0) {
				clearInterval(timer);
