			...
			<div class="col-xs-12 col-sm-12 col-md-6 col-md-offset-3 col-lg-6 col-lg-offset-3">
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 space20">
					<div id="report-settings" class="text-center">
						<h3 class="space10">Письмо заказчику о задолженности по договору/этапу договора</h3>
				...
				<div class="col-xs-hidden col-sm-12 col-md-12 col-lg-12">
					<div id="export-format-block" class="center-block" style="">
						<form class="form-horizontal" method="GET" action="/dognet/dognet-report.php">
							...
							<input type="hidden" name="zak" value="<?php echo (isset($_GET['zak']) && !empty($_GET['zak'])) ? $_GET['zak'] : ""; ?>">
							<div class="col-xs-hidden col-sm-12 col-md-12 col-lg-12">
								<!--
...
-->
								<div class="form-group text-left">
									<label for="chf">
										<h4>Выберите счета-фактуры для письма</h4>
									</label>
									<select id="selectCHF" class='form-control' name='chfID[]' multiple size='10' style="font-size:0.9em; min-height:205px">
										<?php
										...
										$_QRY_B = mysqlQuery("SELECT * FROM sp_contragents WHERE kodzakaz = '" . $_GET['zak'] . "'");
										...
										while ($_ROW_A = mysqli_fetch_assoc($_QRY_A)) {
											...
											while ($_ROW = mysqli_fetch_assoc($_QRY)) {
										?>
												<option value='<?php echo $_ROW["kodchfact"]; ?>' selected="selected"><?php echo 'С/Ф №' . $_ROW["chetfnumber"] . ' от ' . date("d.m.Y", strtotime($_ROW["chetfdate"])) . ' / Договор №3-4/' . $_ROW_A["docnumber"] . ' от ' . $docdate . ' / ' . number_format($_ROW["summazadol"], 2, '.', ' ');; ?></option>
										<?php
											}
										}
										?>
								...
								<div class="text-left">
									<div class="form-group">
										...
										<div class="checkbox text-left">
											<label><input id="fio-manual" type="checkbox" name="fio_manual" value="yes">Ввести данные Заказчика вручную</label>
										</div>
										<div id="div-fio-manual" style="display:none">
											<div class="radio" style="margin-bottom:20px">
												<label class="control-label col-sm-4" for="zak-hello" style="text-align:left"><span>Обращение</span></label>
												<div class="col-sm-8">
													<input type="text" class="form-control" id="zak-hello" name="zak_hello" placeholder="Обращение" disabled>
												</div>
											</div>
											<div class="radio" style="margin-bottom:20px">
												<div class="col-sm-4">
													<input type="text" class="form-control" id="zak-lastname" name="zak_lastname" placeholder="Фамилия" disabled>
												</div>
												<div class="col-sm-4">
													<input type="text" class="form-control" id="zak-firstname" name="zak_firstname" placeholder="Имя" disabled>
												</div>
												<div class="col-sm-4">
													<input type="text" class="form-control" id="zak-midname" name="zak_midname" placeholder="Отчество" disabled>
												</div>
											</div>
											<div class="radio" style="margin-bottom:20px">
												<label class="control-label col-sm-4" for="zak-org" style="text-align:left"><span>Организация</span></label>
												<div class="col-sm-8">
													<input type="text" class="form-control" id="zak-org" name="zak_org" placeholder="Организация" disabled>
												</div>
											</div>
											<div class="radio" style="margin-bottom:20px">
												<label class="control-label col-sm-4" for="zak-dolg" style="text-align:left"><span>Должность</span></label>
												<div class="col-sm-8">
													<input type="text" class="form-control" id="zak-dolg" name="zak_dolg" placeholder="Должность" disabled>
												</div>
											</div>
										</div>
									</div>
							...
							<div class="col-xs-hidden col-sm-12 col-md-12 col-lg-12 space30">
								<div class="text-left">
									<div class="form-group">
										...
										<div class="col-sm-8">
											<div class="radio" style="margin-bottom:20px">
												<input type="text" class="form-control" id="isp-name" name="isp_name" placeholder="ФИО Исполнителя">
											</div>
										</div>
										<div class="col-sm-6">
											<input type="text" class="form-control" id="isp-tel" type="tel" name="isp_tel" placeholder="Телефон">
										</div>
										<div class="col-sm-6">
											<input type="text" class="form-control" id="isp-email" type="email" name="isp_email" placeholder="Email">
										</div>
									</div>
								</div>
							</div>
							<div class="col-xs-hidden col-sm-12 col-md-12 col-lg-12 space30">
								<div class="checkbox text-left">
									<label><input type="checkbox" name="log" value="yes" checked="checked">Сделать запись в "Журнале писем"</label>
								</div>
								<div class="checkbox text-left">
									<label><input type="checkbox" name="save" value="yes" checked="checked">Сохранить файл письма на сервере (будет доступен в "Журнале писем")</label>
								</div>
							</div>
							<div class="col-xs-hidden col-sm-12 col-md-12 col-lg-12 space30">
...
-->
							...
							<div>
								<button id="exportBtn" class="btn btn-default" type="submit" name="done" value="ok">Эскпортировать</button>
							</div>
						</form>
					</div>
				</div>
		...
		<div class="col-xs-12 col-sm-12 col-md-8 col-md-offset-2 col-lg-8 col-lg-offset-2">
			<div class="text-center">
				...
				if (isset($_GET['format'])) {
					switch ($_GET['format']) {
						case "doc":
							include($_SERVER['DOCUMENT_ROOT'] . "/dognet/php/examples/simple/report/report-details/restr_5/reports/letter/zakzadolchf_ondate/export/_docx/export2docx.php");
							break;
						case "xls":
							include('');
							break;
