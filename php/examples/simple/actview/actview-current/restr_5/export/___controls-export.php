				...
				<div id="export-format-block" class="center-block" style="">
					<form class="form-horizontal" method="GET" action="/dognet/dognet-actview.php">
						...
						<div class="col-xs-hidden col-sm-12 col-md-12 col-lg-12 space30">
							...
							<div id="filesave-enbl" class="checkbox text-left" style="display:none">
								<label><input type="checkbox" name="save" value="yes">Сохранить файл акта на сервере (будет доступен в "Журнале актов")</label>
							</div>
						</div>
						<div class="col-xs-hidden col-sm-12 col-md-12 col-lg-12 space30">
...
-->
						...
						<div>
							<button class="btn btn-default" type="submit" name="done" value="ok" disabled="disabled">Эскпортировать</button>
						</div>
					</form>
				</div>
			</div>
...
<?php
				if (isset($_GET['format'])) {
					switch ($_GET['format']) {
						case "doc":
							include($_SERVER['DOCUMENT_ROOT']."/dognet/php/examples/simple/actview/actview-current/restr_5/export/_docx/export2docx.php");
							break;
						case "xls":
							include($_SERVER['DOCUMENT_ROOT']."/dognet/php/examples/simple/actview/actview-current/restr_5/export/_xlsx/export2xlsx.php");
							break;
						case "pdf":
							include($_SERVER['DOCUMENT_ROOT']."/dognet/php/examples/simple/actview/actview-details/restr_5/export/_pdf/export2pdf.php");
							break;
						default:
...
<script type="text/javascript">
	...
	$('select[name="uniqueID2"]').change(function(){
		if ($('select[name="uniqueID2"]').val() == "") {
			$('button[name="done"]').prop('disabled', true);
			console.log('disabled');
		}
		else {
			$('button[name="done"]').prop('disabled', false);
			console.log('enabled');
		}
	});
