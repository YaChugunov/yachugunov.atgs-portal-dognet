...
#
#
	$__rowFileID = $db->sql( "SELECT docFileID FROM dognet_docjurnalact WHERE id=".$id )->fetchAll();
	if (!empty($__rowFileID)) {
		$row2delete = $__rowFileID[0]['docFileID'];
		if ($row2delete!="") {
		// Удаление оригинального файла ($__tmp2) и сим-ссылки ($__tmp1) с диска
			$__file = $db->sql( "SELECT file_truelocation, file_syspath FROM dognet_docjurnalact_files WHERE id=".$row2delete )->fetchAll();
			if(file_exists($__file[0]['file_syspath'])) {
				$__tmp1 = unlink($__file[0]['file_syspath']);
			}
			if($__file[0]['file_truelocation']) {
				$__tmp2 = unlink($__file[0]['file_truelocation']);
			}
			// Удаление записи в таблице файлов
// 			if ( $__tmp1 && $__tmp2 ) {
				$query = $db->sql( "DELETE FROM dognet_docjurnalact_files WHERE id=".$row2delete );
// 			}
		}
	}
#
