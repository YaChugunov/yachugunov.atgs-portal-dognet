...
//
$innerTable->addRow(100, $_TBL_BODYStyle_Row);
$innerTable->addCell(10000, $_TBL_BODYStyle_Cell_Inner)->addTextRun($_TBL_CELLAlign_H_Center)->addText('КПП 772801001', $_FontStyle_Doc_P10);
//
$innerTable->addRow(100, $_TBL_BODYStyle_Row);
...
// -- + -- + -- + -- + -- + -- + -- + --
// Запрашиваем и выводим реквизиты контрагентов (kodcontragent)
...
$_QRY_ORG2_KONTR = mysqlQuery("SELECT * FROM sp_contragents WHERE kodcontragent = '".$_ROW_docbase['kodzakaz']."'");
...
//
$opfabbr = '';
$kodformlegal = $_ROW_ORG2_KONTR['kodformlegal'];
...
/**
 * Редакция 14.12.2023
 * Заменяем старое поле банковских реквизитов zakbankch на новое склеенное из отдельных полей
 */
$zakbankch_str1 = ! empty($_ROW_ORG2_KONTR['zakbankch']) ? $_ROW_ORG2_KONTR['zakbankch'] : 'Р/с '.str_replace(' ', '', $_ROW_ORG2_KONTR['payment_account']).' в банке '.$_ROW_ORG2_KONTR['payment_bank'];
$zakbankch_str2 = ! empty($_ROW_ORG2_KONTR['correspondent_account']) ? 'К/с '.str_replace(' ', '', $_ROW_ORG2_KONTR['correspondent_account']) : '';
$zakbankch_str3 = ! empty($_ROW_ORG2_KONTR['bik']) ? 'БИК '.$_ROW_ORG2_KONTR['bik'] : '';
// -- + -- + -- + -- + -- + -- + -- + --
//
//
//
...
//
$innerTable->addRow(100, $_TBL_BODYStyle_Row);
$innerTable->addCell(10000, $_TBL_BODYStyle_Cell_Inner)->addTextRun($_TBL_CELLAlign_H_Center)->addText('КПП '.$_ROW_ORG2_KONTR['kpp'], $_FontStyle_Doc_P10);
//
$innerTable->addRow(100, $_TBL_BODYStyle_Row);
$innerTable->addCell(10000, $_TBL_BODYStyle_Cell_Inner)->addTextRun($_TBL_CELLAlign_H_Center)->addText($zakbankch_str1, $_FontStyle_Doc_P10);
//
$innerTable->addRow(100, $_TBL_BODYStyle_Row);
$innerTable->addCell(10000, $_TBL_BODYStyle_Cell_Inner)->addTextRun($_TBL_CELLAlign_H_Center)->addText($zakbankch_str2, $_FontStyle_Doc_P10);
//
$innerTable->addRow(100, $_TBL_BODYStyle_Row);
$innerTable->addCell(10000, $_TBL_BODYStyle_Cell_Inner)->addTextRun($_TBL_CELLAlign_H_Center)->addText($zakbankch_str3, $_FontStyle_Doc_P10);
//
$innerTable->addRow(100, $_TBL_BODYStyle_Row);
...
$textrun->addText('В том числе НДС (22%) составляет: ', $_FontStyle_Doc_P11);
// Считаем НДС
$_ACT_NDS = ($_ACT_SUMMA != '') ? ($_ACT_SUMMA / (1 + 0.22)) * 0.22 : 0.0;
...
function countDown() {
    ...
    timer = setInterval(function() {
        ...
        url2 = 'http://' + host + '/dognet/dognet-actview.php?actview_type=list&export=yes&uniqueID1=' +
            uniqueID1 + '&uniqueID2=' + uniqueID2;
        if (i < 0) {
            clearInterval(timer);
